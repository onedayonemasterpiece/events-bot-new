...
async def ensure_kaggle_status_tables(conn: Any) -> None:
    await conn.execute(
        """
    ...
    try:
        await conn.execute("ALTER TABLE kaggle_run_event ADD COLUMN event_uid TEXT")
    except Exception:
        pass
    await conn.execute("CREATE INDEX IF NOT EXISTS ix_kaggle_run_event_run_id ON kaggle_run_event(run_id, seq)")
    await conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_kaggle_run_event_uid ON kaggle_run_event(run_id, event_uid) WHERE event_uid IS NOT NULL")
    await conn.execute("CREATE INDEX IF NOT EXISTS ix_kaggle_run_event_created_at ON kaggle_run_event(created_at)")
    await conn.execute(
        """
    ...
    await conn.execute("CREATE INDEX IF NOT EXISTS ix_kaggle_resource_lease_status ON kaggle_resource_lease(status, expires_at)")
...
async def create_kaggle_run_config(
    ...
    *,
    ...
    async with db.raw_conn() as conn:
        await ensure_kaggle_status_tables(conn)
        await conn.execute(
            """
...
async def record_kaggle_run_event(db: Database, payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    ...
    async with db.raw_conn() as conn:
        ...
        terminal_at = now if event in TERMINAL_EVENTS and status in {"done", "failed", "error"} else None
