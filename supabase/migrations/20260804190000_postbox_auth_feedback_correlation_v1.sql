...
alter table personalization.focus_auth_delivery_attempt
  add column if not exists postbox_event_count integer not null default 0;
...
alter table personalization.focus_auth_delivery_attempt
  ...
  check (postbox_event_count between 0 and 1000);
...
grant select on email_control.postbox_message_correlation to service_role;
