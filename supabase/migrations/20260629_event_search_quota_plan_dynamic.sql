-- MVP dynamic quota baseline for authorized event search.
-- Current provider facts documented in docs/features/unsigned-personalization/model-selection.md:
-- - gemini-embedding-2: 1000 RPD per AI Studio project
-- - gemma-4-26b-a4b-it: 1500 RPD per AI Studio project
-- The Edge Function currently uses one Google project/key path directly, not the
-- shared Python GoogleAIClient limiter, so the quota plan reserves provider RPD
-- for static generation and caps per-user abuse until central limiter routing is wired.

create or replace function public.refresh_registered_search_quota_v1(
  p_embedding_rpd integer default 1000,
  p_llm_rpd integer default 1500,
  p_embedding_static_reserve integer default 400,
  p_llm_static_reserve integer default 500,
  p_min_daily_search integer default 10,
  p_max_daily_search integer default 50,
  p_min_daily_llm integer default 5,
  p_max_daily_llm integer default 40,
