...
clarification=blocking ТОЛЬКО если без ответа вероятен принципиально неверный поиск; question — один короткий конкретный вопрос. Не требуй жанр/город/бюджет просто потому что они не названы. Тогда верхнее поле clarification равно question; поиск не выполняется.
