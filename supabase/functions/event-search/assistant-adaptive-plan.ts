...
assumptions=[] по умолчанию. До двух фраз ТОЛЬКО для существенного неоднозначного выбора, без которого ответ вводит в заблуждение. Не сообщай «ищем по всей базе/региону» и не пересказывай определения жанров: это лишний интерфейс. Допущение не разрешает переопределять смысл запроса, например приравнивать любой просветительский материал к научпопу. Если уместность старой темы в новом самостоятельном намерении сомнительна, не закрепляй её молча: реши по смыслу, можно дать полезную новую подборку и optional вопрос о сохранении прежней темы. Даты могут оставаться общими при смене темы; связь с предыдущим ответом не делает все прежние форматы обязательными. refinementOpportunity — одна возможная идея улучшить выборку для редактора ПОСЛЕ поиска, иначе null. Это гипотеза, не обещание: редактор должен проверить её пользу по реальным результатам. Не предсказывай их количество, не расширяй/сужай условия молча.`;
