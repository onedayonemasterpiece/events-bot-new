...
async function harness(fn,{enabled=true,deny=false,partial=false,timeout=false,budgetMs=20000}={}){
 ...
 const body=JSON.parse(opts.body);assert.match(body.contents[0].parts[0].text,/название площадки/);assert.match(body.contents[0].parts[0].text,/2026-09-13/);assert.equal(body.generationConfig.maxOutputTokens,2048);assert.deepEqual(body.generationConfig.responseJsonSchema.properties.classifications.required,['7422','8680']);
 return new Response(JSON.stringify({candidates:[{content:{parts:[{text:JSON.stringify({classifications:partial?{'7422':'exact'}:{'7422':'exact','8680':'rejected'}})}]}}],usageMetadata:{promptTokenCount:50,candidatesTokenCount:30,totalTokenCount:80}}),{status:200});};
 try{return await fn({backend,counters,calls,verify:(facts=digests)=>llmVerify(intent.goal,rows,facts,{voiceIntent:intent,deadline:Date.now()+budgetMs,gemma_overflow_allowed:false,quota_backend:backend,counters})});}
...
test('sub-minimum voice deadline refuses a paid lease and provider dispatch',()=>harness(async({verify,calls,counters})=>{
 const r=await verify();assert.equal(r.used,false);assert.match(r.status,/verification_budget_exhausted/);assert.equal(counters.llm_provider_attempts,0);assert.ok(!calls.includes('google_ai_reserve'));assert.ok(!calls.includes('provider'));
},{budgetMs:8000}));
