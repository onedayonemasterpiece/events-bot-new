...
export function planVerifierSchema(candidates:Card[],plan:SemanticPlan){
 ...
 const verdict={type:'object',additionalProperties:false,required:['verdict','fact_index'],properties:{verdict:{type:'string',enum:['exact','possible','rejected']},fact_index:{type:'integer',minimum:-1,maximum:100}}};
...
export function classifyPlanPayload(parsed:Record<string,any>,candidates:Card[],plan:SemanticPlan,facts:Map<number,string>){
 ...
 for(const id of ids){
  ...
  for(const key of keys){const v=groups[key];if(!v||typeof v!=='object'||Object.keys(v).some(k=>!['verdict','fact_index'].includes(k))||!['exact','possible','rejected'].includes(v.verdict)||!Number.isInteger(v.fact_index)||v.fact_index< -1||v.fact_index>=lines.length||v.verdict==='exact'&&v.fact_index<0)return malformed();verdicts.push(v.verdict);}
