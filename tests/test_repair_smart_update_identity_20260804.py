...
def _make_db(path: Path) -> None:
    ...
    for event_id, title, date, time, ticket in events:
        con.execute(
            "INSERT INTO event(id,title,date,time,ticket_link,source_text,source_texts,telegraph_url,telegraph_path,ics_url,is_free,ticket_price_min,ticket_price_max,age_restriction_status) VALUES(?,?,?,?,?,'legacy','[]',?,?,?,0,1500,2000,'conflict')",
    ...
    for poster_id, event_id in poster_events.items():
        con.execute("INSERT INTO eventposter(id,event_id,review_status) VALUES(?,?,'pending')", (poster_id, event_id))
    con.commit()
    con.close()
