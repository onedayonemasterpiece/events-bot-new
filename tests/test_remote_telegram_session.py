...
@pytest.mark.asyncio
async def test_running_kenigsberg_story_job_is_busy(monkeypatch):
    async def fake_list_jobs(job_type=None):
        return [
            {
                "type": "kenigsberg_story",
                "kernel_ref": "zigomaro/koenigsberg-stories",
                "created_at": "2026-06-12T18:30:00+00:00",
                "meta": {
                    "run_id": "kenigsberg-session-421",
                    "session_id": 421,
                    "remote_telegram_auth_scope": "TELEGRAM_AUTH_BUNDLE_STORY",
                },
            }
        ]
    ...
    conflicts = await guard.find_remote_telegram_session_conflicts(
        ...
        current_auth_scope="TELEGRAM_AUTH_BUNDLE_STORY",
    )

    assert len(conflicts) == 1
    assert conflicts[0].job_type == "kenigsberg_story"
    assert conflicts[0].status == "RUNNING"
    assert conflicts[0].auth_scope == "TELEGRAM_AUTH_BUNDLE_STORY"


@pytest.mark.asyncio
async def test_different_auth_scope_remote_job_is_not_busy(monkeypatch):
    ...
    class FakeKaggleClient:
        def get_kernel_status(self, kernel_ref):
            status_calls.append(kernel_ref)
            return {"status": "RUNNING", "failureMessage": ""}
