...
test('checked inventory is the exact eight-fixture Astro-generated output', () => {
  ...
  assert.equal(inventory.conformance_tooling_repository_sha, 'c4d9dca3e3a77c94e66df6a228f31fccb3871527');
  ...
  const absent = inventory.cases.find((row) => row.fixture_id === 'event.real.7888');
  assert.equal(absent.admission.component_id, 'event.meta.admission');
  assert.equal(absent.admission.state, 'absent');
  assert.equal(absent.admission.visible, false);
  assert.equal(absent.admission.label, null);
  assert.equal(absent.source_rendered.admission.label, 'Условия уточняются');
  assert.equal(absent.domain_evidence.admission_cta_component, 'not_applicable');
  assert.equal(absent.admission_cta, undefined);
  assert.equal(absent.actions.like.component_id, 'event.action.like');
  assert.equal(absent.social_proof.like.component_id, 'event.social-proof.like');
  assert.equal(absent.social_proof.like.count, 53);
