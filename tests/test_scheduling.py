...
@pytest.mark.asyncio
async def test_critical_scheduler_watchdog_defers_guide_after_failed_full_run(
    tmp_path, monkeypatch
):
    db = Database(str(tmp_path / "db.sqlite"))
    await db.init()
    _configure_guide_critical_env(monkeypatch)
    monkeypatch.setenv("GUIDE_MONITORING_FAILED_RETRY_SECONDS", "900")
    scheduling._critical_catchup_inflight.clear()
    scheduling._critical_catchup_completed.clear()
    scheduling._critical_catchup_deferred_until.clear()
    ...
    run_id = await start_ops_run(
        ...
        trigger="scheduled",
        operator_id=0,
        started_at=datetime(2026, 4, 13, 20, 51, tzinfo=timezone.utc),
        details={"mode": "full", "run_id": "failed-kaggle"},
    )
    await finish_ops_run(
        db,
        run_id=run_id,
        status="error",
        finished_at=datetime(2026, 4, 13, 20, 55, tzinfo=timezone.utc),
        details={
            "mode": "full",
            "run_id": "failed-kaggle",
            "errors": ["Guide Kaggle kernel failed (failed)"],
        },
    )
    ...
    _FixedCriticalSchedulerDatetime.fixed_now = datetime(
        2026, 4, 13, 21, 5, tzinfo=timezone.utc
    )
    first_dispatched = await scheduling.maybe_dispatch_critical_scheduler_watchdog(
        db, bot=object()
    )
    _FixedCriticalSchedulerDatetime.fixed_now = datetime(
        2026, 4, 13, 21, 11, tzinfo=timezone.utc
    )
    scheduling._critical_catchup_deferred_until.clear()
    second_dispatched = await scheduling.maybe_dispatch_critical_scheduler_watchdog(
        db, bot=object()
