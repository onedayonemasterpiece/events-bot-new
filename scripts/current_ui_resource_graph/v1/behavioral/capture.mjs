...
async function settlePlanImages(root,plan){
  const policy=plan.screenshot_stabilization?.images;
  if(policy!=='eager-complete-bounded')return {status:'not-requested',policy:null,image_count:0,timeout_ms:null};
  const timeoutMs=plan.screenshot_stabilization.timeout_ms;
  try{
    const result=await bounded(`Behavior ${plan.id} image settle`,root.evaluate(async(node)=>{
      ...
      await Promise.all(images.map((image)=>image.naturalWidth>0?image.decode().catch(()=>{}):Promise.resolve()));
      await new Promise((done)=>requestAnimationFrame(()=>requestAnimationFrame(done)));
      return {image_count:images.length,complete_count:images.filter((image)=>image.complete).length,loaded_count:images.filter((image)=>image.naturalWidth>0).length,error_count:images.filter((image)=>image.complete&&image.naturalWidth===0).length};
    }),timeoutMs);
    return {status:'settled',policy,timeout_ms:timeoutMs,instrumentation:'test-only-loading-eager',...result};
  }catch(error){throw new Error(`Behavior ${plan.id} image settle failed after ${timeoutMs}ms: ${String(error?.message||error).slice(0,320)}`,{cause:error});}
}
...
async function capturePhase({page,root,plan,phase,outputDir,imageComparator,telemetryState,index,transition,actionReceipts}){
  const image_settle=await settlePlanImages(root,plan);const font_settle=await settleFontsBounded(page);const name=`${plan.id.slice('behavior-packet.'.length)}-${String(index).padStart(2,'0')}-${phase}.png`;const path=join(outputDir,'behavior-rasters',name);
