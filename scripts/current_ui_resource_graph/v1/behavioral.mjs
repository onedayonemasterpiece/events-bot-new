...
export function assertBehavioralSupplement(root,{allowIncomplete=false}={}) {
  ...
  if(manifest.status===BEHAVIOR_STATUS){
    if(manifest.supplement_id!=='behavioral-supplement-v1.1-snapshot-20260808T124842-4786ac53bc'||plans.length!==50||observations.length!==99||plans.some((row)=>!['captured-and-reviewed','explicit-blocker'].includes(row.capture_status))) throw new Error('Ready supplement has incorrect identity, counts, or uncaptured plans');
    if(plans.filter((row)=>row.capture_status==='explicit-blocker').length!==5) throw new Error('Ready supplement exact blocker coverage mismatch');
