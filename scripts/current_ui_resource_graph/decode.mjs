...
import {
  ...
  observedFamilies, outputHashes, pageFamiliesFromSource, parseArgs, redactFactory,
  scanLocalRuntime, scanRemoteRuntime, screenshotIndex, sha256, stableJson,
  styleObservations, validateSourcePin, writeDeterministic,
} from './graph-lib.mjs';
...
async function fetchManifest(base, redact, maxBytes) {
  ...
  if (!response.ok) throw new Error(redact(`Candidate manifest fetch failed (${response.status})`));
...
function validateRuntimeIdentity(manifest, bytes, identities, verifyProduction) {
  const actual = {
    ...
    html_count: manifestHtmlFiles(manifest).length,
    page_count: manifest.counts?.page_count ?? null,
    file_count: manifest.counts?.file_count ?? manifest.files?.length ?? null,
    bytes: manifest.files?.reduce((sum, file) => sum + Number(file.size || 0), 0) ?? null,
  };
...
function summaryMarkdown(identity, counts, families, coverage) {
  ...
  return `# Current UI Resource Graph v0\n\n` +
    ...
    `- Current public root release: \`${identity.current_root_prelaunch.release_id}\` (source \`${identity.current_root_prelaunch.source_sha}\`, published ${identity.current_root_prelaunch.published_at}).\n` +
...
async function run(argv) {
  ...
  const deterministicTime = DEFAULT_IDENTITIES.snapshot_time;
  writeFileSync(receiptPath, stableJson({ schema_version: SCHEMA, status: 'started', snapshot_time: deterministicTime }, true));
  try {
    ...
    const rootOverrides = {
      ...
      published_at: args.root_published_at, runtime_url: args.root_runtime_url,
    };
    ...
    const sourceRecords = await inventorySource(sourceRoot, parsers);
    const runtime = args.runtime_root
    ...
    let styles = await styleObservations(sourceRoot, sourceRecords, parsers);
    const families = observedFamilies(sourceRecords, runtime);
    const pageFamilies = pageFamiliesFromSource(sourceRoot, sourceRecords, runtime);
    const candidates = candidateGraph(families, sourceRecords);
    ...
    if (bool(args.browser_observations, false)) {
      const browserResult = await captureBrowserEvidence({
        candidateBase, manifest: runtimeManifest, runtimeObservations: runtime, families,
    ...
    const fragmentation = fragmentationReport(families, styles);
    const desktop = desktopMobile(families, runtime, viewportEvidence);
    ...
    const manifest = {
      ...
      identity_invariant: 'candidate_is_not_production_root', source_pin: sourcePin,
      runtime_manifest: runtimeFacts, counts, coverage_results: Object.fromEntries(['FOUND', 'MISSING', 'DISCOVERED', 'AMBIGUOUS'].map((status) => [status, coverage.filter((row) => row.status === status).length])),
    ...
    writeFileSync(receiptPath, stableJson({ schema_version: SCHEMA, status: 'failed', snapshot_time: deterministicTime, error: message }, true));
