...
SEARCH_CHANGED_SURFACES_ENV="${SEARCH_CHANGED_SURFACES:-}"
SEARCH_CHANGED_SURFACES=()
if [[ -n "$SEARCH_CHANGED_SURFACES_ENV" ]]; then
  IFS=',' read -r -a SEARCH_CHANGED_SURFACES <<<"$SEARCH_CHANGED_SURFACES_ENV"
fi
unset SEARCH_CHANGED_SURFACES_ENV
FLY_ARGS=()
