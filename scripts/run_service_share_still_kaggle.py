#!/usr/bin/env python3
"""Stage, run and audit the research-only service-share Blender still on Kaggle."""
...
def validate_output(out_dir: Path, config: dict, *, require_bundle_sha: str) -> dict:
    ...
    if not result.get("ok") or not result.get("research_only"):
...
def main() -> int:
    ...
    run_id = f"service-share-still-{profile}-{run_stamp}"
    ...
    with LOCK_PATH.open("w") as lock_file:
        ...
        with tempfile.TemporaryDirectory(prefix="service-share-still-") as temp:
            ...
            config = {
                ...
                "current_event_count": 274,
                "new_event_count_7d": 75,
                "research_only": True,
            }
            (dataset_dir / "render_config.json").write_text(json.dumps(config, ensure_ascii=False, indent=2) + "\n")
            (dataset_dir / "kaggle_run.json").write_text(json.dumps({
                "run_id": run_id,"session_id":None,"kind":"service_share_still","notebook":"ServiceShareStill",
                "callback_url":"","token":"","resource_leases":[],
            ...
            try:
                ...
                wait_dataset_ready(client, dataset_ref, ["service_share_bundle.tarball","render_config.json","kaggle_run.json","kaggle_status_client.py"])
                client.push_kernel(kernel_path=kernel_stage, dataset_sources=[dataset_ref])
                ...
                receipt = {"ok":True,"run_id":run_id,"kernel_ref":kernel_ref,"dataset_ref":dataset_ref,"artifact_dir":str(artifact_dir),"result":result}
                ...
                if not args.keep_dataset:
                    try:
                        ...
                        print(f"[service-share-kaggle] deleted dataset={dataset_ref}", flush=True)
                    except Exception as exc:
                        print(f"[service-share-kaggle] dataset cleanup failed: {exc}", flush=True)
