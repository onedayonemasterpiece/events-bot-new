...
def build(ui_sot_root: Path, output: Path, registry_output: Path) -> None:
    ...
    scenario_path = ui_sot_root / "catalog/fixtures/design-system-reference/v2/scenarios/archetype.collections.free.september.desktop-ready.v3.json"
    mobile_scenario_path = ui_sot_root / "catalog/fixtures/design-system-reference/v2/scenarios/archetype.collections.free.september.mobile-ready.v3.json"
    ...
    mobile_scenario = read_json(mobile_scenario_path)
    ...
    if scenario["scenario_id"] != "free-collection-september-desktop-v3":
        raise SystemExit("unexpected reference scenario")
    if mobile_scenario["scenario_id"] != "free-collection-september-mobile-v3":
        raise SystemExit("unexpected mobile reference scenario")
    ...
    if mobile_scenario["fixture_input_order"] != scenario["fixture_input_order"]:
        raise SystemExit("desktop/mobile scenario fixture input order drift")
    if scenario["reference_clock"] != mobile_scenario["reference_clock"]:
        raise SystemExit("desktop/mobile scenario clock drift")
    clock = scenario["reference_clock"]
    if clock["reference_iso"] != corpus["reference_clock"]["reference_iso"]:
        raise SystemExit("scenario/corpus reference clock drift")
    ...
    frozen = {
        ...
        "authority": {
            ...
            "scenario": "catalog/fixtures/design-system-reference/v2/scenarios/archetype.collections.free.september.desktop-ready.v3.json",
            "scenario_sha256": raw_sha256(scenario_path),
            "mobile_scenario": "catalog/fixtures/design-system-reference/v2/scenarios/archetype.collections.free.september.mobile-ready.v3.json",
            "mobile_scenario_sha256": raw_sha256(mobile_scenario_path),
    ...
    runtime_registry = {
        ...
        "scenarios": {
            current["scenario_id"]: {
                "route": current["route_ref"]["route"],
                "viewport": current["viewport"],
                "reference_timestamp": current["reference_clock"]["reference_iso"],
                "reference_date": current["reference_clock"]["current_date"],
                ...
                "review_states": current["acceptance"]["review_states"],
            }
            for current in (scenario, mobile_scenario)
        },
    }
