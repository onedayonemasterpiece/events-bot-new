...
const shareSource = fs.readFileSync(path.join(siteRoot, 'src/components/ServiceShareAction.astro'), 'utf8');
const shareControllerSource = fs.readFileSync(path.join(siteRoot, 'src/lib/service-share/controller.js'), 'utf8');
...
const checks = [
  ...
  ['shared footer owns scoped physical P/S hotkeys and desktop-only keycaps', shareSource.includes('data-service-shortcut-badge') && shareSource.includes("surface === 'footer'") && shareSource.includes('@media (min-width: 1024px)') && shareControllerSource.includes("['KeyP'") && shareControllerSource.includes("['KeyS'") && shareControllerSource.includes("root.addEventListener('keydown'") && shareControllerSource.includes('event.defaultPrevented')],
  ...
  ['photo deck manifest keeps real source geometry and responsive sources', rowSource.includes('width: asset.width') && rowSource.includes('height: asset.height') && rowSource.includes('srcset: asset.thumbnailSrcset') && pageSource.includes('image.width = Number(asset.width || 1)')],
  ...
  ['crop decisions are inspectable on rebound frames', rowSource.includes('data-deck-manifest={JSON.stringify(deckMedia)}') && pageSource.includes('frame.dataset.mediaToken = asset.token') && pageSource.includes('frame.dataset.cropLoss = Number(asset.cropLoss || 0).toFixed(5)') && pageSource.includes('frame.dataset.cropReason = asset.cropReason')],
  ...
  ['deck uses real conditional right-overlap frames', !rowSource.includes('ex-deck__edge-stack') && rowSource.includes('data-deck-frame') && pageSource.includes("state:'stack'") && pageSource.includes('firstLeft + stackIndex * sliver') && pageSource.includes('String(120 - placement.depth)')],
  ...
  ['deck depth uses shrinking directional planes while hover remains geometrically static', pageSource.includes('const depthScale = 1 / (1 + depth * .05)') && pageSource.includes('frame.style.blockSize = `${placement.height}px`') && pageSource.includes('calc(7px - var(--deck-depth,1) * .8px)') && !pageSource.includes('scale(1.04)')],
  ...
  ['bounded virtual deck turns the fifth depth plane neutral until dealt', rowSource.includes('const deckSlotCount = Math.min(7') && rowSource.includes('data-deck-slot={String(slotIndex)}') && rowSource.includes('ex-deck__depth-plane') && pageSource.includes("placement.depth < MAX_STACK_DEPTH") && pageSource.includes("frame.dataset.deckVisual = showImage ? 'media' : 'depth-tail'")],
  ...
  ['desktop media gallery preserves native mobile and modified-click navigation', pageSource.includes('const galleryOpener = target.closest(\'[data-gallery-open]\')') && pageSource.includes('!compactDeck.matches && !modifiedClick') && pageSource.includes('openGallery(galleryOpener,row);') && pageSource.includes('if (deckState.phase !== \'idle\')')],
  ...
  ['deck images expose reusable skeleton, cached-load, and error lifecycle', rowSource.includes('data-image-state="idle"') && rowSource.includes('data-image-skeleton') && pageSource.includes("frame.dataset.imageState = 'loading'") && pageSource.includes("image.complete ? (image.naturalWidth > 0 ? 'loaded' : 'error')") && pageSource.includes("settleDeckImage(frame,image,requestId,'error')")],
  ...
  ['deck counter means actual non-full media on the active page', rowSource.includes('data-media-total') && pageSource.includes('remaining:Math.max(0,total - next)') && pageSource.includes('count.textContent = `+${page.remaining}`')],
  ...
  ['tail uses an explicit keyboard disclosure boundary and bounded cumulative batches', pageSource.includes('const TAIL_BATCH_SIZE = 4') && pageSource.includes('current === lastPriority') && pageSource.includes('return focusBoundary(toggle,current)') && pageSource.includes("announce('Нажмите Enter или пробел") && pageSource.includes('tailRevealCount += TAIL_BATCH_SIZE') && pageSource.includes("target?.closest('.site-footer')")],
  ['tail media is viewport-gated and capped to three real planes per visible deck', pageSource.includes('const MAX_TAIL_MEDIA_PER_DECK = 3') && pageSource.includes("new IntersectionObserver") && pageSource.includes("rootMargin:'80px 0px'") && pageSource.includes('placementIndex < MAX_TAIL_MEDIA_PER_DECK') && pageSource.includes('unbindDeckMedia(deck)')],
  ['like-only sync preserves exact focus without re-appending stable rows, view transitions or deck relayout', pageSource.includes('if (!ordered.some((row,index) => row !== current[index])) return') && pageSource.includes('exactFocus.focus({ preventScroll:true })') && pageSource.includes("syncActionState(); syncFilters(focusedTitle,{ relayout:false })")],
