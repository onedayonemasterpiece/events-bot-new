...
export const EXHIBITIONS_PRIVATE_THEME_ALIASES = [
  '--ex-bg',
  '--ex-surface',
  '--ex-raised',
  '--ex-border',
...
const replaceInCssRule = (source, selector, replacements) => {
  const pattern = new RegExp(`(^|\\n)(  ${escapeRegex(selector)} \\{[\\s\\S]*?\\n  \\})`, 'u');
  const match = source.match(pattern);
  assert.ok(match, `missing CSS rule ${selector}`);
  let block = match[2];
  for (const [from, to] of replacements) block = replaceAllIfPresent(block, from, to);
  return source.replace(pattern, `${match[1]}${block}`);
};

const cssCustomPropertyNames = (source, prefix = '--ex-') => [...new Set(
  [...source.matchAll(new RegExp(`${escapeRegex(prefix)}[a-z0-9-]+`, 'gu'))].map((match) => match[0]),
)].sort();
...
const transformClubDetail = (input) => {
  ...
  ]) source = replaceOnceIfPresent(source, from, to, `club-detail ${from}`);
  return source;
};
...
const transformFestivals = (input) => {
  ...
  if (!source.includes('<Icon ')) source = replaceOnceIfPresent(source, "import Icon from '../../components/Icon.astro';\n", '', 'unused festival Icon import');
  source = replaceOnceIfPresent(source, '<span class="festival-guide__icon" aria-hidden="true">↗</span>', '<span class="festival-guide__icon" aria-hidden="true"><SemanticIcon name="link" role="control" /></span>', 'festival link guide glyph');
  source = replaceOnceIfPresent(source, '<span class="festival-guide__icon" aria-hidden="true">＋</span>', '<span class="festival-guide__icon" aria-hidden="true"><SemanticIcon name="calendar" role="control" /></span>', 'festival calendar guide glyph');
  source = replaceOnceIfPresent(source, "rel={item.isExternal ? 'noreferrer' : undefined}", "rel={item.isExternal ? 'noopener noreferrer' : undefined}", 'festival safe external relation');
  ...
  source = ensureFoundationConsumer(source, "    data-ds-state={resolvedItems.length > 0 ? 'populated' : 'empty'}", 'festival-route');

  source = replaceInCssRule(source, '.festival-guide__icon--heart', [
    ['background: rgba(165, 72, 33, 0.1);', 'background: var(--ke-color-festival-guide-like-surface);'],
  ]);
  source = replaceInCssRule(source, '.festival-month__categories li', [
  ...
  source = replaceInCssRule(source, '.festival-month__categories i', [
    ['width: 21px;', 'width: var(--ke-festival-category-asset-size);'],
    ['height: 21px;', 'height: var(--ke-festival-category-asset-size);'],
  ]);
  source = replaceInCssRule(source, '.festival-card__like', [
  ...
  for (const [from, to] of [
    ...
    ['radial-gradient(circle at 82% 22%, rgba(15, 118, 110, 0.16), transparent 19rem)', 'radial-gradient(circle at 82% 22%, var(--ke-color-festival-hero-glow), transparent 19rem)'],
    ['linear-gradient(135deg, #fffdf8, #f0e3d2)', 'linear-gradient(135deg, var(--ke-color-festival-hero-surface-start), var(--ke-color-festival-hero-surface-end))'],
  ...
  source = replaceOnceIfPresent(source, '  .festival-guide__icon :global(svg) { width: 0.95rem; height: 0.95rem; fill: currentColor; }\n', '', 'festival guide local icon square');
  source = replaceOnceIfPresent(source, '  .festival-card__like :global(svg) { width: 1.12rem; height: 1.12rem; fill: currentColor; }\n', '', 'festival favorite local icon square');
  return source;
};
...
const transformExhibitions = (input) => {
  ...
  source = replaceOnceIfPresent(source, '<span aria-hidden="true">←</span>', '<SemanticIcon name="arrow-left" role="control" />', 'exhibitions previous gallery glyph');
  source = replaceOnceIfPresent(source, '<span aria-hidden="true">→</span>', '<SemanticIcon name="arrow-right" role="control" />', 'exhibitions next gallery glyph');
  ...
  source = ensureFoundationConsumer(source, "  data-ds-state={newItems.length + priorityItems.length + tailItems.length > 0 ? 'personal all-topics populated' : 'personal all-topics empty'}", 'exhibitions-personal-surface');
  ...
  for (const [from, to] of Object.entries(aliases)) source = replaceAllIfPresent(source, `var(${from})`, `var(${to})`);
  source = source.replace(/\n  :root \{\n[\s\S]*?--ex-motion-base:420ms;\n  \}\n(?=  body:has\(\.ex-page\))/u, '\n');

  source = replaceInCssRule(source, '.ex-page kbd', [
    ['border:1px solid #50555a;', 'border:1px solid var(--ke-color-exhibitions-kbd-line);'],
    ['border-bottom-color:#777d82;', 'border-bottom-color:var(--ke-color-exhibitions-kbd-line-strong);'],
    ['background:#202326;', 'background:var(--ke-color-exhibitions-kbd-surface);'],
    ['color:#e9ecee;', 'color:var(--ke-color-exhibitions-kbd-text);'],
    ['box-shadow:0 1px 0 #050607;', 'box-shadow:0 1px 0 var(--ke-color-exhibitions-kbd-shadow);'],
  ]);
  source = replaceInCssRule(source, '.ex-mode', [
    ['background:#08090a;', 'background:var(--ke-color-exhibitions-mode-surface);'],
  ]);
  source = replaceInCssRule(source, '.ex-mode button[aria-checked="true"]', [
    ['box-shadow:inset 0 0 0 1px #3a3f43;', 'box-shadow:inset 0 0 0 1px var(--ke-color-exhibitions-mode-selected-line);'],
  ]);
  source = replaceInCssRule(source, '.ex-controls', [
    ['background:rgba(13,15,16,.96);', 'background:var(--ke-color-exhibitions-sticky-surface);'],
  ]);
  source = replaceInCssRule(source, '.ex-page', [
    ['rgba(84,172,247,.14)', 'var(--ke-color-exhibitions-blue-glow)'],
    ['rgba(241,130,36,.07)', 'var(--ke-color-exhibitions-orange-glow)'],
    ['font-size:16px;', 'font-size:var(--ke-exhibitions-page-font-size);'],
    ['line-height:1.45;', 'line-height:var(--ke-exhibitions-page-line);'],
  ]);
  source = replaceInCssRule(source, '.ex-list::before', [
    ['#626a70', 'var(--ke-color-exhibitions-timeline-mid)'],
    ['#4d555b', 'var(--ke-color-exhibitions-timeline-end)'],
  ]);
  source = replaceInCssRule(source, '.ex-row::after', [
    ['rgba(20,23,25,.98)', 'var(--ke-color-exhibitions-row-start)'],
    ['rgba(13,15,17,.98)', 'var(--ke-color-exhibitions-row-end)'],
    ['box-shadow:inset 0 1px 0 rgba(255,255,255,.055),0 10px 32px rgba(0,0,0,.2),0 0 0 1px rgba(255,255,255,.018);', 'box-shadow:var(--ke-elevation-exhibitions-row);'],
  ]);
  source = replaceInCssRule(source, '.ex-row:hover::after,.ex-row:focus-within::after', [
    ['box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 18px 52px rgba(0,0,0,.38),0 0 34px rgba(84,172,247,.1),0 0 0 1px rgba(84,172,247,.18);', 'box-shadow:var(--ke-elevation-exhibitions-row-hover);'],
  ]);
  source = replaceInCssRule(source, '.ex-row.is-keyboard-active::after', [
    ['box-shadow:inset 0 1px 0 rgba(255,255,255,.16),0 18px 52px rgba(0,0,0,.4),0 0 34px rgba(84,172,247,.16),0 0 0 2px rgba(84,172,247,.72);', 'box-shadow:var(--ke-elevation-exhibitions-row-keyboard);'],
  ]);
  source = replaceInCssRule(source, '.ex-discussed svg', [
    ['width:14px;', 'width:var(--ke-exhibitions-signal-icon-size);'],
    ['height:14px;', 'height:var(--ke-exhibitions-signal-icon-size);'],
  ]);
  source = replaceInCssRule(source, '.ex-signal svg', [
    ['width:14px;', 'width:var(--ke-exhibitions-signal-icon-size);'],
    ['height:14px;', 'height:var(--ke-exhibitions-signal-icon-size);'],
  ]);
  source = replaceInCssRule(source, '.ex-action svg', [
    ['width:21px;', 'width:var(--ke-exhibitions-action-icon-size);'],
    ['height:21px;', 'height:var(--ke-exhibitions-action-icon-size);'],
  ]);
  source = replaceInCssRule(source, '.ex-action--like svg', [
    ['width:19px;', 'width:var(--ke-exhibitions-action-icon-size);'],
    ['height:19px;', 'height:var(--ke-exhibitions-action-icon-size);'],
  ]);
  source = replaceInCssRule(source, '.ex-action--reject svg', [
    ['width:18px;', 'width:var(--ke-exhibitions-action-icon-size);'],
    ['height:18px;', 'height:var(--ke-exhibitions-action-icon-size);'],
  ]);
  source = replaceInCssRule(source, '.ex-gallery__stage button', [
    ['font-size:2rem;', 'font-size:var(--ke-exhibitions-gallery-arrow-icon-size);'],
  ]);
  ...
  for (const [from, to] of [
    ...
    ['background:linear-gradient(135deg,#15181a,#111315);', 'background:linear-gradient(135deg,var(--ke-color-exhibitions-keyboard-help-start),var(--ke-color-exhibitions-keyboard-help-end));'],
...
const transformFocusCollection = (input) => {
  ...
  if (!source.includes('root.dataset.dsState = `found-${found}-of-${eligible}`;')) {
    source = replaceOnceIfPresent(source, `      const foundOutput = root.querySelector<HTMLElement>('[data-collection-found]');`, `      root.dataset.dsState = \`found-\${found}-of-\${eligible}\`;\n      const foundOutput = root.querySelector<HTMLElement>('[data-collection-found]');`, 'focus collection runtime state');
  }
  return source;
};
...
export function assertA0ConsumerPostconditions(path, source) {
  switch (path) {
    case 'site/src/components/InterestClubCard.astro':
      for (const raw of ['rgba(255,255,255,.035)', 'rgba(255,255,255,.025)', 'rgba(255,255,255,.3)', 'rgba(255,255,255,.24)']) assert.ok(!source.includes(raw), `${path} retains ${raw}`);
      for (const token of ['fallback-orbit-ring-inner', 'fallback-orbit-ring-outer', 'fallback-orbit-line', 'fact-divider']) assert.ok(source.includes(`--ke-color-club-card-${token}`), `${path} misses ${token}`);
      break;
    case 'site/src/pages/kluby-po-interesam/[slug]/index.astro':
      ...
      for (const token of ['--ke-color-club-detail-hero-start', '--ke-color-club-detail-note-surface', '--ke-club-detail-action-min']) assert.ok(source.includes(`var(${token})`), `${path} misses ${token}`);
    ...
    case 'site/src/pages/festivali/index.astro':
      ...
      for (const token of ['--ke-festival-page-max', '--ke-festival-hero-radius', '--ke-festival-guide-icon-size', '--ke-festival-like-target-min', '--ke-color-festival-guide-like-surface', '--ke-color-festival-category-surface']) assert.ok(source.includes(`var(${token})`), `${path} misses ${token}`);
      for (const forbidden of ['<Icon name="heart" />', '.festival-guide__icon :global(svg) { width: 0.95rem; height: 0.95rem;', 'width: clamp(2rem, 2.35vw, 2.2rem);', 'height: clamp(2rem, 2.35vw, 2.2rem);']) assert.ok(!source.includes(forbidden), `${path} retains ${forbidden}`);
    ...
    case 'site/src/components/ExhibitionsPersonalSurface.astro': {
      ...
      assert.ok(!source.includes('<span aria-hidden="true">←</span>'), `${path} retains previous text arrow`);
      assert.ok(!source.includes('<span aria-hidden="true">→</span>'), `${path} retains next text arrow`);
      ...
      for (const rawBlock of [
        ...
        '.ex-action svg { width:21px; height:21px;',
        '.ex-action--like svg { width:19px; height:19px;',
        '.ex-action--reject svg { width:18px; height:18px;',
      ]) assert.ok(!source.includes(rawBlock), `${path} retains raw icon block ${rawBlock}`);
      break;
    }
...
export async function runA0ConsumerClosure({ checkOnly = false } = {}) {
  ...
  const receipt = {
    ...
    exhibitions_private_theme_removed: true,
    exhibitions_runtime_layout_variables: EXHIBITIONS_RUNTIME_VARIABLES,
