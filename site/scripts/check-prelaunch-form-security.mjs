...
const directOrigin = String(process.env.PUBLIC_PERSONALIZATION_SUPABASE_URL || '').replace(/\/+$/u, '');
const relayOrigin = String(process.env.PUBLIC_PERSONALIZATION_SUPABASE_RELAY_URL || '').replace(/\/+$/u, '');
assert(directOrigin, 'PUBLIC_PERSONALIZATION_SUPABASE_URL is required for the artifact-bound form gate');
...
async function openScenario(name, behavior) {
  ...
  try {
    ...
    await page.route(`${directOrigin}/**`, handle('direct'));
    if (relayOrigin) await page.route(`${relayOrigin}/**`, handle('relay'));
    ...
    if (behavior.registrationHint) {
      ...
      for (const value of behavior.invalidInputs) {
        ...
        assert(calls.length === before, `${name}: invalid input reached transport`);
      ...
      if (behavior.consent === false) {
        ...
        if (behavior.assertDuplicateSubmitLocked) {
          await page.evaluate(() => {
            const node = document.querySelector('[data-prelaunch-form]');
        ...
        if (behavior.expectSuccess) {
          const expectedState = behavior.expectedSuccessState || 'success';
          await page.waitForFunction((state) => (
            document.querySelector('[data-prelaunch-form]')?.getAttribute('data-experience-state')
          ) === state, expectedState);
          ...
          const completeText = await complete.textContent() || '';
          assert(
            completeText.includes(expectedState === 'registered' ? 'Вы уже записаны' : 'Готово, вы записаны'),
            `${name}: completion headline does not match the backend receipt`,
          );
        } else {
          await page.waitForFunction(() => (
    ...
    const result = {
      ...
      ok: true,
      calls: publicCalls,
      storedIdentityCount: stored.size,
      directRegistrationAttempts,
      relayRegistrationAttempts,
    ...
    evidence.push({
      ...
      ok: false,
      error: message,
      calls: calls.map(({ route, path, method }) => ({ route, path, method })),
      storedIdentityCount: stored.size,
    });
  } finally {
    await context.close();
...
try {
  ...
  await openScenario('submitting-single-request', {
    ...
    assertDuplicateSubmitLocked: true,
    expectSuccess: true,
  ...
  await openScenario('already-registered-success', {
    responseStatus: 'already_registered',
    expectedSuccessState: 'registered',
    expectSuccess: true,
    email: 'repeat@example.com',
    assertions: async ({ stored, directRegistrationAttempts }) => {
      assert(stored.size === 1, 'already-registered-success: expected one normalized identity');
      assert(directRegistrationAttempts === 1, 'already-registered-success: unexpected duplicate request');
    },
  });
...
const summary = {
  ...
  coveredStates: [
    ...
    ...(relayOrigin ? ['direct-to-relay-fallback', 'idempotent-replay-after-lost-response'] : []),
  ],
  evidence,
  failures,
};
