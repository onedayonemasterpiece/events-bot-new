...
if (!controlHtml.includes('data-share-experiment') || !controlHtml.includes('data-share-experiment-mode="caption_url"') || !controlHtml.includes('Поделиться эксперимент 2') || !controlHtml.includes('navigator.canShare') || !controlHtml.includes('shareExperimentPayload(button, [file])') || !controlHtml.includes('createGeneratedShareImage') || !controlHtml.includes('1080') || !controlHtml.includes('1350')) throw new Error('Control event page misses experimental Web Share file/text/url and caption-url buttons with generated share-image fallback');
...
if (!controlVisibleHtml.includes('event-source-gate--section') || !/<div class="event-info-block"[\s\S]*?<\/dl>\s*<\/div>\s*<\/div>\s*<p class="event-source-gate event-source-gate--section"/u.test(controlVisibleHtml)) throw new Error('Source/mentions auth gate must belong to the parent details section, not to the compact facts block');
