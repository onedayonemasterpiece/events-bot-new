...
const searchCorpusReceiptPath = join(siteDir, 'src/data/event-search-corpus-receipt.json');
...
const searchCorpusReceipt = existsSync(searchCorpusReceiptPath)
  ? JSON.parse(readFileSync(searchCorpusReceiptPath, 'utf8'))
  : null;
const searchCatalogRevision = String(eventsData?.build?.catalog_revision || '');
...
if (searchCorpusReceipt) {
  ...
  if (searchCorpusReceipt?.coverage?.status !== 'complete') throw new Error('Search corpus coverage is not complete');
}
...
const buildMetadata = {
  ...
  search_revisions: searchCorpusReceipt ? {
    catalog_revision: searchCatalogRevision,
    corpus_revision: searchCorpusRevision,
    search_document_revision: searchDocumentRevision,
    coverage_status: searchCorpusReceipt.coverage.status,
  } : null,
};
...
const manifest = {
  ...
  search_revisions: buildMetadata.search_revisions,
  versions: {
