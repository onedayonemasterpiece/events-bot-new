...
def event_table_columns(con: sqlite3.Connection) -> set[str]:
    try:
        rows = con.execute("pragma table_info(event)").fetchall()
...
def public_projection_gate_reason(row: sqlite3.Row) -> str | None:
    ...
    The gate is intentionally independent of Smart Update internals: every row
    selected for public JSON, including manual include/control ids, must pass
    identity, lifecycle, date and narrow leakage checks.  Missing columns are
    treated as old-schema rows and skipped only when a required public field
    such as ``date`` is invalid.
    """
...
def event_active_where(current_date: str, columns: set[str] | None = None) -> str:
    columns = set(columns or ())
    current_date = normalize_date(current_date) or CURRENT_DATE_DEFAULT
    parts = ["date glob '20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]'"]
    if "identity_status" in columns:
        parts.append("lower(coalesce(nullif(trim(identity_status),''),'')) = 'canonical'")
    if "merged_into_event_id" in columns:
        parts.append("(merged_into_event_id is null or trim(cast(merged_into_event_id as text)) in ('','0'))")
    if "lifecycle_status" in columns:
        parts.append(
            "coalesce(lifecycle_status,'active') not in "
            "('cancelled','deleted','duplicate','review','needs_review','manual_review','eventness_review','quality_review','quarantine','quarantined','rejected')"
        )
    for column in PUBLIC_STATUS_COLUMNS:
        if column == "lifecycle_status" or column not in columns:
            continue
        parts.append(
            f"(coalesce({column},'') = '' or lower({column}) not in "
            "('review','needs_review','manual_review','eventness_review','quality_review','quarantine','quarantined','rejected'))"
        )
    if "end_date" in columns:
        parts.append(f"(date >= '{current_date}' or (end_date glob '20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and end_date >= '{current_date}'))")
    else:
...
def fetch_rows(con: sqlite3.Connection, limit: int, current_date: str, include_ids: list[int]) -> list[sqlite3.Row]:
    columns = event_table_columns(con)
    ...
    active = event_active_where(current_date, columns)
    time_order = "coalesce(nullif(time,''),'23:59') asc, id asc" if "time" in columns else "id asc"
    ...
    add_query(
        f"select * from event where {active} and date = ? order by {time_order} limit ?",
    ...
    if len(ordered_rows) < limit:
        add_query(
            ...
            where {active}
              and date > ?
              {"and (end_date is null or trim(end_date) = '' or end_date = date)" if "end_date" in columns else ""}
            order by date asc, {time_order}
            limit ?
            """,
            (current_date, max(limit * 2, 80)),
        )
    if len(ordered_rows) < limit and "end_date" in columns:
        add_query(
            f"""
            select * from event
            where {active}
              ...
              and end_date >= ?
            order by end_date asc, date desc, {time_order}
            limit ?
            """,
    ...
    if len(ordered_rows) < limit:
        add_query(
            f"select * from event where {active} order by date asc, {time_order} limit ?",
