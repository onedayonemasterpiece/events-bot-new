...
function initializePrelaunchForm(): void {
    ...
    if (completeState) {
      ...
      if (completeBody) {
        completeBody.textContent = options.body
          || '20 сентября пришлём ссылку на сервис. Дальше будем писать только о важных обновлениях и полезных подборках; отписаться можно из любого письма.';
      }
  ...
  if (readRegistrationHint()) {
    setState('registered', {
      ...
      body: '20 сентября пришлём ссылку на сервис. Для подписавшихся будет приятный сюрприз, а дальше — только важные обновления и полезные подборки.',
    });
  } else {
    setState('idle');
  }
    ...
    try {
      ...
      setState(alreadyRegistered ? 'registered' : 'success', alreadyRegistered
        ...
        : {
            ...
            body: '20 сентября пришлём ссылку на сервис. Для подписавшихся мы приготовили приятный сюрприз; затем будем писать только о важных обновлениях и полезных подборках.',
            focus: true,
          });
    } catch (error) {
      setState('error');
