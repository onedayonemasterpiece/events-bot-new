...
export type Patch = { goal?: string; localityIds?: string[]; excludedFormats?: string[];
  freeOnly?: boolean; maxPrice?: number | null; dateFrom?: string | null; dateTo?: string | null;
  timeOfDay?: string | null; audience?: string[]; timezone?: 'Europe/Kaliningrad' };
...
export function applyIntentPatch(base: Intent, patch: Patch): Intent {
  fields(patch, ['goal', 'localityIds', 'excludedFormats', 'freeOnly', 'maxPrice', 'dateFrom', 'dateTo', 'timeOfDay', 'audience', 'timezone']);
  ...
  for (const key of ['dateFrom', 'dateTo'] as const) {
    ...
    if (date !== null && (typeof date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(date) ||
        !Number.isFinite(Date.parse(date)) || new Date(date).toISOString().slice(0, 10) !== date)) fail('invalid_date');
    next[key] = date;
  }
  if (next.dateFrom && next.dateTo && next.dateFrom > next.dateTo) fail('conflicting_dates');
  if (Object.hasOwn(patch, 'timeOfDay')) {
    if (patch.timeOfDay !== null && !['morning', 'day', 'evening', 'night'].includes(patch.timeOfDay as string)) fail('invalid_time_of_day');
    next.timeOfDay = patch.timeOfDay;
  }
  if (Object.hasOwn(patch, 'audience')) next.audience = ids(patch.audience, 16);
  if (Object.hasOwn(patch, 'timezone')) {
    if (patch.timezone !== 'Europe/Kaliningrad') fail('invalid_timezone');
