import {handoffScope,validHandoffId} from './searchHandoff.ts';
import {receiveHomeHandoff} from './homeHandoffReceiver.ts';
...
export async function mountConversationalSearch(root:HTMLElement,presentation?:ReturnType<typeof mountComposerPresentation>):Promise<void>{
  ...
  const scope=await handoffScope(location.origin,import.meta.env.BASE_URL||'/');
  const handoffId=new URL(location.href).searchParams.get('voice_handoff');
  try{store=(await VoiceStore.open()).scoped(scope.storageScope);}catch(error){
    ...
    for(const row of rows){
      ...
      panel.append(button('Прослушать сохранённое аудио',async()=>{
        try{const parts=await store.parts(own,row.id);if(owner!==own)return;
          const {assemble,sha256}=await import('./domain/assistant-media.ts');
  ...
  auth.subscribe(snapshot=>{
    ...
    void active.initialize().then(async()=>{
      ...
      await loadRecordings();
      if(handoffId&&validHandoffId(handoffId)){await receiveHomeHandoff({owner,id:handoffId,scope,store,api,controller:active,isCurrent:()=>epoch===generation&&document.contains(root),status:copy=>{processing.textContent=copy;}});return;}
      if(state.draft)processing.textContent='Найден сохранённый незавершённый запрос. Обработка возобновится только по кнопке.';
