...
function assertRegistry(): void {
  ...
  for (const scenario of Object.values(registry.scenarios)) {
    if (scenario.reference_timestamp !== frozenCorpus.authority.reference_clock.reference_iso
      || scenario.reference_date !== frozenCorpus.authority.reference_clock.current_date) {
      throw new Error('Design fixture scenario drifted from the frozen source-bound clock');
    }
  }
}
