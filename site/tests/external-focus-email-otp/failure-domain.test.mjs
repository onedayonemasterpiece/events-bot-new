...
test('failure domains preserve Safari startup, simulator control, viewport and keyboard distinctions', () => {
  ...
  assert.equal(focusOtpFailureDomain(new Error('safari_native_find_elements_non_array')), 'BLOCKED_INFRASTRUCTURE');
  assert.equal(focusOtpFailureDomain(new Error('safari_native_alert_buttons_non_array')), 'BLOCKED_INFRASTRUCTURE');
