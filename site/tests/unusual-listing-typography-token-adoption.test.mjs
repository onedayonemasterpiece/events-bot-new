...
test('Unusual listing section headings consume the canonical editorial heading role', async () => {
  const source = await readFile(path.join(siteRoot, 'src/components/UnusualListingSurface.astro'), 'utf8');

  assert.match(source, /\.unusual-page__feed-head h2,\.unusual-page__empty h2\s*\{[^}]*font:var\(--ke-type-editorial-section\)/u);
