...
test('build identity remains full-SHA and N0 publication policy is Kaggle-only', async () => {
  ...
  assert.ok(realGate, 'canonical full-real Kaggle gate is missing');
  assert.equal(realGate.owner, 'R0');
  assert.equal(typeof realGate.status, 'string');
  assert.ok(realGate.status.length > 0, 'delivery status must be explicit without being frozen into this identity test');
