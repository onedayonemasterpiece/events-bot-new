...
test('Popular binds the accepted density choice to exactly two visible canonical representations', async () => {
  ...
  const largeRoot = large.match(/<div[\s\S]*?data-popular-representation="mobile-large"[\s\S]*?>/u)?.[0] || '';
  assert.ok(largeRoot, 'mobile-large representation root is present');
  assert.doesNotMatch(largeRoot, /\bhidden\b|\binert\b/u);
