...
for(const viewport of viewports){
 ...
 for(const route of ['/date-2026-07-23/','/segodnya/','/zavtra/']){
  ...
  await toggle.click();await page.waitForTimeout(200);assert.equal(await toggle.getAttribute('aria-expanded'),'true');assert.notEqual(await toggle.evaluate(n=>getComputedStyle(n,'::before').transform),closedArrow.transform);await page.screenshot({path:`${out}/${viewport.width}-${route.split('/').filter(Boolean).join('-')}-picker.png`});const choice=page.locator('[data-island-city-panel]:popover-open [data-mobile-v23-city]:not([data-mobile-v23-city="all"]):visible').first();
