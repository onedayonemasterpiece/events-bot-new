...
for(const viewport of viewports){
 ...
 for(const route of ['/date-2026-07-23/','/segodnya/','/zavtra/']){
  ...
  assert.ok(docked.headingBottom<0);assert.equal(docked.docked,'true');assert.ok(docked.cityX>=docked.brandRight+8-1);assert.ok(Math.abs(docked.cityY-20)<1,JSON.stringify({viewport,route,...docked}));assert.equal(docked.titleSkinVisible,false);assert.equal(docked.sectionVisible,false);assert.equal(docked.visibleCount,0);assert.equal(docked.caption,'Все города');assert.equal(docked.cityHeight,44);assert.ok(docked.cityWidth<=130);assert.ok(Math.abs(viewport.width-docked.cityX-docked.cityWidth-12)<1,JSON.stringify(docked));
  const closedArrow=await page.locator('[data-mobile-listing-rails] [data-island-city-toggle]:visible').evaluate(n=>{const s=getComputedStyle(n,"::before");return {content:s.content,width:s.width,border:s.borderRightWidth,transform:s.transform};});assert.equal(closedArrow.content,'""');assert.equal(closedArrow.width,'6px');assert.equal(closedArrow.border,'2px');
  ...
  const toggleBox=await toggle.boundingBox();await page.touchscreen.tap(toggleBox.x+toggleBox.width/2,toggleBox.y+toggleBox.height/2);await page.waitForTimeout(200);assert.equal(await toggle.getAttribute('aria-expanded'),'true');assert.notEqual(await toggle.evaluate(n=>getComputedStyle(n,'::before').transform),closedArrow.transform);await page.screenshot({path:`${out}/${viewport.width}-${route.split('/').filter(Boolean).join('-')}-picker.png`});const choice=page.locator('[data-island-city-panel]:popover-open [data-mobile-v23-city]:not([data-mobile-v23-city="all"]):visible').first();
  ...
  if(stillDocked){const selectedRect=await page.locator('[data-mobile-listing-rails] [data-listing-controls]').boundingBox();assert.ok(Math.abs(viewport.width-selectedRect.x-selectedRect.width-12)<1);const selectedCaption=await toggle.textContent();assert.ok(selectedCaption.includes(city),JSON.stringify({selectedCaption,city}));await toggle.click();assert.ok(await page.locator('[data-island-city-panel]:popover-open [data-mobile-v23-city="all"]:visible').count());await page.keyboard.press('Escape');}
...
await page.goto(base+'/vyhodnye/',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.body.dataset.fiMotion==='ready');assert.equal(await page.locator('body').getAttribute('data-fi-single-day'),null);await page.evaluate(()=>scrollTo(0,900));await page.waitForTimeout(900);assert.ok(await page.locator('.fi-weekend-days:visible').count()>0);await page.screenshot({path:out+'/384-weekend-context.png'});results.push({weekendContextRetained:true});
