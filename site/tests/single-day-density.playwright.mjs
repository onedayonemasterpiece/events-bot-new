...
for(const viewport of viewports){
 ...
 for(const route of ['/date-2026-07-23/','/segodnya/','/zavtra/']){
  ...
  const docked=await page.evaluate(()=>{const root=document.querySelector('[data-mobile-listing-rails]'),h=root.querySelector('h1').getBoundingClientRect(),city=root.querySelector('[data-listing-controls]'),r=city.getBoundingClientRect(),brand=document.querySelector('[data-mobile-discovery-menu]>summary').getBoundingClientRect();return {headingBottom:h.bottom,cityX:r.x,cityY:r.y,cityWidth:r.width,brandRight:brand.right,visibleCount:Number(city.dataset.fiVisibleCount),docked:city.dataset.fiDocked,titleSkinVisible:!!root.querySelector('.fi-mobile-title-skin')?.getClientRects().length,sectionVisible:!!root.querySelector('[data-floating-section-context]')?.getClientRects().length};});
  assert.ok(docked.headingBottom<0);assert.equal(docked.docked,'true');assert.ok(docked.cityX>=docked.brandRight+8-1);assert.ok(Math.abs(docked.cityY-14)<1,JSON.stringify({viewport,route,...docked}));assert.equal(docked.titleSkinVisible,false);assert.equal(docked.sectionVisible,false);assert.ok(docked.visibleCount>=1);
  ...
  const city=await choice.getAttribute('data-mobile-v23-city');await choice.click();await page.waitForTimeout(200);
