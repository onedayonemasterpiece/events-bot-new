...
test('future health permits only manual, twice-daily and Search runtime deploy triggers', () => {
  for (const trigger of ['workflow_dispatch', 'twice_daily', 'search_runtime_deploy']) {
    const plan = planProductionHealthRun({ plane: 'production_health', trigger });
    assert.equal(plan.eligible, true, trigger);
    assert.equal(plan.dry_run, true, trigger);
    assert.equal(plan.live_calls.search_post, 0, trigger);
    assert.equal(plan.future_health_contract.query.search_post_count, 1, trigger);
    assert.equal(plan.future_health_contract.forbidden_activity.llm_calls, 0, trigger);
    assert.equal(plan.future_health_contract.forbidden_activity.pagination_requests, 0, trigger);
  }
