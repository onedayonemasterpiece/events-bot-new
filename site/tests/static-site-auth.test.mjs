...
test('focus identity supports real email OTP and Yandex linking through the shared controller', () => {
  ...
  assert.match(auth, /attemptRedirect\.searchParams\.set\('focus_auth_attempt', attemptId\)/u);
  assert.match(auth, /emailRedirectTo:\s*attemptRedirect\.href/u);
  assert.match(auth, /latestOutcome\('auth\.otp', transportStartedAt\)/u);
  assert.match(auth, /latestOutcome\('auth\.verify', transportStartedAt\)/u);
  assert.match(auth, /focus_auth_get_delivery_receipt_v1/u);
  assert.match(auth, /focus_auth_record_method_attempt_v1/u);
  assert.match(auth, /focus_auth_record_verification_v1/u);
  assert.match(auth, /p_verified: verified/u);
  assert.match(auth, /Authorization: `Bearer \$\{accessToken \|\| this\.config\.publishableKey\}`/u);
  assert.match(auth, /p_auth_method: 'custom:yandex'/u);
