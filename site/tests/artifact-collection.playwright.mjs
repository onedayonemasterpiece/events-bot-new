...
try {
  ...
  await page.waitForURL(/\/artefakty\/kollektsii\/znaki-yantarnogo-kraya\/#amber-cosmonaut$/u);
  ...
  assert.equal(await page.locator('[data-artifact-state="locked"]').count(), 7);
  assert.equal(await page.getByRole('link', { name: 'Сообщить об артефакте' }).count(), 1);
  ...
  await legacyPage.goto(`${baseUrl}/artefakty/kollektsii/znaki-yantarnogo-kraya/`, { waitUntil: 'domcontentloaded' });
