...
- Scroll Android Chrome and iOS Safari documents through shared `performNativeTouchSwipe` in `NATIVE_APP`, resolving viewport dimensions only after that context switch and restoring the original web context before reading `scrollY`. Web-context/CSS dimensions are not native touch coordinates. Appium's `mobile: scrollGesture` (UiAutomator2) and `mobile: scroll` (XCUITest) are native control/table shortcuts and are not accepted as proof for browser page content.
