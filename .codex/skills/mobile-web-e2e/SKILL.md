...
- Dismiss an observed software keyboard through the shared native-context helper before measuring the scrolling baseline. XCUITest Safari may return its exact `Did not know how to dismiss the keyboard` response; only that exact iOS response may trigger the documented user-equivalent downward native swipe, followed by a mandatory `isKeyboardShown() == false` observation. Never treat the error itself as success, swallow other driver errors, or allow a document swipe to run while the IME is still visible.
