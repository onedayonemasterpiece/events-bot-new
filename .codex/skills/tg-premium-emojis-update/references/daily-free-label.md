...
- Daily audience rows use `@kenigevents adaptive pack` (`https://t.me/addemoji/kenigeventsadaptivepack`): `❤️` document id `5339188899241570417`; `🔂` document id `5336998942661975661`.
...
- For Tretyakov venue markers, use `🖼🖼` only where the venue is visible/structured: insert before `Третьяков...` in venue/date lines and render future added-section Tretyakov rows with `🖼🖼` instead of `🚩` from structured location data. Do not infer this marker from titles/descriptions such as `Александр Дейнека`; old title-level `👉 🖼🖼 ...` markers are cleaned back to ordinary `🖼️`, and title-level `👉 🖼️ ...` must not retain a Tretyakov custom-emoji entity.
...
- Daily audience rows (`❤️ N  🔂 N`) must retain/re-attach custom emoji entities from `kenigeventsadaptivepack` after any Telethon edit; do not leave these as ordinary Unicode fallback.
...
- `👉`, `🎭`, `🤘`, date/custom `🎟`, `🖼🖼`, `💰`, `📗`, `🏰`, and daily audience `❤️/🔂` custom entities use the ids above; title-level ordinary `🖼️` has no Tretyakov custom entity; and a second dry-run reports `replacements=0`.
