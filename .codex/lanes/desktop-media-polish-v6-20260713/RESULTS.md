...
| R04 | Done | Split CTA is sticky at `top:85px`, below the `73px` header, remains there through the long text, intentionally masks content passing behind its opaque surface and the `12px` header gap, and exits at the related-section containing-block boundary. |
...
| R06 | Done | Public related-card audit shows visual media at square-to-`4:3` with `cover`, document/OCR media with `contain`, aligned card bottoms, and no lab attributes below `1024px`; a deliberately delayed visual image stays transparent over the neutral placeholder. |
...
- The final v4 Astro run generated and packaged all `442` pages normally; retained standalone `check-preview-final-v4.log` passed against that exact published tree.
...
- The post-review public polish gate also reports `failures: []`: Split CTA top `85px`, opaque `13.5938px` paper pseudo-mask plus `-12px` paper shadow, and a deliberately delayed remote visual image remaining `naturalWidth=0` with transparent image background over the neutral gradient placeholder.
