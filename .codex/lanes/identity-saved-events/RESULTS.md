...
| ID | Status | Evidence |
|---|---|---|
| ID-1 saved-occurrence schema | **Done (code), Partial (production)** | Private `site_identity`/`saved_events` schemas model account/device identity, durable occurrence saves, separate signals, consent evidence, reminder subscriptions/deliveries and bounded audit/retention state. Browser/device occurrence times stay untrusted until canonical Fly lifecycle validation. Production migration is not applied. |
...
| ID-5 explicit D-1 consent/masked verified email | **Done (code), Partial (production)** | An idempotent consent request requires the synchronized verified `email_control` identity, matching active transactional purpose consent and a service-validated canonical occurrence start. Only a masked address snapshot and verification timestamp are stored in the reminder row; plaintext remains in `email_control`. |
...
browser-time rejection and service-only canonical occurrence validation;
