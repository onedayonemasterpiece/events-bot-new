...
The Kaggle `StaticSiteBuilder` now invokes the command automatically in this
exact order:

1. export current event data;
2. enrich/cache durations through the server-side provider gateway;
3. run the production root, secret-candidate or preview Astro build.

The same generated JSON therefore feeds both production-candidate variants and
preview builds. The kernel result records an internal duration summary
(`estimate_count`, `failure_count`, bounded candidate limit), never credentials.

Runner/build config:

- `STATIC_SITE_DURATION_ENRICHMENT=1` / `--[no-]duration-enrichment`;
- `STATIC_SITE_DURATION_MODEL=gemini-3.1-flash-lite`;
- `STATIC_SITE_DURATION_GOOGLE_KEY_ENVS=GOOGLE_API_KEY4`;
- `STATIC_SITE_DURATION_MAX_EVENTS=12` (validated 1–50);
- optional `STATIC_SITE_DURATION_REQUIRE_COMPLETE=1`.

The runner copies only named provider/limiter secrets into the existing
encrypted split Kaggle datasets. Values are never command-line arguments or
logs. A valid cache hit was exercised with every `GOOGLE_API_KEY*` variable
removed: no provider SDK import/call was needed and the cache SHA remained
unchanged.

Additional focused checks:

- `tests/test_static_site_duration_kaggle_integration.py`: **5/5 passed**
  - enrichment call ordering before all three build modes;
  - bounded/no-secret command;
  - keyless cache hit;
  - unsafe config rejection;
  - encrypted payload contains the named duration key and Supabase limiter
    credentials.
