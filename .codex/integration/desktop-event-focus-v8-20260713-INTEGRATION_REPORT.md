...
| R04 | Done locally | OCR companion visible poster `192×267.83`, exact-index fullscreen opener. |
...
| R11 | Done locally | Changes are lab-only and desktop media-query scoped; `390×844` root absent, overflow `0`. |
...
- Playwright CLI against final `dist`: two desktop viewports, nine routes, gallery interaction, motion samples, ordering and mobile-isolation smoke; no console errors.
