...
- После отключения Gemma 3 этот `T212029Z` artifact является frozen baseline pack для дальнейших Smart Update staged benchmarks. `scripts/inspect/benchmark_smart_update_g4_stages.py` должен запускаться с `--reuse-baseline-artifact artifacts/codex/lollipop_g4_benchmark_20260501T212029Z.json`, чтобы не пытаться пересоздать baseline live через `gemma-3-27b-it`. Candidate generation по-прежнему не получает baseline facts/text.
- Smart Update G4 staged single-fixture rerun `artifacts/codex/smart_update_g4_stage_benchmark_20260502T145722Z.{md,json}` now reaches accepted source/fact/final-description/derived-field gates for `RED-COSMOS` using frozen Gemma 3 baseline and `writer_model=gpt-4o->gemma-4-compact` after 4o quota failure. Remaining blocker: latency `131.30s` vs frozen baseline `36.98s` (`~3.55x`), plus epigraph parity review before expanding to the five-fixture pack.
