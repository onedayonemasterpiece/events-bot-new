...
Фиксированный период shadow mode не обязателен. Зафиксированный рабочий контракт: каждый конкретный выпуск сначала показывается на preview, и после ручного approve его можно публиковать. Без approve public side effect запрещён.

Preview доставляется в закрытый Telegram admin-чат с inline-кнопками `✅ Одобрить к публикации` / `❌ Отклонить выпуск`. Approve до слота разрешает publisher отправить выпуск в `10:00 Europe/Kaliningrad`; callback сам не публикует.
...
### MVP-5 — возможная guarded automation, пока не разрешена

Текущий продуктовый контракт требует ручного approve каждого выпуска. Full autopublish остаётся отдельной будущей гипотезой и не включается без нового решения владельца. Если такое решение появится, автопубликация допустима только для high-confidence editions:
...
Нужны ответы владельца продукта, но они не блокируют docs-first архитектуру. Уже решены: `0..1`, Telegram admin-чат с кнопками, ручной approve каждого выпуска, slot 10:00, любые публичные source photos со ссылкой, полноценный голос официальных источников и sensitive-topic policy с узким heritage exception.

1. Если approve получен позже 10:00, публиковать сразу в тот же день или переносить/пропускать выпуск?
2. Какие дополнительные кнопки нужны кроме `Одобрить` и `Отклонить`: `Пересобрать текст`, `Сменить фото`, `Выбрать другую тему`?
3. Какие источники объединяются в одну ownership group?
4. Нужен ли Telegram album/video в первом релизе или начинаем с одной Bento-карточки?
5. Как называются и кем администрируются новый Telegram channel и VK community?
