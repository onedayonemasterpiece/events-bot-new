...
> onboarding, авторизация и отправка feedback собраны для нового immutable
> candidate; cohort cap и rollout ещё не запущены.
...
- `/fokus-gruppa/priglashenie/` — fragment intake, немедленное удаление кода,
  ...
  установка постоянного приложения «Анонсы» и необязательное подтверждение
  через email/Яндекс. Экран показывает только один текущий шаг:
  установка/пропуск → подтверждение/пропуск → готово → обычная главная;
  установленное приложение стартует с `?launch=pwa`, а главная возвращает
  незавершившего вступление к шагу подтверждения, не отправляя уже вступившего
  в отдельный hub.
  Страница программы и все активные Lab-панели показывают один общий fragment
  URL и соответствующий QR: ссылку можно
...
- reusable badge с колбой и видимой подписью `Lab`, оценка каждой основной
  page family по шкале `0…10`, сообщение о проблеме с необязательным
  скриншотом и единая кнопка приглашения.

Оценка полезности сохраняет на устройстве подтверждённый сервером выбор на
24 часа для текущего аккаунта и **типа** страницы. `Сегодня`, `Завтра` и
конкретная дата образуют один тип `calendar`; event detail, home, Search,
Collections и остальные families разделены. На следующей странице того же
типа выбранное число восстанавливается, а пользователь может сразу заменить
его другим: новая оценка отправляется тем же authenticated RPC и только после
успеха заменяет локальный 24-часовой receipt. Это UI continuity, а не второй
relationship NPS и не обещание удалить предыдущую исследовательскую запись.

Это **не закрытая production-сборка**. LocalStorage и opaque path не являются
авторизацией. Экран вступления
...
готовый cohort cap/backend. Feedback от подтверждённого пользователя теперь
отправляется через узкий authenticated RPC в private schema; необязательный
скриншот хранится в private bucket с owner-only path, лимитом 5 МБ и
PNG/JPEG/WebP allowlist. Неподтверждённый участник продолжает пользоваться
афишей, но для отправки ответа получает короткое предложение подтвердиться.
Подарок не объявлен. Обычный
...
- повторное открытие массового QR больше не запирает уже участвующего человека
  на финальном экране: видны два простых действия — открыть афишу или заново
  пройти подключение на этом устройстве;
- если на устройстве уже есть аккаунт, шаг подтверждения показывает его имя и
  предлагает либо продолжить с ним, либо выйти и выбрать другой способ.
  Выход завершает только сессию аккаунта и не требует «отвязывать» identity;
- кнопка установки показывается только после реального browser install prompt.
  Если prompt недоступен (в том числе когда приложение уже установлено),
  показывается `Открыть «Анонсы»` и остаётся явное `Продолжить на сайте`.
  Сопроводительный текст честно объясняет, что это лёгкий доступ к афише, а не
  отдельная тяжёлая программа;
- экран приглашения использует компактную постоянную иконку `Анонсы` с
  фиксированными размерами, поэтому мобильный браузер не может растянуть
  исходное квадратное изображение в длинную полосу;
- неоднозначный ответ сети после запроса email больше не утверждает, что письмо
  не отправлено: форма шестизначного кода всё равно появляется и предлагает
  проверить почту, потому что письмо могло уже быть принято почтовым сервисом.
  Повторная отправка остаётся доступна после минутной защиты от дублей;
- onboarding стал последовательным: на экране виден только текущий выбор;
  основной результат каждого шага один, а отказ от установки и подтверждения
  остаётся маленьким явным действием. Внутри подтверждения по почте адрес и
  код — два отдельных состояния: поле кода не появляется до отправки письма,
  а выбор способа скрывается после перехода к конкретной форме;
- первый экран объясняет приглашение в фокус-группу, а не начинает разговор
  с авторизации: установка/открытие приложения → почта или Яндекс →
  явное подтверждение вступления. Условия розыгрыша двух билетов перечислены
  до начала действий: использование афиши, поиск артефактов, сообщения об
  ошибках, обратная связь об удобстве и честные оценки;
- после установки PWA незавершённое приглашение возобновляется автоматически;
  завершение ведёт на обычную главную, а не на `/zakrytaya-afisha/`;
- общий `EventLayout` добавляет активному участнику компактную Lab-панель внизу
  главной, календарных страниц, Popular, Search, подборок, фестивалей, клубов,
  выставок, необычного, избранного, «Для меня» и event detail;
- Lab-панель позволяет оценить текущую страницу, сообщить о проблеме и
  приложить один скриншот, а также показать общий QR или поделиться той же
  массовой ссылкой;
- onboarding обращается к новому пользователю обычными словами и не показывает
  ...
  `localStorage`, `PWA`);
- это же правило действует для коллекции, обратной связи, закрытой афиши,
  блока о призе и завершения:
...
- owner-only query `focus_test_reset=1` существует только для повторной
  приёмки на одном телефоне: он очищает focus participation и локальную
  auth-сессию, удаляет сам параметр из URL и заново проигрывает то же массовое
  приглашение. Персонализация, сохранённые события и установка PWA не
  затрагиваются; для повторной проверки системной установки приложение нужно
  удалить средствами телефона;
- вошедший пользователь видит имя на экране вступления; общий mobile account
  control и «Для меня» сохраняют явную кнопку выхода. Logout не удаляет
  участие, сохранённые события или настройки;
...
QR / invite URL
  -> предложить установку PWA
       или маленькое «Продолжить на сайте»
  -> предложить email-код / magic link ИЛИ Яндекс
       или маленькое «Пропустить»
  -> показать «Готово»
  -> открыть обычную главную
```

Подтверждение не является стеной перед продуктом. После явного продолжения
...
start-controller возвращает незавершённый вход на следующий шаг, а завершённый
открывает обычную главную. Сброс персонализации не
...
- PWA на каждом старте бесшумно восстанавливает эту же device session и сразу
  открывает обычную главную с доступными Lab-панелями, без повторного
  email/Яндекс;
...
user agent и IP страница не выводит. Публичный экран использует только
нейтральные подписи `Вход по почте`, `Данные сайта`, `Резервный канал` и не
показывает названия провайдеров. Если YDB доступен, а Supabase Auth нет,
...
служебная страница программы показывают account/logout состояние через общий auth runtime,
...
- `{{ .Token }}` и собственная ссылка из `{{ .RedirectTo }}` +
  `{{ .TokenHash }}` живут в одинаково оформленных hosted `confirmation` и
  `magic_link` шаблонах Supabase. Ссылка проверяется напрямую через
  `verifyOtp({ token_hash, type: "email" })`, поэтому работает и когда почтовое
  приложение передало её из вкладки браузера в установленное PWA без исходного
  PKCE verifier. Email-код и ссылка остаются двумя интерфейсами одной
  одноразовой выдачи, но E2E проверяет их на разных выдачах;
- при неуспешной или уже использованной ссылке экран показывает только
  восстановление через адрес: пользователь получает новое письмо, после чего
  видит отдельный экран шестизначного кода. Две формы одновременно не
  показываются;
- канонические файлы:
...
Для массового входа фокус-группы hosted project limit поднят с
`rate_limit_email_sent=2` до `200` писем в час; verification limit также
`200`. Защита от повторной отправки одному адресу остаётся
`smtp_max_frequency=60` секунд. Код оставлен длиной 6 цифр — это минимальная
длина, которую поддерживает Supabase Auth (допустимый диапазон 6–10), поэтому
основной путь в письме — одна кнопка входа, а ввод кода является запасным.
...
После финальной последовательной UI-сборки тот же реальный приёмник повторно
подтвердил две отдельные выдачи: code-only экран после отправки, resend после
cooldown, auto-submit шестой цифры, отказ повторного кода, вход по новой ссылке
в отдельном app-like контексте, очистку callback-параметров и отказ повторной
ссылки. Sanitized receipt:
`artifacts/codex/focus-r18/live-email-e2e-receipt.json`.

После мобильного дефекта передачи ссылки из почты в установленное PWA
PKCE-ссылка считается историческим evidence, но не текущим acceptance
контрактом. Новый обязательный live gate выпускает свежее письмо после
публикации token-hash шаблона, открывает ссылку в отдельном PWA-контексте,
проверяет session и отказ повторного использования. Отдельной выдачей
проверяются шесть цифр, auto-submit и resend.

### Установка и переход в приложение

На Android `appinstalled` означает принятие системного окна, а WebAPK может
доустанавливаться ещё несколько секунд. Поэтому экран не показывает
`Открыть «Анонсы»` сразу после нажатия: оставляет заблокированное
`Устанавливаем…`, ждёт `getInstalledRelatedApps()` либо ограниченный fallback,
и только затем предлагает открыть приложение. Ссылка открытия вызывается
только пользовательским нажатием через Android VIEW intent с безопасным HTTPS
fallback. Если Android сохранил браузер как обработчик ссылки, текст честно
просит запустить «Анонсы» с главного экрана; сайт не обещает принудительное
переключение, которое браузер не гарантирует. Стартовый `?launch=pwa`
возвращает незавершённое подключение на шаг подтверждения.
...
Уже отправленный surface score не превращает шкалу в заблокированное
подтверждение: в течение 24 часов интерфейс показывает текущий выбор на всех
страницах того же типа и разрешает его изменить. Недельный sampling управляет
автоматическим приглашением поставить оценку; он не запрещает человеку
исправить собственный последний выбор.
...
Целевая cohort-модель остаётся в private `focus_group` schema:
...
Уже применённый запусковой срез использует
`personalization.focus_group_feedback` и
`public.submit_focus_group_feedback_v1`: raw rows недоступны браузерным ролям,
RPC принимает только authenticated caller и сам берёт `auth.uid()`. Скриншоты
лежат в private bucket `focus-feedback` по пути первого уровня `auth.uid()`;
RLS разрешает владельцу только upload/read/delete собственных объектов.
