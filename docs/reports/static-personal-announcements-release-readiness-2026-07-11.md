...
| **M5** | Возрастная маркировка события везде, где событие показывается | **Missing / release blocker specified** | Several import parsers/tests use `age_restriction`, but canonical `Event` has no field, static `PreviewEvent`/exporter omit it and shared Astro renderers do not provide guaranteed marking; values may survive only as prose | Nullable canonical field + provenance and LLM-first Smart Update reconciliation; production backfill/audit; one projection/formatter across homepage/listings/search/tags/related/personal/favorites/festival/transport/detail/share and applicable ICS/structured data; full renderer Playwright/check-preview coverage; no invented `0+` or unresolved reliable-source loss |
...
| Service sharing F18 | Footer-only implementation preview готов в PR #44 (`421d6bca`): mobile share, explicit desktop image/text copy, component/controller, metric manifest, deterministic WebP+PNG renderer, lab and browser checks | Код ещё не в `main`; шапка/mobile menu не тронуты; точный Pharmastaff source/SHA и Android/iOS + Windows/macOS native matrices отсутствуют; production scheduler/publisher выключен | Review/merge PR; реализовать вторую обязательную placement после согласования шапки; привязать Pharmastaff reference; выполнить native matrices; owner утверждает desktop mode/copy/visual; включить и проверить production promotion |
...
16. **Service sharing — частично решено:** F18 обязательна; mobile menu/footer share one centrally prerendered card and desktop never invokes native share. PR #44 (`421d6bca`) preliminarily implements the footer with one mobile action and two explicit desktop actions: image-only card or text+URL. This does not select the final release mode: D0, D1, D2 or the two-action candidate remains open until the Windows/macOS matrix and owner decision. Header/mobile-menu placement is intentionally deferred, V1 uses factual catalog-bound copy, and poster cubes remain future V2.
...
2. **P0 — Smart Update quality stabilization:** G2 plus M5 semantic age-rating reconciliation/backfill, регулярные аудиты, incident burn-down, root-cause fixes, closure-grade replay, dashboard/SLO.
...
5. **P0 — UI release freeze:** F5 plus clean preview contract, responsive-navigation comparison with the shallow desktop hybrid as default, H1 categories-first/static/semantic-motion prototype comparison and explicit owner `ship|defer`, M5 shared age badge/fact across the complete renderer inventory, PR #38 medallion baseline, completed/owner-deferred medallion P0 shortlist and exhaustive SHA-bound Playwright medallion surface inventory/screenshots with zero visual defects, followed by real-device/a11y/owner evidence. If H1 ships it is integrated before freeze; UI/UX is refrozen if any later feature integration changes it.
...
| F18 service share | Contract and release gates documented; preliminary footer-only component/controller, mobile share, explicit desktop image/text copy, metric-bound renderer/assets/lab and browser evidence recorded from PR #44 (`421d6bca`) | **Partial implementation; header/mobile-menu, native matrices, Pharmastaff reference, main merge and production activation missing** |
