...
| **M1-QA** | Exhaustive medallion visual cleanliness | **Missing / release gate specified** | Lab load/screenshot evidence exists, but there is no SHA-bound Playwright inventory and screenshot verdict for every actual target page/layout | Discover every rendered surface; capture every actual static page at 390/1440 plus breakpoint combinations and Telegram layouts; zero clipping, dirty/cut shadows, alpha mattes, overlap, overflow, broken/unreadable medallions or uncaptured targets |
...
- [ ] Run the separate [medallion visual QA protocol](../features/static-site-pages/medallion-visual-qa.md): inventory every actual renderer/URL, capture every medallion-bearing static page through Playwright at `390×844` and `1440×1000`, capture each distinct breakpoint/layout plus lab asset sheet and enabled Telegram variants, and attach the SHA-bound inventory/verdict ledger.
