...
| **M2-B** | Фирменная бирка на text-free Telegram/VK images с release timestamp | **Publishers implemented; brand derivative planned** | Existing Telegram/VK event publication remains canonical; product contract фиксирует `visual_only` eligibility, shared derivative, quality/rollback и удаляемый VK postponed smoke | Add only off/shadow/on derivative selection for `2026-07-30 18:00 Europe/Kaliningrad`; deterministic high-quality render, TG/VK parity, visual matrix and postponed-create/delete evidence |
...
- [ ] Реализовать и принять [social publish brand tag](../features/event-media/social-publish-brand-tag.md): не меняя существующие Telegram/VK publishers, с `2026-07-30 18:00 Europe/Kaliningrad` (`2026-07-30T16:00:00Z`) выбирать для event images с уверенным `visual_only` один high-quality mobile-site brand derivative; `ocr_text|unknown`/unsafe/render failure используют оригинал; до включения VK postponed smoke проверен full-size/API и удалён.
