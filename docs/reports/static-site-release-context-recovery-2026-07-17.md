...
| Production publisher | not in main | `62ba7110` plus later hardening in production integration branch | **Side candidate/Blocked**; no release claim |
| Identity/favorites/reminders | not in main | commit `8bc59dc0` plus dirty completion in `static-release-identity-saved-events`; local contracts passed, production migration intentionally not applied | **Unintegrated WIP**; preserve and review, never claim production |
| Transport refresh | not in main | dirty `static-release-transport-refresh` worktree contains provider jobs, fan-in, scripts and tests | **Uncommitted WIP**; needs scoped commit/review/integration |
...
The crash lane map is preserved at
`static-release-foundations-20260717/.codex/integration/static-release-foundations-20260717-LANE_MAP.md`
in the local worktree set. It proves unfinished work exists; it is not itself a
mergeable or production artifact.
