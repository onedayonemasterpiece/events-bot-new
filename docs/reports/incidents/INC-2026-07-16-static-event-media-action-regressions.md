...
- 2026-07-17: the first checked Smart Update/Kaggle secret candidate was rejected
  because its source tree came from `main` before the accepted v11 integration
  branch had been promoted. Catalog, geometry and overflow checks passed against
...
20. The automatic pipeline correctly packaged its checked-out repository SHA,
    but release acceptance never proved that this source contained the latest
    accepted event-detail integration. The v11 source remained on
    `integration/static-event-v11-transport-phone-carousel` while `main` still
...
- repository freshness was treated as equivalent to template freshness; no
  machine-readable contract tied Smart Update/Kaggle artifacts to the accepted
  v11 source and all of its media-family scenarios.
...
- generating a checked production artifact or secret Smart Update candidate.
...
- `site/src/data/eventTemplateContract.json`, production/secret release
  manifests and the Kaggle source payload.

### Mandatory checks before closure or deploy

- every generated event page carries `static-event-detail-v11` and accepted
  source SHA `3b17e536e4dffa9c9fcebab6e641a7cd4ba99b6a` markers and mounts
  `DesktopEventPage.astro`, never the legacy desktop `EventHero + event-grid`;
- preview acceptance renders every scenario declared by the template contract,
  including horizontal, horizontal+OCR companion, document-contain,
  portrait-series, low-resolution and quality-fallback combinations;
- production and secret-candidate manifests record the same template contract,
  and their checks fail before publication if any catalog page bypasses the
  `editorial|split` router;
