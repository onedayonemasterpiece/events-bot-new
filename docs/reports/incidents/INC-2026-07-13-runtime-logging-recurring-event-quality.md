...
- [x] keep Telegram public hook writing exclusively on Gemini Lite, bounded to one call with no cross-model fallback; use canonical grounded text if Lite is unavailable;
...
- deployed SHA: `be2b5105` (`origin/main`), including `533db6fa` quota/media prevention, `1b1364c8` exact live-id recovery, `cb22ff3d` managed source-ledger reconciliation, `0a96da95` partial-media repair and `be2b5105` Lite-only public-writer correction
- deploy path: Fly releases `1630` (logging/retention), `1632` (staged Smart Update), `1633` (VK postponed idempotency/E2E acceptance), `1635`/`1636` (quota/media), `1637`/`1638` (live-id + source ledger), `1639` (partial late media), superseded `1640`/`1641` hook-routing mitigations and `1642` (Lite-only Telegram public writer)
- regression checks: runtime logging/disk/source-debug `9 passed`; guide-media retention and adjacent guide publishing `13 passed`; Smart Update focused contracts `42 passed`; VK postponed/E2E focused contracts `3 passed`; Telegram public-writer routing `3 passed`. Full `tests/test_tg_event_publish.py` was `70 passed, 9 failed`: one stale mock signature and eight already-recorded date-sensitive June fixtures, none touching the model-routing assertion; these failures are not represented as green.
...
- public-writer routing: release `1642` hard-codes `gemini-3.1-flash-lite` for `tg_event_publish`, ignores runtime model overrides, clears the global fallback chain and permits one provider attempt. A live production smoke (`request_uid=f860d1de-c319-4790-ac55-815444810930`) recorded exactly one Lite reservation and one successful Lite response (`357` tokens total), with no Gemma reservation or model fallback. Internal event processing remains independently routed.
