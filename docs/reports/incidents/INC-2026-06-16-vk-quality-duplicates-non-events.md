...
- 2026-06-16 — operator reports the same false `Дайджест` surfaced again as public `https://vk.com/wall-231920894_3512`; authenticated VK API confirms it is the promoted wall post with `postponed_id=3484`.
- 2026-06-16 — `wall-231920894_3512` is deleted and postponed queue scan finds no remaining `Дайджест` / `приходи` postponed items.
...
5. Immediate cleanup deleted the then-visible VK ids, but a stale VK postponed item could later be promoted to a new public id while keeping the original `postponed_id`; the scheduled prune path did not yet resolve that drift for explicitly cancelled future events.
...
- Production repair verification:
  ...
  - live posts promoted from stale postponed ids are removed and no matching postponed `Дайджест` items remain;
...
- Deleted leaked public VK post `wall-231920894_3512`, which VK exposed as a new live wall id for original `postponed_id=3484`, and verified no remaining postponed `Дайджест`/`приходи` copies.
...
- Extended VK post pruning so explicitly non-active events are eligible even when their date is in the future, and so recent live wall posts can be resolved/deleted by their original `postponed_id`.
...
- follow-up regression 2026-06-16: `wall-231920894_3512` deleted with `wall.delete -> response=1`; follow-up code adds postponed-id drift pruning for non-active future events and tests `test_prune_deletes_cancelled_future_event` / `test_prune_deletes_live_post_by_postponed_id_for_cancelled_event`; deployed SHA `b202876b79e35ebb8b25e7805577bade5f4dedb3`, image `registry.fly.io/events-bot-new-wngqia:deployment-01KV7MGSV51QY32TM0BH505VRW`, machine version `1430`; post-deploy VK API shows `wall-231920894_3512` is deleted and exact postponed `Дайджест` matches are empty.
