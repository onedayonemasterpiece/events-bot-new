...
- `INC-2026-05-12-kenigsberg-postprocess-db-lock-and-final-copy.md`
  - Scope: Kenigsberg manual `/kenigsberg` launch blocking, Kaggle handoff/post-processing status transitions, SQLite-lock retry around video session/status writes, `/a` list-bans routing, final-copy handling from `thoughts.md`, source-video CFR frame extraction, source-range anti-repeat, Kenigsberg notebook story-helper import gating, and manual stuck local-handoff cleanup.
