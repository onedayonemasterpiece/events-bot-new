...
- `INC-2026-05-12-kenigsberg-postprocess-db-lock-and-final-copy.md`
  - Scope: Kenigsberg manual `/kenigsberg` launch blocking, Kaggle handoff/post-processing status transitions, SQLite-lock retry around video session/status writes, `/a` list-bans routing, final-copy handling from `thoughts.md`, source-video CFR frame extraction, and manual stuck local-handoff cleanup.
  - Must not regress: a completed Kaggle run must not leave `/kenigsberg` blocked behind a misleading generic “рендерится” message; SQLite locks during heavy imports must be retried for Kenigsberg state/status writes and local-to-Kaggle handoff metadata; story text must be copied from curated `thoughts.md` entries and only split into readable screens without LLM rewriting; source scenes must not be extended by visibly repeating the last decoded source frame; stale `local:KoenigsbergStories` rows must be auto-failed or clearable via `/kenigsberg unlock`.
