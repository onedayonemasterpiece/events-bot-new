...
- `INC-2026-05-13-kenigsberg-production-story-boosts-required.md`
  ...
  - Must not regress: direct `@mostvkenig` Telethon story upload must not be the required/blocking render gate while Telegram returns `BOOSTS_REQUIRED`; encrypted Business story targets with `can_manage_stories=True` are the required production gate; after any failed same-day scheduled/catch-up production attempt, deploy closure requires a compensation rerun and terminal publish evidence.
