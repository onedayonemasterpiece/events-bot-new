...
- [x] Implement Yandex OAuth notification email persistence and fallback email capture UI foundation: branch `feature/event-email-notifications-static-20260702` adds static calendar consent UI plus `event-email-follow` enqueue function.
- [x] Implement calendar-follow durable outbox foundation, Postbox dry-run sender adapter, suppression tables, YDB stats contract and targeted tests. Real Postbox sending remains disabled until Postbox identity and YDB stats writes are verified.
- [ ] Implement provider callbacks/suppression ingestion and admin queue status.
- [ ] Add static event page lifecycle badge and cancellation/reschedule note.
- [x] Add foundation email tests for confirmation, 24h reminder skip, reschedule diff, cancellation disclaimer, idempotency key shape, rate limits, Postbox dry-run and YDB stats visibility.
- [ ] Extend worker tests for real retry/backoff and provider callback suppression ingestion when the outbox drainer is implemented.
