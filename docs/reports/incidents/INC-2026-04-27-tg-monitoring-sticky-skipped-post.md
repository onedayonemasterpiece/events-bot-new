...
3. The affected post was therefore sticky: future monitoring/import passes would classify it as `metrics_only` and skip Smart Update instead of retrying the valid event payload.
...
- Regression tests cover the sticky-skip retry and documented-skip no-retry cases.
...
- deploy path: —
- regression checks: `pytest -q tests/test_tg_monitor_reprocess_incomplete_scan.py -vv` passed locally on 2026-04-27
