...
- **VK formatting/post ids:** `sanitize_for_vk` now breaks inline Markdown `###` headings into VK-friendly plain heading blocks before `wall.post`/`wall.edit`, and `sync_vk_source_post` lazy-resolves stale stored `postponed_id` URLs to the real wall id if `wall.getById` for the stored id is empty.
