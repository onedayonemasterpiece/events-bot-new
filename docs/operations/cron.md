...
If a catch-up dispatch materializes a `guide_monitoring` full `error`/`failed` run (for example the Kaggle kernel reaches terminal `ERROR`), the slot also stays pending, but the watchdog defers the next attempt by `GUIDE_MONITORING_FAILED_RETRY_SECONDS` instead of pushing a new Kaggle kernel every watchdog tick.
...
- `GUIDE_MONITORING_FAILED_RETRY_SECONDS` – cooldown before the critical guide watchdog retries a `full` catch-up whose latest materialized attempt finished as `error`/`failed`, including terminal Kaggle kernel failures (default: `3600`, minimum: `300`).
