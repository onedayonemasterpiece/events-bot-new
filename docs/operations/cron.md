...
- **KGD80 social activity report** – sends an automatic private Telegram report
  to the resolved superadmin chat every evening
  (`KGD80_SOCIAL_REPORT_TIME_LOCAL`, default `20:30 Europe/Kaliningrad`) when
  `ENABLE_KGD80_SOCIAL_REPORT=1` (default enabled). The report is generated by
  `kgd80_social.py`, says explicitly that it is automatic, aggregates verified
  VK metrics for `80 историй о главном` promo posts, and shows the current
  social-bonus points by type: views, likes, comments, reposts.
