...
- **Exhibition duplicate audit** – read-only daily acceptance gate for the Smart Update Vector Identity Gate rollout. It scans the file-backed SQLite DB with `scripts/inspect/audit_public_exhibition_duplicates.py` semantics and treats similarity only as recall. Current/future canonical `/vystavki/` candidates are partitioned into confirmed duplicate, evidence-backed keep-distinct and unresolved counts from pair-correlated final/manual identity-ledger verdicts; Event IDs or `linked_event_ids` alone are not evidence. The run succeeds only when the **complete current corpus** has confirmed=0 and unresolved=0, records `ops_run(kind='exhibition_duplicate_audit')`, joins rollout counters from `scripts/inspect/audit_identity_gate_rollout.py`, and alerts on actionable pairs. Separate window metrics use `event.added_at >= EXHIBITION_DUPLICATE_AUDIT_SINCE_DATE` when set, otherwise `current_date - EXHIBITION_DUPLICATE_AUDIT_SINCE_DAYS`; they do not hide older current candidates (schema fallback counts all pairs). Disabled by default; enable after `SMART_UPDATE_IDENTITY_GATE=enforce` with `ENABLE_EXHIBITION_DUPLICATE_AUDIT=1`.
...
- **Video announce `/v tomorrow`** – optional scheduled automatic `/v` run when `ENABLE_V_TOMORROW_SCHEDULED=1` (legacy alias: `ENABLE_V_TEST_TOMORROW_SCHEDULED=1`).
  ...
  - if all configured video lanes are busy, the scheduled `/v tomorrow` attempt is recorded as `ops_run.status='skipped'` with `skip_reason='video_lanes_busy'` and no new `SELECTED` session/dataset is created. The watchdog defers another busy retry for `V_TOMORROW_WATCHDOG_BUSY_RETRY_SECONDS` (default 900), and the operator notice is deduplicated per scheduled slot for six hours. A materialized `SELECTED` scheduled slot also suppresses same-day watchdog cloning so a stale pre-handoff attempt does not multiply while an operator investigates;
...
- **CherryFlash partner tracks** – always-registered daily partner story tracks with per-track defaults and watchdog retry until `22:00 Europe/Kaliningrad`.
  ...
  - Partner scheduled attempts use the same video-lane preflight as `/v tomorrow`: a busy lane pool is an explicit `skipped/video_lanes_busy` attempt with no new `SELECTED` session, its watchdog observes the persisted busy retry window, and repeated notices for the same track/slot are suppressed. Real created sessions must reach confirmed Kaggle handoff before the `ops_run` is marked `success`.
...
- `EXHIBITION_DUPLICATE_AUDIT_RAISE_ON_DUPLICATES` – when enabled (default), any confirmed or unresolved pair finishes the ops run as `failed` and raises a scheduler job error; fully grounded `KEEP_DISTINCT` candidates are non-actionable.
...
- `V_TOMORROW_WATCHDOG_BUSY_RETRY_SECONDS` – persisted defer window after a watchdog attempt records `video_lanes_busy` or `render_in_progress` (default: `900`, minimum: `60`).
