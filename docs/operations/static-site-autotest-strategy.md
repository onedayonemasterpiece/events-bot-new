...
- общий `session_fixture` для Search/personalization/authorized page suites;
...
| Focus onboarding/Auth/Supabase/Yandex relay/email hook | да | да | да | да | `real_mail_otp` |
...
- извлечь общий issuer из focus-specific admin credential script;
- реализовать neutral callback/bootstrap и ephemeral Playwright state;
- доказать reload, второй context, `auth.getUser`/protected probe;
- доказать `product OTP=0`, `external mail=0`;
- добавить fixed persona pool и per-worker allocation;
- запретить fallback на real mail и state artifact/cache upload.
