...
Для полного предрелизного аудита **внутри галерей событий**, включая legacy `Event.photo_urls`, точный SHA-256 скачанных байтов, perceptual candidates и обязательный визуальный просмотр каждого multi-image event, используй отдельный [event image duplicate audit](event-image-duplicate-audit.md). Команды ниже проверяют storage/path rollout и являются только одним слоем этого аудита.
