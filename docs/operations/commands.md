...
| `/vk_auto_import [N|all] [--include-skipped]` | optional limit flags | Суперадмин: авторазбор очереди VK (таблица `vk_inbox`) через Smart Update. По умолчанию без аргументов обрабатывает всю активную очередь; `N` ограничивает количество постов. Ручной запуск не ждёт общий heavy-job gate, чтобы его можно было использовать во время удалённого Telegram/Kaggle monitoring; scheduled auto-import остаётся сериализованным. Показывает прогресс `X/Y`, унифицированный отчёт с фактами и количеством добавленных иллюстраций, ссылки на Telegraph и `/log`. |
...
| `/kenigsberg [status|bans|unlock|ban #N ranges|bans reset]` | optional subcommand | Суперадмин: управление генератором сторис `Мост в Кёнигсберг`. Без аргументов запускает production Kaggle-генерацию и публикацию сторис в `@mostvkenig`; отдельного `KENIGSBERG_STORIES_*_ENABLED` gate больше нет. `bans` показывает запрещённые source-отрезки, `ban #15 1-3, 7, 16-17` мапит секунды готового выпуска обратно на исходные видеоотрезки, `unlock` снимает зависшую local pre-handoff сессию, `bans reset` очищает баны для тестирования. Через `/a` эти действия доступны фразами вроде «в выпуске kenigsberg #15 бан 1-3, 7». |
...
| `/promo [report|seed80|add festival ...|add event ...|pause ID|start ID|archive ID]` | optional subcommand | Суперадмин: управление промо-кампаниями для будущих событий/фестивалей. `/promo seed80` заводит начальную кампанию фестиваля `80 историй о главном` до 2026-07-18, только если фестиваль уже есть и имеет будущие события. `add festival/event` создаёт кампанию с дефолтным сроком 90 дней, если дата не указана; `/promo report` показывает текущие будущие события и публичные `PUBLISHED_MAIN` видео-показы. Через `/a` доступны фразы вроде `продвигай события фестиваля Кантата` и `добавь в промо событие примерное название`. |
