...
| `/kenigsberg [status|bans|ban #N ranges|bans reset]` | optional subcommand | Суперадмин: управление MVP генератором сторис `Мост в Кёнигсберг`. Без аргументов запускает ручную генерацию только когда включён `KENIGSBERG_STORIES_KAGGLE_ENABLED=1`; сейчас безопасно сообщает, что Kaggle запуск закрыт. `bans` показывает запрещённые source-отрезки, `ban #15 1-3, 7, 16-17` мапит секунды готового выпуска обратно на исходные видеоотрезки, `bans reset` очищает баны для тестирования. Через `/a` эти действия доступны фразами вроде «в выпуске kenigsberg #15 бан 1-3, 7». |
