...
- валидное приглашение создаёт/восстанавливает `anonymous_session`; email и
  Yandex identity не являются admission wall для NPS, page score, текста или
  screenshot;
- anonymous subject остаётся владельцем данных; identity upgrade/merge —
  отдельная операция и только она может дать raffle eligibility;
