...
|---|---|---:|---:|
| `anonymous` | анонимный product contract | 0 | нет |
...
Пробел: admin-issued credential пока привязан к focus onboarding и не оформлен
как общий per-worker fixture для Search, personalization и других authorized
surfaces.
...
- извлечь общий issuer из focus-specific script;
- создать neutral callback/bootstrap;
- сохранить ephemeral Playwright storage state;
- доказать reload + second context + protected probe;
- доказать `product OTP=0`, `external mail=0`;
- fail-closed redaction/cleanup.
