...
> **Статус:** каноническая продуктовая модель / research and release-design track.  
> **Дата среза:** 2026-08-03.  
> **Реализация в `main`:** существует статический `HomeHeroTalk`, но полного
> Hero-talk compiler, narrative engine, phrase-pack pipeline, page-end placement
> и production acceptance пока нет.  
> **Параллельная стратегия:**
> [онбоардинг стандартного пользователя](../static-site-onboarding/README.md).  
> **Глубокое исследование:** [research prompt](deep-research-prompt.md).  
> **Release companion:** [release plan](release-plan.md).  
> **Testing companion:** [testing strategy](testing.md).

## Owner-audit recovery candidate — 2026-08-28

The owner rejected the static event-feature block as the Home Hero-talk product
mechanism. Draft PR `#596` therefore introduces `HeroTalk@2` in
`mode=photo-mosaic`:

- the first useful event scene is present in static HTML;
- copy is split into accelerated semantic fragments rather than character-by-
  character typing;
- one exact canonical event image is projected through a `5×4` square tile
  mosaic;
- copy, CTA, factual details and image source change in one bounded scene
  transition;
- hover, focus or pointerdown pauses automatic succession without consuming the
  first link activation;
- no-JS and reduced-motion retain the complete first scene;
- the rejected static component is preserved only as the catalogued deprecated
  `HeroTalk@1` comparison.

This is a Git-bound product candidate, not production-root promotion and not
evidence that later compiler, cross-session or LLM stages are complete.
