...
VK daily использует компактные однострочные секции: утренний выпуск `НЕ ПРОПУСТИТЕ СЕГОДНЯ` показывает время начала события вместо даты, а вечерний `ДОБАВИЛИ В АНОНС` оставляет дату будущего события. Название в VK-строке кликабельно, если есть подходящий wall-post: сначала используется управляемый пост события в `vk.com/klgdevents` (`vk_repost_url`), затем исходный VK wall-post (`source_post_url` или `source_vk_post_url`).
