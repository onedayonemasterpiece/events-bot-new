...
В секции `ДОБАВИЛИ В АНОНС` daily может добавить подпись аудитории под наиболее популярными событиями: строка с отступом и кастомными emoji `❤️`/`🔂` из `@kenigevents adaptive pack`. Сигнал считается по сумме всех доступных source-post метрик события (Telegram + VK distinct posts), а не по одному VK-посту: `score = likes_sum + 5 * reposts_sum`, с daily-only адаптивным снижением порога при малом числе подписей.

VK daily использует компактные однострочные секции: утренний выпуск `НЕ ПРОПУСТИТЕ СЕГОДНЯ` второй строкой показывает полную дату выпуска (`18 мая 2026`), а в строках событий ставит время начала вместо даты; вечерний `ДОБАВИЛИ В АНОНС` оставляет дату будущего события. Название в VK-строке кликабельно, если есть подходящий wall-post: сначала используется управляемый пост события в `vk.com/klgdevents` (`vk_repost_url`), затем исходный VK wall-post (`source_post_url` или `source_vk_post_url`).
