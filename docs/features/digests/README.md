...
В секции `ДОБАВИЛИ В АНОНС` daily может добавить подпись аудитории под наиболее популярными событиями: строка с отступом и кастомными emoji `❤️`/`🔂` из adaptive pack. Сигнал считается по сумме всех доступных source-post метрик события (Telegram + VK distinct posts), а не по одному VK-посту: `score = likes_sum + 5 * reposts_sum`, порог `>=20`, лимит — не больше `floor(N * 0.10)` событий секции.
