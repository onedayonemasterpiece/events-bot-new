...
- Дважды в сутки (`vk_post_prune_scheduler`, по умолчанию `02:30,14:30`
  ...
  (`reposts.count == 0`), чтобы лента не рекомендовала прошедшие события.
- Удаляются только посты, чей `Event.source_vk_post_url` указывает на
  ...
  с непустым `reposts.count` не трогаются. Прошлость берётся из БД, поэтому daily,
  опросы и промо-репосты не сопоставляются и не удаляются.
...
- Promo campaign activity `vk_story` publishes a caption-free image story into
  ...
  the source wall image/poster, passes the source wall URL as VK internal
  `link_url` when possible, and treats upload as successful only after
