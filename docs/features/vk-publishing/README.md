...
- После Smart Update событие может получить отдельный VK-пост в событийном сообществе `VK_EVENTS_GROUP_ID`; если переменная не задана, используется исторический `VK_AFISHA_GROUP_ID`.
