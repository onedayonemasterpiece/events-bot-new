...
- В заголовке события `📅` показывает весь подтверждённый диапазон дат (например, `21 сентября — 13 декабря`), включая начавшиеся, но ещё идущие мероприятия. Вычисленный `end_date` сам по себе не расширяет заголовок. Поиск живого managed-поста по старому postponed ID принимает также прежний точный заголовок с одной начальной датой.
...
- Загрузка каждой картинки через VK upload server делает до трёх попыток с новым `photos.getWallUploadServer` URL. Для WebP/AVIF VK-путь преобразует файл в JPEG quality=95, subsampling=0: в инциденте 24 сентября обычный JPEG от того же исходника возвращал HTTP 200 с пустым `photo`, а этот профиль вернул непустое `photo`. HTML/timeout/JSON failures и пустое поле `photo` от `upload.php` считаются неудачной загрузкой; `photos.saveWallPhoto` с пустым `photo` не вызывается, частичная публичная медиагруппа не создаётся.
...
- Если у события есть медиаледжер, но все его кандидаты явно отклонены (`rejected`, `duplicate`, `unavailable`), sync существующего VK-поста передаёт пустой список вложений в `wall.edit` и удаляет старую ошибочную картинку. В этом случае fallback со старой Telegraph-страницы запрещён. Временная пустота `photo_urls` при ещё одобренном или ожидающем проверку постере не очищает уже опубликованное медиа.
...
- VK `error_code=9` является actor-level flood signal, а не обычной transient-ошибкой отдельного запроса. Общий VK transport после первого ответа `9` немедленно прекращает внутренние повторы, открывает отдельный circuit для точного credential на `VK_FLOOD_COOLDOWN_SECONDS` (по умолчанию один час) и не блокирует сервисный read-token. Следующие вызовы этого credential до окончания cooldown отклоняются локально без обращения к VK. JobOutbox переносит затронутые задачи к концу cooldown и распределяет их через детерминированный `VK_FLOOD_OUTBOX_JITTER_SECONDS` (по умолчанию 15 минут), не создавая по уведомлению на каждую запись очереди.
- Явные actor/auth отказы (`VK_FALLBACK_CODES`, `method is unavailable with group auth`, `access denied`) также не повторяются тем же credential внутри transport. Внешний actor-loop может перейти к следующему реально отличающемуся токену. Flood-circuit из photo upload обязан пробрасываться в `vk_sync`, а не превращаться в `attachments=0` и последующие VK-вызовы той же задачи.
