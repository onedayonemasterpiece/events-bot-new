...
- Тот же fail-closed contract обязателен для promo `vk_publication`: Telegram-origin кандидаты с пустым `event.photo_urls` пропускаются на выборе, а если promoted Telegram-origin событие всё-таки дошло до публикации без `photo...` attachments, runner должен логировать media snapshot (`event_id`, `campaign_id`, `activity_id`, `source_kind`, `photo_urls_count`, `attachments_count`) и завершать действие ошибкой `vk_sync_missing_media_for_telegram_event`, а не создавать text-only promo wall post.
