...
- В конце событийного поста перед редакционным футером добавляется VK hashtag line: базовые `#анонс #анонс39 #кудапойтиКалининград #афишаКалининград`, город события, две даты события (`#17мая` и `#17_мая`) и, если событие привязано к фестивалю, отдельный хештег из названия фестиваля без пробелов/разделителей (например `#80историйоглавном`).
