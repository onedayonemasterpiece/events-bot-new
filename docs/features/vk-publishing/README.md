...
- Для отложенных постов VK может вернуть `post_id`, который фактически является `postponed_id`, а реальный `wall.get` item получает другой `id`. `post_to_vk` обязан сразу после успешного `wall.post` резолвить `postponed_id -> id` через `wall.get` и сохранять URL с реальным wall id, иначе последующие `wall.getById`/`wall.edit` будут видеть пустой/удалённый пост.
