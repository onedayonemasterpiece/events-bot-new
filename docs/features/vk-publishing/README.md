...
- Source posts that look like third-party ticket giveaways (`розыгрыш` / `разыгрываем` / `выиграть` + `билет` / `пригласительный`) are fallback content for managed VK/TG event publication: they are skipped only when a non-giveaway event is already queued or published, and still publish when no alternative exists.
...
- Runtime-параметры отложки: `VK_POSTPONED_ENABLED` (default `true`), `VK_POSTPONED_TZ` (default `Europe/Kaliningrad`), `VK_POSTPONED_MIN_INTERVAL_SECONDS` (default `600`), `VK_POSTPONED_START_HOUR` (default `6`), `VK_POSTPONED_MAX_ANCHOR_AHEAD_SECONDS` (default `64800`). Последний параметр защищает обычный Smart Update VK scheduler от случайного дальнего postponed-хвоста: marked `afishaengagement` debug/shadow посты игнорируются отдельно, а обычные anchors дальше заданного горизонта не сдвигают свежую очередь на дни вперёд.
