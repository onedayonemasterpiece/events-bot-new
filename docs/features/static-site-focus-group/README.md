...
> **Статус на 31.07.2026:** программа запущена, но инцидент
> `INC-2026-07-30-focus-email-otp-false-success` остаётся открытым до повторного
> live E2E кода и ссылки. Устойчивый direct/relay transport принят шестью
> телефонными проверками; его распространение на все динамические поверхности,
> безопасная очередь действий и компактный обмен данными находятся в
> `integration/static-site-resilient-egress-20260731`.
> **Аудитория:** закрытая исследовательская группа до 200 участников.
> **Период:** 30 дней от присоединения; затем аккаунт и установленное приложение
> продолжают работать как обычный KenigEvents.
> **Release truth:** только `origin/main`; текущие реализации нельзя считать
> доставленными до merge, миграций, root publication и live checks.
> **Инцидент:**
...
- `/fokus-gruppa/` показывает программу, одну массовую ссылку и QR приглашения.
- `/fokus-gruppa/priglashenie/` ведёт пользователя последовательно: установка
  или отказ, затем email либо Яндекс, затем подтверждение участия.
- Email даёт два равноправных способа: одноразовую ссылку и шестизначный код.
  Шесть цифр — минимально допустимая длина Supabase Auth. На телефоне открывается
  цифровая клавиатура, проверка начинается автоматически после шестой цифры и
  имеет single-flight guard.
- OTP issuance выполняется ровно по одному заранее выбранному здоровому
  маршруту. Неясный timeout не выдаётся за успешную отправку и не запускает
  слепой повтор через второй маршрут.
- Яндекс и email используют один Supabase Auth user/session; подтверждённый
  email, включая email из Яндекс identity, проецируется в приватную таблицу
  участников.
- Неподтверждённый участник может продолжить на текущем устройстве; локальная
  30-дневная метка — UX continuity, а не доказательство личности.
- На обычных page families активному участнику показывается компактный Lab-блок:
  оценка страницы, сообщение о проблеме со скриншотом и приглашение другого
  человека. Оценка хранится по типу страницы 24 часа, допускает изменение и
  отправляется через ограниченную идемпотентную очередь.
- Browser storage имеет явные TTL, лимиты числа записей и байтов; устаревшие
  preview-ключи удаляются. Auth session storage Supabase не переписывается
  прикладной компактизацией.
- Динамические данные и Auth используют общий resilient client. Safe reads могут
  восстановиться вторым маршрутом; OTP и другие неоднозначные записи не
  дублируются. Серверные RPC остаются источником RLS, квот и дедупликации.
- Диагностика `/fokus-gruppa/diagnostika-ustoychivost/` намеренно проверяет
  прямой, резервный и общий клиент раздельно. Это инструмент расследования, а
  не альтернативный backend.

Live gate перед закрытием инцидента: отдельные реальные issuance для кода и
ссылки, повторное использование каждого токена, email/Yandex participant rows,
provider receipt и проверка после публикации с телефона.
...
start-controller возвращает его сразу в обычную афишу, а не в отдельное
тестовое приложение. Сброс персонализации не
...
- PWA на каждом старте бесшумно восстанавливает эту же device session и сразу
  открывает обычную афишу, без повторного email/Яндекс;
