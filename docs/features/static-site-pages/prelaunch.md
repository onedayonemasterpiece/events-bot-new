# Заглушка запуска 20 сентября: форма уведомления

## Scope

До публичного запуска корень `https://kenigevents.ru/` показывает утверждённую
prelaunch-страницу с датой запуска **20 сентября 2026 года**. Композиция, фон,
visual CSS и остальные маршруты не меняются. Их принятый byte-level baseline —
production source commit `9d8fc9203a69f385407a57e23310bb47f2db4e2d`; компонент и layout заглушки
закреплены отдельными source-хешами, потому что дата синхронно присутствует в
видимом тексте, semantic `<time>`, SEO/GEO metadata и сообщениях формы.
