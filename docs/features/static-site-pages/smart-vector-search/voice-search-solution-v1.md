...
решает, что требуется для полезного ответа, и возвращает единый структурный план:

- existing intent/queryPlan: связь с диалогом и релевантное наследование условий;
- knowledgeAction: internal либо web_lookup;
- externalNeed: null либо конкретное неизвестное, минимальный справочный запрос
  и краткое назначение результата (не полная переписка/профиль и не рассуждения);
- clarification: none, blocking или optional; при необходимости один короткий
  вопрос о существенной неоднозначности и условие, которое он уточняет;
- assumptions: только существенные допущения, которые важно явно показать;
- refinementOpportunity: null либо кандидат полезного следующего шага с условием
  применимости после выдачи. Это не обязательный вопрос/совет в каждом ответе.

Это НЕ четыре взаимоисключающих режима. Можно использовать только базу и дать
необязательное уточнение; можно уточнить внешнюю отсылку и затем показать внутренние
карточки без вопросов. Сложность сама по себе не включает интернет: отличаем
нехватку внешнего знания от сложного рассуждения и неизвестного желания человека.
Самооценка уверенности модели не является единственным критерием; внешний этап
должен иметь конкретный разрешаемый пробел, а вопрос — существенное влияние на
выборку. Длина запроса и отсутствие даты не являются механическими правилами.

Исполнение:
1. blocking: только если без ответа велик риск искать принципиально не то;
   не тратить web/retrieval/verification на заведомо неразрешённое намерение.
   Ответ пользователя продолжает сохранённый draft, повторный ввод не нужен.
2. optional: выполнить полезный поиск сейчас; существенное допущение/один вопрос
   можно включить в короткий ответ ассистента, не блокируя карточки.
3. web_lookup: ограниченное обогащение внешней справкой (исполнитель, отсылка,
   неизвестное имя/актуальный факт), затем обычный внутренний поиск. Интернет
   не угадывает личное предпочтение и не подменяет историю диалога.
4. refinementOpportunity: исходный план может предложить направление, но лишь
   существующий post-search editorial решает по реальным результатам, полезно ли
   его показать. До retrieval нельзя заявлять, что результатов слишком много,
   слишком мало или нет. Нет отдельного обязательного дополнительного LLM-пасса
   ради совета; отсутствие уместного продолжения означает null и никакого блока.

Пример поведения (не словарные правила): простой жанр/город/даты — база; культурная
отсылка, для которой не хватает знаний, — справочный web; неясно, к какому из двух
обсуждённых мест относится «там», — вопрос, если это существенно; детский запрос
без возраста может получить подходящую широкую выдачу и необязательное уточнение
возраста, а не обязательный барьер перед каждым поиском.

Чистый UI: вопрос/предложение — одна естественная фраза внутри ответа ассистента
перед карточками, а не отдельное окно, технический footer, обязательные кнопки
«улучшить подборку» или повторяющееся приглашение после каждой выдачи. При
blocking вместо ложных карточек — один понятный вопрос; при поиске сохраняются
существующие сообщение пользователя, skeleton и обычные карточки/действия.

Сервер сохраняет границы: внутренние event IDs/ссылки/даты/доступность, проверка
происхождения внешних фактов, игнорирование инструкций из веб-страниц, общий limiter
и deadline. Справочные гипотезы не превращаются незаметно в обязательные фильтры.
Web failure не изображается пустой внутренней выдачей: fallback к базе при достаточном
понимании либо честное уточнение. Ноль результатов сам по себе не запускает web.

«Один запрос» здесь означает один LLM-вызов для выбора стратегии; ASR, выполнение
web, retrieval и итоговая проверка/ответ не объявляются одним сетевым обращением.
При встроенном Google Search модель может выполнить несколько оплачиваемых
поисковых запросов внутри одного API-вызова: бюджет нельзя считать просто по
количеству вызовов. Перед включением нужны проверка совместимости schema/tools,
учёта расходов, минимизации передаваемых данных и правил атрибуции провайдера.
Источник механики: https://ai.google.dev/gemini-api/docs/generate-content/google-search .

Acceptance TO-BE: одинаковые диалоги с внутренним, выборочным web и уточняющим
маршрутом; сравнить релевантность/пропуски, ошибки наследования, полезность вопросов,
долю ненужных web/вопросов, задержку и стоимость. Отдельно проверить helpful optional
