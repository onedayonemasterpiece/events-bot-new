...
| Audience facts v3 implementation | Gate B/D/E PASS; full shadow WATCH; Fly pending | clean-main branch `integration/static-collections-audience-shadow-main-20260803` selectively preserves the current project-scoped rolling/Pacific limiter. Prior 50-source Gate B and fixed-20 Gate D remain valid because prompt/model/definitions/validator did not change. Post-limiter immutable Telegram/VK/parser Gate E is accepted: warm collection calls/sends/writes are zero, exact binding and product SHA are stable; parser changes only `EventSource.imported_at`; unrelated Telegram/VK prose drift is issue #297. Full current/future coverage is `62 = 2 evaluated + 60 deferred + 0 unprocessed` over a `377`-event catalog. Kaggle then completed an exhaustive 377-event StaticSiteBuilder slice with BGE/QA PASS, 0 provider calls and product WATCH. Zero families and the deferred supply keep publication blocked. See [clean-main integration report](../../../.codex/integration/static-collections-audience-main-INTEGRATION_REPORT.md) |
