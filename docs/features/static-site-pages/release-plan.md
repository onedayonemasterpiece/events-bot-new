...
| Audience facts v3 implementation | Code PASS; real Gate B NO-GO | separate stacked branch `integration/static-collection-facts-v3`; existing one-call adjudicator now returns child/family/joint facts with exact persisted-source quotes, per-source warm receipts and deterministic legacy projection. Fresh 50-source Gemma-only replay proved one send/source, zero writes and 100% quote validity, but recall/provisional review consistency failed; apply/warm, ingestion and Fly canary are therefore not run. See [integration report](../../../.codex/integration/static-collection-facts-v3-INTEGRATION_REPORT.md) |
