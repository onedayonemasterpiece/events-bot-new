...
| Atomic event-site publisher (`62ba7110`) и последующее hardening | open PR `#43`, 66 commits behind current `main` and conflicting on 2026-07-17 | **Нет:** отдельный side candidate; не включён автоматически в текущую integration branch и не является release truth |
| Identity/favorites/reminders foundation | clean pushed worker `bcd1d118`; accepted into fresh-main integration `integration/static-release-identity-transport-20260717` | **Да как unapplied code candidate:** SQL/RLS/RPC/rollback contracts пройдены; production migration/Edge/scheduler/mail остаются off |
| Transport refresh foundation | clean pushed worker `a83704b0`; accepted into ту же fresh-main integration branch | **Частично:** two-provider schema/LKG/fan-in/coalescing и synthetic mechanics canary готовы; official adapters/production status-ledger canary/schedule остаются blocked/off |
| Mobile-v8 и последние presentation UI candidates | side branches | **Нет:** отдельный UI review/integration gate; они не были неявно перенесены вместе с identity/transport foundation |
