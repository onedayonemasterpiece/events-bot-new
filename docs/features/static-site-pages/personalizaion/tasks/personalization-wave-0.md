# Задание кодовому агенту: P13N-00 characterization + extraction
...
> **Запрещено:** считать эту волну готовой персонализацией.
...
3. `docs/features/static-site-pages/personalizaion/personalization-implementation-contract.md`;
4. `docs/features/static-site-pages/personalizaion/personalization-current-runtime-audit-2026-08-02.md`;
5. `docs/features/static-site-pages/personalizaion/implementation-status.yml`;
6. `docs/architecture/personalization-data-ownership.md`;
7. открытый legal gate PR #266 и его фактический merge status;
8. `site/src/layouts/EventLayout.astro`;
9. `site/src/lib/resilientDataClient.ts`;
10. `site/src/lib/resilientSupabaseTransport.ts`;
11. `site/src/lib/backendOperationCatalog.ts`;
12. personal-feed/related/search tests и canonical `EventCard` runtime.
...
- source guard против нового giant-inline personalization code.
...
site/src/lib/personalization/legacy-profile.ts
...
- явные `LegacyProfileV1`, `RankCandidateV1`, `RankPlanV1`,
  `SurfacePolicyV1`;
...
- reproduces current compatibility rules для characterization;
...
Перенести без изменения behavior:
...
- weights передаются versioned config;
...
- current semantic flaw exact-hide→negative facet **не исправлять в Wave 0**,
  но локализовать за typed adapter и пометить P13N-01 test as expected legacy.
...
- Wave 0 может возвращать legacy behavior для уже существующих adapters, но
  registry должен уметь вычислить целевую policy и diagnostic drift;
- production DOM mutation под новой policy пока выключена.

### `presenter-plan.ts`

- строит абстрактный rank plan без DOM mutation;
- принимает current order, ranks, frozen ids;
...
computed plan ids/scores rounded
...
checked-in baseline. Нельзя молча классифицировать всё как thematic.

## 5. Characterization tests

### 5.1. Pure scorer fixtures
...
Цель guard — заставить дальнейшие изменения идти в shared modules. Он не должен
ломаться от несвязанных CSS/markup изменений layout.
...
- `local-shadow`: строит plan, но не меняет DOM и не пишет state/network;
...
- новый code path импортируем и покрыт pure tests;
...
- следующий PR P13N-01 может удалить legacy consent/storage без повторного
  извлечения scorer/surface contracts.
...
Начни с актуального main, прочитай весь personalization document index и
открытый legal PR #266. Сделай characterization + pure-module extraction без
изменения production behavior, без DB/remote writes и без tuning весов.
Обязательны generated route/surface inventory, unknown-surface fail closed,
source guard против нового personalization code в giant EventLayout inline
script, realistic scorer parity fixtures, real Astro build и честный evidence
report. Не выдавай regex-only tests, mock transport или draft component за
работающую production персонализацию.
```
