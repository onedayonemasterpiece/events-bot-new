...
- **ADD-EVENT-01 — Несколько дат/времени одной программы.** В accepted desktop Split/Continuous Editorial и mobile accepted-v8 detail все даты и времена сразу видны в компактном `Расписание`, без dropdown/`details`: дата выводится один раз, времена переносятся чипами, текущий слот не является ссылкой и имеет `aria-current="page"`, остальные ведут на occurrence с его URL/ICS/reminder. Desktop пишет день недели полностью и повторяет тот же schedule в нижнем `Перед посещением → Когда`; отдельная секция больших event-карточек выбор не дублирует. Проверить минимум 3 времени в один день + другие даты и плотный предел 7 дат × 5 времён без горизонтального скролла.
