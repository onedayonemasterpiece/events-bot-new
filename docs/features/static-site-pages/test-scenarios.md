...
- **ADD-LIST-28 — Mobile Popular dual density V20.** На `360×800`, `390×844`, `430×932` Popular использует один ordered DOM и `ListingMobileDensitySwitch@1`: default `Крупно` даёт горизонтальную shelf с 82–86vw карточкой и видимым продолжением, `Компактно` превращает ту же shelf в вертикальный scan-list без горизонтального scroll. IDs, counts, family allocation и ranking до/после совпадают; preference восстанавливается из `ke_listing_density_v1`, текущая видимая карточка сохраняет vertical anchor с допуском 16px. Radio group имеет 48px targets, roving tabindex и Arrow/Home/End; dock учитывает safe area и footer. Viewport meta не запрещает native zoom, pinch не перехватывается, OCR/unknown не кропается и не получает полей, document overflow отсутствует.
