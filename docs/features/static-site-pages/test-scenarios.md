...
| Generated event release journeys | `ADD-KEY-08..10`, `ADD-DISC-08..13` | Blocking Chromium production/candidate gate | `npm --prefix site run check:browser-release -- --root <generated-root>`; Smart Update/Kaggle invokes it before each archive; festival cards/images equal the manifest-bound active rendered subset and never exceed its SQLite source-ledger count | native Safari remains a root-rollout gate |
