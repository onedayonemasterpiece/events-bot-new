...
- **ADD-LIST-12 — Последняя карточка и полное название.** Canonical title целиком остаётся в DOM. Singleton центрируется как полный painted envelope (media + rail + copy), получает до 420px читаемого пространства и имеет оптический center offset ≤1px. В multi-card ряду enhancement получает только фактическая последняя видимая карточка при tail ≥72px и available ≥280px. После filter/resize/fonts actual rows пересчитываются; >4 строк, collision или overflow откатывают enhancement. До/после x/width/flex-basis других карточек совпадают с допуском 1px.
- **ADD-LIST-13 — Header/sticky stack geometry V16.** На desktop opaque sticky site header имеет 57px и точно переиспользует общий компонент. Под ним единая light discovery surface: expanded direct city links имеют читаемый размер/воздух, фактическое число строк измеряется runtime, а в `data-stuck=true` gaps/font/counts компактизируются. Time/daypart navigation остаётся отдельной семантической строкой, когда города не помещаются рядом. Города не имеют horizontal scroll/overflow. После scroll `elementFromPoint` во всём stack попадает в header/discovery controls, а не в движущееся событие.
...
- **ADD-LIST-15 — Desktop adaptive matrix V16.** Playwright проверяет 1366×768, 1440×900, 1536×864 (FHD@125% CSS viewport) и 1920×1080. `6762`/`6867` остаются reviewed wide фото без полей; `3794` имеет rendered media ровно 300×174 и не содержит `bb778...`; protected controls `6899`, `6876`, `6893`, `3262`, `5204`, `6732`, `6546`, `6849`, `6944` не входят в `visual-crop` и имеют vertical retention ≥0.8. Меряются 52…60px quiet medallions без universal border/shadow, unified tail ≤64px, no-overflow cities, sticky stack, Saturday/Sunday heads и counts после city filter. Row count при 1920 не может ухудшаться только из-за viewport-coupled media height.
- **ADD-LIST-16 — Popular long-list flow.** `/populyarnoe/` рендерит общие `ListingEventCard@6` в intrinsic flex-wrap stream. Длинный список переносится вниз, `document.scrollWidth === viewport`, и нет carousel-only тупика или `Смотреть все` для продолжения. Категорийные shelves не добавляются без управляемой taxonomy coverage; порядок одного потока остаётся score/relevance order.
- **ADD-LIST-17 — Identity/social-proof rail V16.** External listing-ready identity/Free medallions идут identity-first в одном `60px` side rail: максимум два `52px` при ненулевом proof и максимум три без proof, без добавленной рамки/тени, с quiet saturation/opacity и `scrollHeight === clientHeight`. Safe wide classified photo может иметь один overlay medallion; true no-image fallback может поместить его внутрь нейтрального media field. Calendar, Share button и Like button отсутствуют. Ненулевые Share/Like рендерятся в shared-system порядке снизу того же rail как статичная muted icon+count ссылка на detail; каждый нулевой metric отсутствует в DOM, а при всех нулях proof полностью схлопывается. Личный liked-state только выделяет уже существующий Like proof.
...
- **ADD-LIST-19 — No preview fields.** Для каждой natural listing card Playwright сравнивает фактический ratio `.ke-listing-card__media` с `data-listing-source-width/height`, допуск ≤0.012. Низкоразрешённые controls `3794` (300×174), `3795` (300×199) и `4785` (300×225) используют собственную source height; `contain` в несовпадающей рамке, UI side/top fields и hardcoded 174px для всех low-res media блокируют preview.
- **ADD-LIST-20 — Weekend complete sticky context.** Marker top равен `site header + measured discovery + Weekend day heads + 12px` (±2px) и не перекрывается при обычном scroll. Прямая загрузка `#weekend-time-17:00` после `document.fonts.ready` и double-rAF ставит строку в тот же safe top; верх media не обрезан.
...
- **ADD-LIST-23 — Weekend range cloud.** Над и после расписания присутствуют selected/current + пять будущих weekend ranges в умном русском диапазоне. Текущий range имеет нецветовую selected-state рамку/weight; только terminal item содержит локальный CC0 `svgrepo-450220-long-arrow-right.svg`. Ссылки ведут на существующие статические routes.
...
- **ADD-LIST-25 — Qualified personalization preview.** Normal preview без compatible profile показывает полный список без декоративного switch. Только preview/localhost `?personalization=qualified` создаёт контролируемую реальную разницу, показывает `Для меня / Полный список`, оставляет full default и после переключения пересчитывает все city/daypart/exact/day counts. Production URL/query не получает фикстуру.
- **ADD-LIST-26 — Order-preserving smart packing.** DOM/visual/relevance order совпадают на всех ширинах; никакая permutation не применяется ради случайного сокращения строки. Глобальный listing container растёт до `1600px`, finite media tokens не зависят от viewport height; документ не переполняется по X и расширение 1536→1920 не увеличивает число visual rows.
