...
- **ADD-LIST-08 — Семантический выбор медиа V14.** Wide candidate выигрывает внутри одного source inventory (control `6875`: 1280×960 вместо 750×1000). OCR сохраняет natural ratio; source-reviewed no-OCR может adaptive 3:2/4:3, а большой pending no-OCR portrait — только консервативный square floor. Ширина 180px без source-manifest replacement не upscale-ится; `no_event_relevance` (control `6904`) не может вернуться generic fallback-ом. `3794` не содержит event-id override и не рендерит V13 `bb778...` 1024×683 asset; пока general source adapter не записал manifest, действует общий quality fallback. Source-page manifest lookup работает по устойчивому `source_url`, а не по event ID или меняющейся строке thumbnail URL. Skeleton занимает финальную рамку до load/decode.
...
- **ADD-LIST-13 — Header/sticky stack geometry.** На desktop opaque sticky header имеет 57px и точно переиспользует site header; `240×88` brand tag выступает ниже, но sticky city rail 56px начинает чипы правее tag. Сразу под ним full-bleed graphite time rail 56px; его nav прижата вправо. После scroll `elementFromPoint` во всём sticky stack попадает в header/tag/city/time controls, а не в движущееся событие.
...
- **ADD-LIST-15 — Desktop adaptive matrix V14.** Playwright проверяет 1366×768, 1440×900, 1536×864 (FHD@125% CSS viewport) и 1920×1080. Для `6762` ожидается source-reviewed 1080×720 и rendered ratio 1.50±0.02; `6867` сопоставляет replacement по стабильному source page и рендерится без полей; `3794` не содержит `bb778...` или 1024px listing substitution; OCR `6869` не входит в `visual-crop`. Меряются 72px medallion, strong time, отсутствие horizontal overflow, sticky stack, Saturday/Sunday heads без repeated day chips и пересчёт counts после city filter.
- **ADD-LIST-16 — Popular long-list flow.** `/populyarnoe/` рендерит общие `ListingEventCard@3` в intrinsic flex-wrap stream. На 1920px первая строка содержит 4…5 карточек разной ширины; длинный список переносится вниз, `document.scrollWidth === viewport`, и нет carousel-only тупика или `Смотреть все` для продолжения.
- **ADD-LIST-17 — Identity/action rail.** Медальоны 72×72 берутся из всего listing-ready venue/festival inventory; OCR может иметь до трёх внешних identity/бесплатно tokens, а safe no-OCR — один overlay. Действия идут строго `Share → Like`, используют общий `Icon.astro`, имеют 44px targets и не выводят нулевую цифру.
