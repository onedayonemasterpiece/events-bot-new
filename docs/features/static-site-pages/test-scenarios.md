...
- **ADD-LIST-05 — Плотные выходные.** Десятки событий остаются на одной странице в двух дневных колонках; общий бар считает оба дня, точные ссылки подписаны `Сб/Вс`, sticky-заголовки дней не перекрывают бар, каждая колонка переносит свои карточки независимо.
- **ADD-LIST-06 — Городской intersection.** Компактный multi-select открывает реальные города и counts. Отключение городов синхронно пересчитывает общий итог, периоды, точные часы, `Начались ранее` и дневные колонки; пустая комбинация показывает reset, а неизвестный сохранённый выбор не оставляет новую страницу пустой.
...
- **ADD-LIST-08 — Семантический выбор медиа.** При нескольких подходящих афишах выигрывает wide classified identity poster `1.85–2.10`; среди фото приоритет имеют квадратные/широкие, безопасная classified event photo с focal metadata может получить crop `3:2`, а вертикальный fallback — только до минимального `4:5`; OCR/unknown/unsafe/низкое разрешение не растягиваются и не кропятся. Venue medallion появляется только на выбранной photo asset, прошедшей fail-closed gate.
