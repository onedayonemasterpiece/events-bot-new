...
- **ADD-EVENT-01 — Несколько дат/времени одной программы.** В accepted desktop Split/Continuous Editorial и mobile accepted-v8 detail альтернативы находятся внутри единственного date/time owner: `Другое время` показывает same-day ссылки в одной строке, `Другие даты N` раскрывает компактные строки дат, а переход ведёт на выбранный occurrence с его URL/ICS/reminder. Отдельная карточная секция под описанием не дублирует выбор.
