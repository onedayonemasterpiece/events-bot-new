...
- **ADD-LIST-08 — Семантический выбор медиа.** При нескольких подходящих афишах выигрывает wide classified identity poster. Ширину карточки определяет media frame: OCR сохраняет natural ratio; `visual_only` без crop evidence сохраняет известную natural geometry; safe+focal `visual_only` может расширяться в свободное место. Известные размеры не теряются из-за quality threshold: control 3794 остаётся `300:174` и никогда не растягивается в `0.8:1`. Copy не меняет flex width, title обычно ≤3 строк, meta одна строка; skeleton занимает финальную рамку до load/decode. Venue medallion разрешён только на `visual_only`, включая явно source-reviewed venue opt-in.
...
- **ADD-LIST-11 — Responsive media и быстрый scroll.** Listing card использует доступные 256/512 WebP derivatives через `srcset/sizes`, задаёт intrinsic `width/height`, первые две карточки первого видимого часа имеют eager/high priority. Остальные получают `src/srcset` только через общий near-viewport observer (`200px`) и сохраняют `<noscript>` fallback, поэтому быстрый переход не ставит цель за очередью далёких изображений. При cache-disabled 250ms/150KB/s 3794 запрашивает thumbnail, skeleton снимается после decode/error, CLS <0.05.
...
- **ADD-LIST-13 — Header geometry.** На desktop sticky bar имеет высоту 57px, `240×88` brand tag выступает ниже на 31px, внешние поля равны `clamp(24px,3vw,48px)` без 1440px cap. После scroll геометрия не меняется; listing nav начинается с clearance 88px и не перекрывает tag.
- **ADD-LIST-14 — Weekend visual scale.** На 1366/1440/1920 media height равен 148/148/~167px (допустимо до 176px), title 17px, meta 15px; нет искусственной min-height/пустых строк. При 200% text spacing selective full-title может отключиться, но controls/content не пересекаются.
