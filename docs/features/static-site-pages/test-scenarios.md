...
- **USR-06 — Добавление в календарь и избранное.** Проверить, что событие после добавления видно в едином разделе «Моё» / «Мои события» и правильно попадает в filters «В календаре» и/или «Избранное».
...
- **ADD-FAV-01 — Идемпотентное сохранение.** Повторное добавление одного occurrence не создаёт дубль durable calendar/favorite/reminder state; повторная выдача того же ICS остаётся разрешённой, но не создаёт новый save. Badge показывает distinct upcoming union только при `N > 0`.
- **ADD-FAV-02 — Полный список «Мои события».** Список открывается из общего меню на всех страницах, одним batch показывает calendar/favorite union и явно различает upcoming, перенесённые, отменённые и уже завершившиеся.
...
- **ADD-CAL-01 — Один unique calendar saver.** Пять ICS requests, reload, concurrent tabs, retry с тем же `action_id` и remove/re-add одного occurrence дают один `first_calendar_saved_at` и aggregate delta `+1`, а не `+5`.
- **ADD-CAL-02 — Честная граница ICS/import.** Прямой/no-JS ICS остаётся рабочим при ошибке save backend, но UI и метрика не утверждают, что внешний calendar app завершил импорт.
- **ADD-CAL-03 — Identity/event merge reconciliation.** Anonymous→authorized link, account merge и canonical event merge сводят дубли одного occurrence к одному owner fact и пересчитывают aggregate; другая дата/время программы остаётся отдельным save.
- **ADD-CAL-04 — Единый хаб «Мои события».** `/moi-sobytiya/` одним batch показывает union calendar-only, favorite-only и dual rows; filters `В календаре`/«Избранное» не создают второй store, а `/izbrannoe/` открывает тот же shell с favorite-filter.
- **ADD-CAL-05 — Badge и lifecycle.** `Моё` / `Мои события` badge считает только distinct upcoming union; dual state считается один раз, past/removed/transport и repeated downloads исключены, cancelled/rescheduled/merged rows видимы с честным status.
- **ADD-CAL-06 — Removal boundary.** Снятие calendar state явно говорит, что сайт не удалит запись из Apple/Google/Яндекс Календаря; favorite-only row остаётся в хабе, а historical first-save/cumulative counter не сбрасываются.
