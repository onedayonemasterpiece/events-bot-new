...
- **ADD-LIST-12 — Последняя карточка и полное название.** Canonical title целиком остаётся в DOM. Singleton получает до 420px свободного copy-space без изменения article/flex width; в multi-card ряду enhancement получает только фактическая последняя видимая карточка при tail ≥72px и available ≥280px. После filter/resize/fonts actual rows пересчитываются; >4 строк, collision или overflow откатывают enhancement. До/после x/width/flex-basis других карточек совпадают с допуском 1px.
- **ADD-LIST-13 — Header/sticky stack geometry V15.** На desktop opaque sticky site header имеет 57px и точно переиспользует общий компонент. Под ним единая light discovery surface: link-like city toggles и time/daypart navigation как две строки по 44px; на ≤1450px city row может занять 88px. На 1536×864 базовый sticky stack равен 145px и не превышает 150px; города не имеют horizontal overflow. После scroll `elementFromPoint` во всём stack попадает в header/discovery controls, а не в движущееся событие.
...
- **ADD-LIST-15 — Desktop adaptive matrix V15.** Playwright проверяет 1366×768, 1440×900, 1536×864 (FHD@125% CSS viewport) и 1920×1080. `6762`/`6867` остаются reviewed wide фото без полей; `3794` имеет rendered media ровно 300×174 и не содержит `bb778...`; protected controls `6899`, `6876`, `6893`, `3262`, `5204`, `6732`, `6546`, `6849`, `6944` не входят в `visual-crop` и имеют vertical retention ≥0.8. Меряются 52…60px quiet medallions без universal border/shadow, tail ≤104px, no-overflow cities, sticky stack, Saturday/Sunday heads и counts после city filter.
- **ADD-LIST-16 — Popular long-list flow.** `/populyarnoe/` рендерит общие `ListingEventCard@5` в intrinsic flex-wrap stream. На 1920px первая строка содержит 4…5 карточек разной ширины; длинный список переносится вниз, `document.scrollWidth === viewport`, и нет carousel-only тупика или `Смотреть все` для продолжения.
- **ADD-LIST-17 — Identity/social-proof rail V15.** External listing-ready identity/Free medallions идут identity-first, максимум три, `52…60px`, без добавленной рамки/тени и с quiet saturation/opacity. Safe wide no-OCR может иметь один overlay medallion; он также не получает универсальную рамку поверх собственного authored ring. Calendar, Share button и Like button отсутствуют. Ненулевые Share/Like рендерятся в shared-system порядке как статичная muted icon+count ссылка на detail; каждый нулевой metric отсутствует в DOM, а при обоих нулях rail полностью схлопывается. Личный liked-state только выделяет уже существующий Like proof.
