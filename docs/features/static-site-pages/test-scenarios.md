...
- **ADD-LIST-16 — Behavioral Popular V21.** `/populyarnoe/` рендерит `PopularBehaviorRows@3` и общий `EventCard@3` в readonly `listing-proof`, но не `ListingEventCard` и не deterministic event-type filter. Должно быть 4–5 полок в приоритетном порядке `fast_growth` → `multi_source` (если ≥3) → `discussed` → `frequently_shared` → fallback, максимум пять карточек в одной desktop-строке. Потребительские названия первых двух полок точны: `Быстро набирают популярность` и `Встречается во множестве источников`. `frequently_shared` ранжируется по реальному `shares_count`, поэтому real-data control `5130` с 69 shares входит без ID override. Ни event id, ни normalized title+type+venue/city family не повторяются между полками; один EventCard article соответствует одному ID. Общий `PersonalFeedSlot` на этой странице запрещён: будущие `Вам может быть интересно` и anti-bubble `Откройте новое` допускаются только единым fail-closed experiment wrapper с consented совместимым профилем, page-wide family exclusion и достаточным числом кандидатов для обеих полок.
...
- **ADD-LIST-28 — Mobile Popular real EventCard dual density V21.** На `360×800`, `390×844`, `430×932` Popular использует один ordered DOM реальных `EventCard` и `ListingMobileDensitySwitch@2`. Default `Крупно` оставляет общий компонент `Смотрите дальше` в горизонтальной shelf шириной `100vw - 58px` (302/332/372px ±2). `2 в ряд` превращает те же узлы в row-major grid `repeat(2,minmax(0,1fr))` с gap 12px, без masonry/dense/reorder и горизонтального scroll. IDs, counts, family allocation и ranking до/после совпадают; preference восстанавливается из `ke_listing_density_v1`, текущая видимая карточка сохраняет vertical anchor с допуском 16px. OCR/document сохраняет natural ratio без contain fields; photo cover не применяется к document. На Popular нет Calendar, `Не интересно` или mutable Share/Like CTA, только ненулевой muted proof. Radio group имеет 48px targets, roving tabindex и Arrow/Home/End; dock учитывает safe area/footer, viewport meta не запрещает native zoom и pinch не перехватывается.
