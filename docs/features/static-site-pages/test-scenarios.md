...
- **ADD-LIST-13 — Одна discovery plane V17.** На desktop opaque sticky site header имеет 57px и переиспользует общий компонент. Под ним cities и time/daypart находятся в одной light grid-plane, не в двух последовательных подшапках. Expanded city links: font ≥15px, target ≥52px, gap 16px; pinned: font ≤12.5px, target 44px, gap 8px, counts скрыты. Фактические wrap-строки измеряются runtime, без dropdown/horizontal scroll/overflow; sticky offsets обновляются после смены измеренной геометрии.
...
- **ADD-LIST-16 — Popular long-list + exact types V17.** `/populyarnoe/` рендерит общие `ListingEventCard@7` в continuous intrinsic flex-wrap stream: без carousel-only тупика и `Смотреть все`. Exact normalized `event_type` с достаточным count доступен как фильтр, не duplicate shelves; результат является подпоследовательностью исходного score/relevance order, а city facets пересчитываются.
- **ADD-LIST-17 — Identity/social-proof rail V17.** До трёх listing-ready identity/Free medallions сохраняются и при proof. При трёх identity + proof используется split rail `56px + 36px`, без overflow/рамки/тени; event `6811` обязан показывать три identity и Share/Like. Overlay разрешён системно только selected `classified event_photo`, confidence ≥0.9, `visual_only`, `safe_crop`, focal, ratio ≥1.2; genuine no-media fallback может содержать identity. OCR/unknown/rejected raw media fail closed. Calendar/Share/Like buttons отсутствуют; только ненулевые Share/Like — muted detail links, нули не занимают DOM/ширину.
...
- **ADD-LIST-26 — Stable within-hour packing V17.** Только внутри одного day + exact time допускается deterministic greedy permutation, если baseline `530px` lane действительно получает меньше строк. Она не пересекает day/time и не меняется с viewport. Control Saturday 12:00 ≤2 rows на 1366/1536; Sunday 11:00 при 1920 не хуже 1536. Listing container растёт до `1720px`, без X overflow.
