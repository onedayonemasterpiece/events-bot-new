...
- **ADD-EVENT-01 — Несколько дат/времени одной программы.** В mobile accepted-v8 сохраняется сильная текущая строка, `Другое время` показывает same-day текстовые ссылки, а `Другие даты` — всегда открытый компактный список без dropdown/`details` и без визуально тяжёлых pills. Desktop Split/Continuous Editorial не выводит матрицу до H1: сверху остаются текущий слот, same-day ссылки и якорь `Другие даты (N) ↓` на полный нижний `Перед посещением → Когда`. Текущий слот не является ссылкой и имеет `aria-current="page"`, альтернативы ведут на occurrence с его URL/ICS/reminder. Desktop пишет день недели полностью. Проверить минимум 3 времени в один день + другие даты, неизменную видимость H1 и отсутствие горизонтального скролла.
