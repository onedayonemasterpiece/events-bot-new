...
- **ADD-LIST-08 — Семантический выбор медиа.** При нескольких подходящих афишах выигрывает wide classified identity poster. Ширину карточки определяет media frame: OCR сохраняет natural ratio; `visual_only` без crop evidence сохраняет известную natural geometry; safe+focal `visual_only` может расширяться в свободное место. Source-reviewed 3:2 crop не кропается повторно шире 1.5. Если canonical calendar thumbnail физически меньше финального desktop frame, допустим только source-grounded listing override из официального материала того же события/программы: control 3794 использует 1024×683 фото из официальной статьи об XI Ассамблее, а detail/canonical media не меняются. Copy не меняет flex width, title обычно ≤3 строк, meta одна строка; skeleton занимает финальную рамку до load/decode. Один 56px medallion разрешён только на выбранном `visual_only`, через `listing_ready` + structured venue/festival binding.
...
- **ADD-LIST-13 — Header geometry.** На desktop opaque sticky bar имеет высоту 57px, `240×88` brand tag выступает ниже на 31px, а full-bleed graphite ListingTimeNav начинается строго на y=57 и имеет 56px. После scroll `elementFromPoint` во всём диапазоне y=57…113 попадает в rail/tag, а не в движущееся событие; nav/content прижаты вправо и не пересекают tag.
...
- **ADD-LIST-15 — Desktop adaptive matrix V13.** Playwright проверяет 1366×768@1, 1440×900@1, 1536×768@1.25, 1536×864@1 и 1920×1080@1 (DPR2 и DPR3 supplemental). Для 6762 ожидается source-reviewed 1080×720, rendered ratio 1.50±0.02 и без upscale; для 6867 ratio 1.33…1.50 без полей; 3794 использует official-article 1024×683 listing still и 1.50±0.02; OCR 6869 не входит в `visual-crop`. Дополнительно проверяются 56px medallion, 36px time, отсутствие horizontal overflow, sticky gap и пересчёт Сб/Вс chips после city filter.
