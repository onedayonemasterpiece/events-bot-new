...
Event-detail MVP после описания нейтрально показывает, как приехать к событию и вернуться: электрички для Светлогорска/Зеленоградска и автобусный пример для Сказочного Холмогорья в Романово. Поезда можно добавить в календарь с напоминанием; прибытие допускается за 20–90 минут. При неизвестном окончании длительность не выдумывается: блок показывает последний поезд дня, наличие ночного рейса и первый рейс следующего дня. Автобусный пример использует однослойные чипы времени, общую проверенную оценку коридора через Северный, расчёт 20–90 минут до начала для отсечения слишком ранних рейсов, минимум 1 ч 15 мин на площадке плюс пеший путь для обратных рейсов, крупную иконку без рамки и отдельные square/portrait-карты предпочтительного маршрута; на desktop оба расписания и карта собраны в компактную трёхколоночную композицию, на mobile остаются вертикальными. Карточка каждой электрички целиком добавляет рейс в календарь и не повторяет маршрут. Публичный блок не называется партнёрским и упоминает перевозчика только одной строкой внизу. Полный контракт и tech debt обновления: [event-transport-schedule.md](event-transport-schedule.md); справочник 14 активных автобусных направлений и 21 venue-to-stop привязки: [bus-transport-directory.md](bus-transport-directory.md).
