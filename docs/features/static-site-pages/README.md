...
The separate `/lab/event-desktop/` desktop-only choice surface now compares Editorial Slab, Split Canvas, Gallery Exhibition and gated Typographic Lead under **two explicit media contracts**: no-OCR imagery uses assertive `cover`, while OCR imagery follows source ratio and may use measured safe-cover only when source-area loss is `<=20%`. The lab uses real event specimens, scales against viewport width and height, preserves the previous six-option preview, and does not change the accepted production mobile overlap. Canonical current document: `event-desktop-media-families-2026-07-12.md`; preserved prior round: `event-desktop-layout-lab-2026-07-11.md`.
