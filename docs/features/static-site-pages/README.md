...
`site/public/favicon.svg` is now a transparent `64×64` SVG containing only the `#98401f` hanging-tag silhouette and an optically open white wide `о`. It replaces the earlier graphite/terracotta PK favicon on static pages and is installed with `sizes="any"`. The full normative contract lives in [`design-system/`](design-system/README.md); `/lab/design-system/` renders both lockups, browser-tab placement and `16/32/64/128px` favicon checks.
