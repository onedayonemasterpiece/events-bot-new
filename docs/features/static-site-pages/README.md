...
The separate `/lab/event-desktop/` desktop-only choice surface compares Editorial Slab, Split Canvas, Gallery Exhibition and gated Typographic Lead under **two explicit media contracts**: no-OCR imagery uses assertive `cover`, while OCR imagery follows source ratio and may use measured safe-cover only when source-area loss is `<=20%`. The 2026-07-12 full-flow round adds a fresh `326`-event production-snapshot audit, quantifies the portrait-primary/landscape-alternative and resolution-constrained populations, records Gemini 3.1 Pro critique, and provides four separate real-event pages that continue through description, planned comment-feedback, manual multi-image gallery, practical/transport slots and `Смотрите дальше`. Desktop parallax is bounded and reduced-motion-safe; production mobile remains unchanged. Canonical current document: `event-desktop-media-families-2026-07-12.md`; preserved prior round: `event-desktop-layout-lab-2026-07-11.md`.
