...
The event hero must expose a strong first-screen date block: long weekday, human date and time are grouped in semantic `time.event-hero__date-card` before the CTA row, with the venue/place rendered as a separate meta line. The current production pattern remains the split-badge control and must stay available for rollback. A 2026-07-04 Pinterest + Gemini Pro + a-opus research pass first selected **P13 date square + companion facts rail** for isolated date testing in `/lab/date-block/`; after product feedback, the active design question moved to the whole below-hero decision space. `/lab/event-decision-block/` now compares P03/P01/P04/baseline/zero-medallion/utility-stack variants and records the consultant consensus: production should test a **P03-F hybrid** (editorial date + one wide primary CTA + quiet utility row + muted/overflow-safe medallions). Canonical research notes: `docs/features/static-site-pages/date-block-design-research-2026-07-04.md` and `docs/features/static-site-pages/event-decision-block-lab-2026-07-04.md`.
