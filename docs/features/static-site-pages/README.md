...
### Brand system / root announcement landing

The approved static-site identity is the solid terracotta tag `#98401f` with a one-colour white full-name lockup. `Полюбить Калининград` remains the umbrella endorsement and the service word `Анонсы` uses the outlined wide-`о` lettering. Desktop uses the stable `240×88px` horizontal lockup; the production mobile drawer handle uses an optically separate `128×96px` vertical lockup with an intentional two-line endorsement. `site/src/components/brand/AnnouncementsLockup.astro` keeps the content relationship shared instead of duplicating it across headers.

The favicon is in an explicit R5 choice round between the transparent `#98401f` hanging tag with a bottom-weighted white wide `о` and the same bare tag without a letter. Candidate A temporarily remains installed as `site/public/favicon.svg` with `sizes="any"`; this is a practical preview default, not a final selection. The full normative contract lives in [`design-system/`](design-system/README.md); `/lab/design-system/` compares both marks at `16/32/64/128px`, while `/lab/design-system/favicon/tag-o/` and `/lab/design-system/favicon/tag-only/` install each candidate in its own browser tab.

### Информационные партнёры

Сервисная institutional-страница `/partners/` добавлена как статическая preview-страница для статуса «информационный партнёр». Она не меняет event-detail модель: стартовый список партнёров хранится отдельно в `site/src/data/info-partners.ts`, рендерится в `site/src/pages/partners/index.astro`, попадает в `sitemap.xml` и доступен из footer-навигации как `Инфопартнёры`. Страница должна оставаться компактной плоской logo-first доской: на mobile — две плотные колонки без больших prose-карточек, на tablet/desktop — aspect-aware spans для широких и вертикальных логотипов; подпись под логотипом остаётся одной короткой партнёрской caption-строкой, а не категорией/типом и не отдельным текстовым блоком. Текущий contract запрещает публичные категории вроде `Пригородные маршруты`/`Лекции и музыка`, неверный label `КППК / РЖД`, тяжёлые card borders/backgrounds/shadows и декоративные highlight-плашки. Внешний переход — вся плитка партнёра с `rel="nofollow noopener noreferrer"`; отдельный CTA `Сайт партнёра` не рендерится. Каноника фичи: `docs/features/info-partners/README.md`.
...
Desktop header choice is isolated in the noindex `/lab/header-desktop/` surface and does not change the accepted mobile drawer tag or production header yet. The lab compares a text-only tag, a compact logo eyelet and a combined signature tab on the real `split-low-resolution` hero; the current focused text-tag uses a fixed `240×88px` solid lockup with a one-line lighter umbrella endorsement and the wide-`о` service wordmark, while all menus share a wider `1440px` container and a stable right edge. Event-detail examples deliberately have no current section; `/lab/header-desktop/listing/` demonstrates the exact-match `Выставки` active state. Canonical comparison, official lockup research and Gemini Pro evidence: `desktop-header-concepts-2026-07-14.md`.

The service word `Анонсы` has a separate noindex construction surface at `/lab/header-lettering/`. Its approved UI optical master uses one expanded `о`, corrected weight and manual pair spacing, while preserving the original user-supplied concept package under `assets/announcements-lettering-wide-o/`. The runtime SVG is one path, transform-free and currentColor-driven; the R2 construction geometry and reviews are documented in `announcements-lettering-wide-o-2026-07-14.md`, while production use is governed by `design-system/brand-lockups.md`. The same lettering is now present in desktop and mobile tags; favicon candidate A uses a separately opened, lower-set small-size `о`, while candidate B intentionally uses no letter.
...
The event hero must expose a strong first-screen date block while preserving the approved parallax hero → overlapping decision-sheet contract, with H1 inside the sheet and large event medallions in a separate horizontal shelf. The current production pattern remains available for rollback. The active `/lab/event-decision-block/` V3 comparison keeps the V2 full-width date as control and tests four quieter layouts: **Compact Date Tile**, **Calm Date Bar**, **Split Date / Free Band** and **Ticket Cluster**. Active variants replace long calendar/share/like labels with accessible icon+truthful-count controls, replace generic `по билетам` with a known price/range or explicit `Бесплатно`, and show venue plus exact address. Pinterest (120 reviewed references), Gemini 3.1 Pro and a-opus converged on **V3-B / Calm Date Bar** as production-first and **V3-A / Compact Date Tile** as the A/B challenger. Canonical research notes: `docs/features/static-site-pages/date-block-design-research-2026-07-04.md` and `docs/features/static-site-pages/event-decision-block-lab-2026-07-04.md`. After the 2026-07-11 product review, A and D are the active shortlist; D tests personal one-at-a-time contextual onboarding for bare icon+count controls. The target calendar action combines `.ics`, private «Мои события» save and consented email follow, but the current branch still provides `.ics` only; the exact status, copy, state routing and rollout gates are in `docs/features/static-site-pages/event-action-onboarding.md`.
