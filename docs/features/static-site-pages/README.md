...
The approved R6 favicon is a transparent `#98401f` hanging tag with one bottom-weighted white wide `о`, installed as `site/public/favicon.svg` with `sizes="any"`. Its final optical centre `y=36` remains below the artboard centre while the `24…48` outer bounds align to whole pixels at a `16px` render. Product review rejected both the older multi-part `ПК` monogram as noisy and the bare tag as too generic. The full normative contract lives in [`design-system/`](design-system/README.md); `/lab/design-system/` keeps the final mark and rejected control at `16/32/64/128px`, while `/lab/design-system/favicon/tag-o/` installs the final favicon in its own browser tab.
