...
- Control event `5878`: <https://kenigevents.ru/preview-20260627-event-pages-v36/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control ICS: <https://kenigevents.ru/preview-20260627-event-pages-v36/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
...
Preview `v36` adds a temporary event-page button `Поделиться эксперимент` in the hero action block. It tests the practical maximum available from a mobile browser:

1. try `navigator.share({ files, text, url })` with the event image as a file;
2. if the browser/share target rejects files, fall back to `navigator.share({ text, url })`;
3. if native share is unavailable, copy text + URL.

The reliable production mechanism remains the event URL with strong Open Graph metadata (`og:title`, `og:description`, `og:url`, `og:image`, `og:image:secure_url`, `og:image:type`, optional width/height). Telegram/VK can render a rich preview from the URL, but a browser cannot guarantee that every target app will accept a prepared “image + caption + link” post. Current Yandex Object Storage images do not expose browser CORS headers, so file sharing may fall back to link sharing until CDN/CORS or same-origin share assets are configured.
...
- `Пушкинская карта` keeps the label and renders only a green check mark as the value when available; do not write `возможна`/`доступна` as visible value copy.
- `Вход` uses a compact value chip. Avoid bare `Бесплатно`; prefer specific labels: `Свободный`, `Бесплатно · регистрация`, `Бесплатно · по записи`, `За донат`, `Запись по телефону`, `Билеты`, or the real price range.
...
For events with multiple images, MVP must **not** auto-rotate the first-paint hero by default. The event page chooses one deterministic best hero image for first paint. A tap/click on the hero opens an explicit immersive fullscreen gallery; only inside this opened viewer `visual_only` photos may run a controlled auto-forward pan/advance sequence. Current v36 state machine: forward pan is `38% → 64%` object-position (slower/farther right-to-left visual motion), auto-advances by timer before the pan fully stops with `gallery-pan-forward` as a fallback, manual backward swipe/click sets `autoAdvance=false` and runs only the local reverse pan, and `prefers-reduced-motion` remains a hard guard. The final gallery slide can be an explicit CTA card (`Купить билет`, `В календарь`, `Смотреть похожее`), visibly labeled as a CTA, not disguised as another image. Pull-to-expand/top overscroll hero behavior is P1/P2 lab-only because it can conflict with browser pull-to-refresh and mobile address-bar gestures.
