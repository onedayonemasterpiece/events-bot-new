...
The event hero must expose a strong first-screen date block: long weekday, human date and time are grouped in semantic `time.event-hero__date-card` before the CTA row, with the venue/place rendered as a separate meta line. The accepted production pattern is the Gemini Pro split-badge variant: large terracotta date, high-contrast weekday/time column and a clear divider on a white surface. The a-opus one-line strip variant is kept for comparison in `/lab/date-block/`. This block is intentionally more prominent than ordinary metadata so the user can scan “when” before reading the description.
