...
The event hero must expose a strong first-screen date block: long weekday, human date and time are grouped in semantic `time.event-hero__date-card` before the CTA row, with the venue/place rendered as a separate meta line. The current production pattern remains the split-badge control and must stay available for rollback. A 2026-07-04 Pinterest + Gemini Pro + a-opus research pass first selected **P13 date square + companion facts rail** for isolated date testing in `/lab/date-block/`; after product feedback, the active design question moved to the whole approved hero-overlap decision space. `/lab/event-decision-block/` now treats the old P03/P01/P04/baseline/zero/utility A–F set as superseded markers and compares constrained V2 variants instead: H1 inside the overlapping sheet, strong date, all buy/calendar/share/like actions visible with icons, and large fast-readable medallions that never shrink to chips. The current production-first candidate is **V1 / Date Card Above CTA** with a large medallion shelf immediately after the sheet; V2/V3 remain identity-heavy probes. Canonical research notes: `docs/features/static-site-pages/date-block-design-research-2026-07-04.md` and `docs/features/static-site-pages/event-decision-block-lab-2026-07-04.md`.
