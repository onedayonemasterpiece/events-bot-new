...
Preview `v38` adds two temporary event-page Web Share buttons in the hero action block. They test two real mobile-browser payload shapes:

1. `Поделиться эксперимент`: `navigator.share({ files, text, url })` — image file plus text and a separate URL field;
2. `Поделиться эксперимент 2`: `navigator.share({ files, text })`, where `text` already contains the event URL — this checks caption-style delivery for targets that attach the whole text to the shared media;
3. if the event image is not fetchable as a browser `Blob` (for example because Object Storage/CDN CORS is absent), generate a same-page 1080×1350 PNG share-card from the event title/date/place/admission/URL and repeat the selected payload shape;
4. if the browser/share target rejects files, fall back to URL/text share or copy text.

The reliable production mechanism remains the event URL with strong Open Graph metadata (`og:title`, `og:description`, `og:url`, `og:image`, `og:image:secure_url`, `og:image:type`, optional width/height). Telegram/VK/Max can render a rich preview from the URL, but a browser cannot guarantee that every target app will accept a prepared “image + caption + link” post in the same way. The observed v38 experiment where Telegram, VK and Max accepted an image is recorded as useful evidence, not a guarantee.
...
- keep at least two formats: `1200×630` for Open Graph and `1080×1350` for Web Share/media-post, with optional `1080×1920` story/poster later;
- include brand, event title, date/time, venue/city, admission/CTA and a short URL/canonical URL;
...
For events with multiple images, MVP must **not** auto-rotate the first-paint hero by default. The event page chooses one deterministic best hero image for first paint. A tap/click on the hero opens an explicit immersive fullscreen gallery; only inside this opened viewer `visual_only` photos may run a controlled auto-forward pan/advance sequence. Current v38 state machine: forward pan is `38% → 64%` object-position (slower/farther right-to-left visual motion), auto-advances by timer before the pan fully stops with `gallery-pan-forward` as a fallback, manual backward swipe/click sets `autoAdvance=false` and runs only the local reverse pan, and `prefers-reduced-motion` remains a hard guard. The final gallery slide can be an explicit CTA card (`Купить билет`, `В календарь`, `Смотреть похожее`), visibly labeled as a CTA, not disguised as another image. Pull-to-expand/top overscroll hero behavior is P1/P2 lab-only because it can conflict with browser pull-to-refresh and mobile address-bar gestures.
