...
The static announcement site uses the warm two-color brand mark: graphite `#2d3035` and terracotta `#af481f`. The drawing follows the T-000053 SVG redraw shape (left monogram, `К`, heart tile, right tile and signal bars) rather than the earlier rejected calendar/church silhouette. `site/public/favicon.svg` now uses the same transparent mark with a tightly cropped `viewBox`, no cream/white backing plate and `sizes="any"`, so Chrome can render the sign optically as large as peer favicons; `site/public/brand-mark.svg` remains the reusable transparent page mark. Both assets are hand-authored SVG paths with no embedded raster images, and `site/scripts/check-preview.mjs` guards the crop, expected ids/colors and absence of a background plate. The 2026-06-29 bucket-root logo publication predates this transparent favicon correction; root promotion remains a separate release step after preview acceptance.
