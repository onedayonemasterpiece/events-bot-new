...
The separate `/lab/event-desktop/` desktop-only choice surface compares Editorial Slab, Split Canvas, Gallery Exhibition and gated Typographic Lead under **two explicit media contracts**: no-OCR imagery uses assertive `cover`, while OCR imagery follows source ratio and may use measured safe-cover only when source-area loss is `<=20%`. The clean v2 review provides six real-event URLs (photo/OCR for Editorial, Split and Gallery) that preserve the accepted geometry and continue through description, a clean consolidated-feedback empty state, practical/transport information and production `split-actions` cards in `Смотрите дальше`. Multiple images open in the production fullscreen viewer; there is no lower gallery. Photo parallax is clipped to the hero, OCR parallax is disabled, the desktop header is sticky, and the entire surface is hidden below `1024px`. The rejected technical full-flow preview remains historical evidence only. Production mobile components/styles remain unchanged. Canonical current document: `event-desktop-media-families-2026-07-12.md`; preserved prior round: `event-desktop-layout-lab-2026-07-11.md`.
