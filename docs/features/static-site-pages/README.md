...
- Preview index: <https://kenigevents.ru/preview-20260627-event-pages-v39/__preview/>
- Today listing: <https://kenigevents.ru/preview-20260627-event-pages-v39/segodnya/>
- Tomorrow listing: <https://kenigevents.ru/preview-20260627-event-pages-v39/zavtra/>
- Weekend listing: <https://kenigevents.ru/preview-20260627-event-pages-v39/vyhodnye/>
- Control event `5878`: <https://kenigevents.ru/preview-20260627-event-pages-v39/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control ICS: <https://kenigevents.ru/preview-20260627-event-pages-v39/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Control discovery JSON: <https://kenigevents.ru/preview-20260627-event-pages-v39/data/discovery/5878.json>
- Hero composition lab: <https://kenigevents.ru/preview-20260627-event-pages-v39/lab/hero/>
- Hero viewport review: <https://kenigevents.ru/preview-20260627-event-pages-v39/lab/hero/review/>
- Sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v39/sitemap.xml>
- Robots: <https://kenigevents.ru/preview-20260627-event-pages-v39/robots.txt>
- Website endpoint fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v39/__preview/>
...
Preview `v39` keeps the v37/v38 corrections and fixes the share-experiment wording/behavior after the mobile messenger test: Web Share payloads are treated as plain text/file/url only, so `Поделиться эксперимент 2` now explicitly tests a markdown-style link in the caption (`[Открыть событие](URL)`) without a separate `url` field, while the new `Скопировать HTML-пост` button writes a rich `text/html` + `text/plain` clipboard payload with a real `<a href>` link for manual paste testing. This does **not** promise formatted hyperlinks through the system share sheet; it separates the Web Share media-post test from the rich-clipboard/admin-post direction.


Preview fixture note: production row `5370` («Точка и линия») currently has a false-free state (`is_free=1`, `ticket_status=бесплатно по регистрации`) because a free curator round-table source was merged into the long-running paid exhibition. The v39 fixture intentionally overrides the preview event to `ticket / Билеты / is_free=false` and attaches five real exhibition photos for gallery testing; production source-of-truth repair remains a separate event-quality task under the existing false-free incident family.
...
Preview `v39` has three temporary event-page share lab controls in the hero action block:

1. `Поделиться эксперимент`: `navigator.share({ files, text, url })` — image file plus plain text and a separate URL field;
2. `Поделиться эксперимент 2`: `navigator.share({ files, text })`, where `text` contains markdown-style `[Открыть событие](URL)` and no separate `url` field — this is a proof test, not a guarantee that Telegram/VK/Max will parse markdown;
3. `Скопировать HTML-пост`: writes `ClipboardItem({ 'text/html', 'text/plain' })`, where the HTML variant contains `<a href="...">Открыть событие</a>`, so manual paste can test whether a target app preserves a formatted hyperlink from the clipboard.

Important limit: Web Share API does not expose a rich-text/HTML payload field and cannot force a messenger to render a hidden hyperlink under arbitrary caption text. The reliable production mechanism remains the event URL with strong Open Graph metadata (`og:title`, `og:description`, `og:url`, `og:image`, `og:image:secure_url`, `og:image:type`, optional width/height). Telegram/VK/Max can render a rich preview from the URL, but a browser cannot guarantee that every target app will accept a prepared “image + formatted caption + hidden link” post in the same way. The observed v39/v38 experiment where Telegram, VK and Max accepted an image is recorded as useful evidence, not a guarantee.
...
For events with multiple images, MVP must **not** auto-rotate the first-paint hero by default. The event page chooses one deterministic best hero image for first paint. A tap/click on the hero opens an explicit immersive fullscreen gallery; only inside this opened viewer `visual_only` photos may run a controlled auto-forward pan/advance sequence. Current v39 state machine: forward pan is `38% → 64%` object-position (slower/farther right-to-left visual motion), auto-advances by timer before the pan fully stops with `gallery-pan-forward` as a fallback, manual backward swipe/click sets `autoAdvance=false` and runs only the local reverse pan, and `prefers-reduced-motion` remains a hard guard. The final gallery slide can be an explicit CTA card (`Купить билет`, `В календарь`, `Смотреть похожее`), visibly labeled as a CTA, not disguised as another image. Pull-to-expand/top overscroll hero behavior is P1/P2 lab-only because it can conflict with browser pull-to-refresh and mobile address-bar gestures.
