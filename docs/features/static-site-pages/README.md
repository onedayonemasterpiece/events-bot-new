...
The event hero must expose a strong first-screen date block: long weekday, human date and time are grouped in semantic `time.event-hero__date-card` before the CTA row, with the venue/place rendered as a separate meta line. The current production pattern remains the Gemini Pro split-badge variant: large terracotta date, high-contrast weekday/time column and a clear divider on a white surface. A 2026-07-04 Pinterest + Gemini Pro + a-opus research pass selected the next candidate for testing: **P13 date square + companion facts rail**, where the square anchors weekday/day/month and the right side carries time, venue, admission and format. The lab page `/lab/date-block/` now keeps the current split-badge as control and renders the model-selected alternatives. Canonical research note: `docs/features/static-site-pages/date-block-design-research-2026-07-04.md`.
