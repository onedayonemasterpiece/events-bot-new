...
- обычная очередь `3/19` не является visual sample. Для сравнения механики
  существует отдельный `?review=media`: **12** review-only сценариев, 12 разных
  event objects и 12 разных источников с declared width `1919–2560px`.
  Они не попадают в cooldown, automatic queue или трёхэкранную цепочку. После
  reveal каждый экран останавливается; под hero всегда видны прямые кнопки
  `1–12`, `Предыдущее`, `Повторить`, `Следующее`, поэтому любую картинку можно
  вернуть без ожидания. Review mode сохраняет exact source order: если именно
  вручную проверенный asset исчез или перестал проходить admission, resolver
  abstain, а не подбирает другой raster того же события. Mobile показывает те
  же тексты и review controls, но не назначает raster URL;
