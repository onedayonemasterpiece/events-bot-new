...
35. Calendar action links directly to `.ics` on event detail for every valid
date/date range. Multi-day ranges are all-day inclusive spans with exclusive
next-day `DTEND`. Feed/preview card utilities keep the stricter one-day rule.
...
48. `.ics` calendar link works without JS for a valid detail date/date range
and should be served as `text/calendar` with inline content disposition.
Compact feed/preview cards remain governed by their separate one-day rule.
