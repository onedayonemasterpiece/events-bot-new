...
- Approved static-site direction; the independent small mark is the matching transparent tag + lower wide-`о` at optical centre `y=36`.
...
The accepted desktop concept and mobile production drawer now share `AnnouncementsLockup.astro`; the desktop concept keeps the fixed R3 geometry while mobile uses a taller R4 optical composition rather than a mechanical resize. Production event/listing route behavior and exact current-state semantics remain unchanged. The final R6 favicon uses the shared tag silhouette with a lower small-size wide `о`; the old monogram and bare silhouette are rejected. Canonical rules have moved to [`design-system/`](design-system/README.md); this dated document remains the research and decision record.
