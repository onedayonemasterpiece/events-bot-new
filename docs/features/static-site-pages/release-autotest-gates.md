...
Adapter уже строит source-bound snapshot в существующем exporter и проверяется
в StaticSiteBuilder до Astro build. Но до owner-accepted baseline, безопасного
ChatGPT launch и terminal live run scenario остаётся `partial`: PR блокируют
adapter/runner contract regressions, но example fixture или отсутствие baseline
не выдаются за подтверждённое качество production-подборок.
