...
live-вставки, принятые mobile journeys, двухфазный Weekend desktop, отдельные
показы выставок и фестивалей, 30 резервных ручных показов страниц (15 страниц
× mobile/desktop), QR-outro и девять кадров базы шуток остаются явными scene
IDs. Универсального DSL и редактора нет.
...
поле Smart Search. Справа добавлена ручная полоса ↑/↓ для bounded wheel, а
снизу — desktop D-pad ↑/←/↓/→, который передаёт настоящие Arrow key в активный
live-frame и показывает на сцене нажатую клавишу и реакцию интерфейса. Ни одна
из этих ручных команд не завершает текущую сцену. Статический кадр остаётся
видимым до следующего Run; browser,
BrowserContext, page и окно не пересоздаются. Stop/Reset нетерминальны, только
Shutdown закрывает среду.

После owner-прогона 30 июля relay обязан сохранять доставляемость команд через
перезапуск процесса. Sequence начинается с JS-safe wall-clock значения,
публичное состояние содержит `boot_id`, а Windows-агент сбрасывает старый
cursor при смене boot или откате sequence. Поэтому первая Run/scroll/D-pad
команда после краткого Fly reconnect не требует повторных нажатий или
перезапуска `.cmd`; TTL команды равен двум минутам. Control/relay parity
проверяется автоматически для каждой run-кнопки, а не выборочным списком.

Сервисные пункты 03.2 и 03.3 являются разными сценами: 03.2 показывает
content-addressed кожаную PWA-иконку крупно и кинематично переводит её в
левую верхнюю бирку, 03.3 отдельно показывает «Найти. Поделиться. Не забыть».
03.8.4 открывает настоящую desktop-коллекцию `/artefakty/`, фокусирует
собранного янтарного космонавта и реальным Enter открывает диалог. Поиск
03.10.2 определяет авторизацию по состоянию control, а не по геометрической
видимости logout внутри закрытого account-menu; нажатие «Искать», настоящий
progress и карточки результатов должны быть видны зрителю.

Музыка intro идёт последовательной ротацией из пяти предоставленных треков:
исходный `Echo Sax End`, два трека джазового трио Александра Маслова,
`Take Five` и `Cantaloupe Island`. После последнего плейлист начинается снова;
одновременно загружается только текущий трек. Все файлы закреплены
content-addressed URL в существующем Yandex CDN. Пункт лекции 3 использует
заполненный авторский `UI and UX.png` (CDN WebP), а не временную фотографию.

Группы на пульте следуют порядку авторского документа: Интро → Лекция → Рынок
→ Презентация сервиса → Демонстрация страниц → Аутро → База шуток. Внутри
«Демонстрации страниц» каждая из 15 страниц имеет соседние кнопки mobile и
desktop. Эти резервные сцены только открывают нужную опубликованную страницу
(для пункта меню mobile открывается само меню), не запускают автопрокрутку и
оставляют управление полосе ↑/↓ и desktop D-pad.

Лекционная группа также следует авторскому порядку 1–11. В ней 12 кнопок,
поскольку пункт 6 использует два предоставленных кадра (`6.1` и `6.2`).
Пункт 3 показывает авторский `UI and UX.png` и вводит подписи UI/UX через
10 секунд после изображения; пункты 10 и 11
показывают предоставленные примеры хорошего и перегруженного UI.

Показы выставок и фестивалей используют маршруты `/vystavki/` и `/festivali/`
той же опубликованной сборки. В выставках runner естественно доходит до события
`5370` с полноценной медиаколодой и оставляет её в фокусе для D-pad. Фестивали
показываются в одном окне: сначала медленный desktop-scroll, затем mobile-scroll.

Девять шуток из авторского документа вынесены в отдельную группу пульта и в
отдельные крупнотипографические кадры с human-like печатью. Для шуток 03 и 07 автор потребовал TTS.
Текстовые кадры готовы, но генерация звука намеренно fail-closed: обязательный
shared quota limiter сейчас не проходит preflight из-за отсутствующей колонки
`google_ai_model_limits.quota_scope` (`PostgreSQL 42703`). До исправления схемы
аудио не подменяется другой моделью, ключом или локальным синтезом.

Live-вставки используют актуальный на 30 июля focus-preview
`preview-20260730-hero-talk-date-donor-r2`; маршрут нормализуется относительно
...
Итерация G заменяет поясняющие заглушки реальным UI там, где surface уже
опубликован: 03.4 открывает мобильное меню и показывает подборку,
«Избранное», «Для меня» и «Популярное»; 03.6 выполняет три лайка и один
`not_interested` в бесплатной подборке; 03.12.1–2 показывают desktop-страницу
дня и event keyboard surface с видимыми клавишами; 03.12.3.2 прокручивает
реальное событие к автобусному расписанию; 03.18 доходит до реальных кнопок
обратной связи. 03.21 нажимает Share, показывает предоставленные скриншоты
885/886, затем отдельная сцена скачивает `.ics` и открывает реальное
«Избранное».

03.13 усиливает тихий CC0 cue через Web Audio gain. Кинематическая S-кривая,
скругления контентных изображений и последовательный крупный фокус элементов
персонализации применены к подходящим сценам. После окончания видео 03.24
ролик уходит, а QR увеличивается до дальне-считываемого размера.
...
подготовка Яндекс-входа открывается во вспомогательной вкладке, не заменяя
постоянную stage-страницу. Любая новая Run закрывает зависшую/ошибочную вкладку
авторизации, проверяет stage и при необходимости восстанавливает её; Reset
остаётся явным ручным способом восстановления.
