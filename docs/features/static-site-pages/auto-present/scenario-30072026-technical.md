...
показывает изображение и только через 10 секунд вводит подписи UI/UX; пункты
...
- исходный `scenario-assets/UI and UX.png` имеет нулевой размер, поэтому для
  пункта 3 временно используется уже лицензированная фотография народной
  тропинки David Smith с требуемой 10-секундной механикой подписей; после
  появления исходного файла меняется только media URL;
