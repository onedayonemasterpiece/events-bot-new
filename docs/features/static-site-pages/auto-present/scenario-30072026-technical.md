...
Знак «Знание» сохраняется. Музыка последовательно проходит пять
content-addressed треков из `scenario-assets` — `Echo Sax End`, марш и вальс
из джазовой сюиты «Щелкунчик», `Take Five`, `Cantaloupe Island` — и после
последнего возвращается к первому. Следующий большой файл загружается только
при его включении. Stop снимает таймеры и останавливает audio.

### Лекция

Лекция — **11 авторских пунктов и 12 отдельных кнопок/сцен**: пункт 6
разложен на два предоставленных изображения. Порядок пульта точно следует
авторскому списку: 1, 2, 3, 4, 5, 6.1, 6.2, 7, 8, 9, 10, 11. Пункт 3 сначала
показывает заполненный авторский `scenario-assets/UI and UX.png` через
content-addressed CDN WebP и только через 10 секунд вводит подписи UI/UX; пункты
10–11 используют предоставленные Telegram-кадры Т-Банка, Яндекс Go и
Госуслуг, а также авторские тезисы про Wildberries, ВКонтакте и Авито.
Это не автопроигрываемая колода: каждый кадр остаётся на экране до следующей
команды.
...
переписывается генератором. Текущая итерация сверена с точным SHA-256
авторской части до технического appendix
`cf396c99a44d8d4cb759c7972a4d7b523f1fd76a568039554e2023056d0f608d`.
...
Эти два ограничения относились к iteration E и superseded фактическими
live-runner из iteration G. Public demo всё равно остаётся `NO-GO` до
финальной репетиции на целевом Windows.

### Аудит авторского порядка · iteration H

Повторная сверка актуального авторского файла выявила, что прежняя лекционная
группа была неполной и из-за пропущенного пункта 3 визуальная нумерация
пунктов 4–6 была сдвинута. Исправлен канонический порядок:

1. `lecture-01`;
2. `lecture-02`;
3. `lecture-ui-ux-path`;
4. `lecture-03`;
5. `lecture-04`;
6. `lecture-05`, затем `lecture-06` как два source-backed кадра одного пункта;
7. `lecture-convenience-emergence`;
8. `lecture-usability-measurement`;
9. `lecture-07`;
10. `lecture-good-ui`;
11. `lecture-poor-ui`.

По всем верхнеуровневым разделам порядок пульта соответствует источнику:
Интро → Лекция → Рынок конкурентов → Презентация сервиса → Демонстрация
страниц → Аутро → База шуток. Внутри презентации сервиса кнопки следуют
авторскому порядку 1–24; одинаковые номера в самом авторском тексте
разрешаются только суффиксами (`03.4.10/03.4.11`, `03.11.1/03.11.2`,
`03.21.1/03.21.2`) без перестановки смысла.

Аудит не считает весь документ полностью закрытым:

- медиа-пробел пункта 3 закрыт: заполненный `scenario-assets/UI and UX.png`
  преобразован в WebP без композиционной переработки, загружен в существующий
  CDN и используется с прежней 10-секундной механикой подписей;
- TTS для шуток 03 и 07 остаётся заблокирован fail-closed preflight общей
  квоты; текст, human-like typing и кнопки готовы, но обязательного аудио пока
  нет;
- живой умный поиск требует один раз подготовить отдельную Яндекс-сессию
  кнопкой «Подготовить поиск»; без авторизации результат намеренно не
  подделывается.
