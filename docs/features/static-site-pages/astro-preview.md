...
> **Build ID:** `preview-20260627-event-pages-v30`
> **Public preview index:** <https://kenigevents.ru/preview-20260627-event-pages-v30/__preview/>
...
- Control event page: <https://kenigevents.ru/preview-20260627-event-pages-v30/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control event calendar file: <https://kenigevents.ru/preview-20260627-event-pages-v30/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Control discovery JSON: <https://kenigevents.ru/preview-20260627-event-pages-v30/data/discovery/5878.json>
- Preview sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v30/sitemap.xml>
- Preview robots: <https://kenigevents.ru/preview-20260627-event-pages-v30/robots.txt>
...
- Yandex Object Storage website fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v30/__preview/>
...
- Preview canonical and `og:url` include `/preview-20260627-event-pages-v30/`; production canonical is not emitted by the preview build.
...
cd site
npm install
PREVIEW_BUILD_ID=preview-20260627-event-pages-v30 npm run build:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v30 npm run check:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v30 npm run deploy:preview
```
...
The first public preview (`preview-20260627-event-pages-v1`) was superseded after visual review. The current `preview-20260627-event-pages-v30` keeps the event page mobile/feed-oriented and feedback-aware, and replaces the v19 safe image block with the v20 hero composition lab from `event-hero-lab-2026-06-27.md`:
...
After direct product review, `preview-20260627-event-pages-v30` rolls back the UX regressions introduced by the split recommendation rails and adds the first static-seed/client-hydration discovery contract:
...
After consultant review, `preview-20260627-event-pages-v30` additionally hardens the first discovery layer:
...
### v30 product/UI corrections

`preview-20260627-event-pages-v30` adds the current product corrections:
...
- hero gallery has an on-image transparent `Фото N` CTA, lazy-loads gallery slides from `data-gallery-src` after open/navigation, uses cover + soft pan for visual-only fullscreen photos, keeps OCR/text images in contain, supports touch/pointer swipe left-right, and JSON-LD `image[]` lists the gallery assets for SEO/GEO;
...
- event `5370` is a documented fixture override: production currently marks the long-running exhibition «Точка и линия» free because a free curator round-table source was merged into it. The v30 fixture renders it as paid/ticketed for preview correctness, while production DB repair remains a separate source-of-truth task.

`npm run check:preview` passed for v30. Additional Playwright smoke evidence is local-only under `artifacts/codex/event-pages-v30/`: `v30-smoke-result.json`, `5370-v30-mobile.png`, `5370-v30-gallery.png`, `5370-v30-drawer.png`.

## v16/v17 personalization-contract correction + v18 UI A/B + v20 hero composition lab

`preview-20260627-event-pages-v30` keeps the discovery implementation aligned with the documented `event_detail_related` contract:
...
`npm run check:preview` passed for `preview-20260627-event-pages-v30`: the control discovery JSON is checked for `schema_version=event-detail-related-v1`, `surface=event_detail_related`, `algorithm_id=static_related_v1`, `related_static[]`, display calendar fields, source/social counter consistency, footer social links/icons and `mailto:info@kenigevents.ru`, parseable Event-class JSON-LD with ISO 8601 `offers.validFrom`, current-event like/share controls, public source/extended-stat auth-gate notice without Telegraph/source links, the split-actions feed-card baseline, the hero lab/review routes, poster/photo hero modes, explicit composition markers, one-H1 contract, monolithic flat drawer rail/no-pill links, the non-disappearing closed handle, and the fullscreen hero gallery contract with lazy `data-gallery-src` slides, visual-only cover/pan, touch/pointer swipe and final similar-event CTA slide. Playwright smoke evidence for v30 is stored locally under `artifacts/codex/event-pages-v30/` (not committed): `5370-v30-mobile.png`, `5370-v30-gallery.png`, `5370-v30-drawer.png`, and `v30-smoke-result.json`. The smoke verifies that the 5370 hero is full viewport width, source links are hidden, email is present, gallery images are lazy-hydrated one-by-one, and the drawer opens with the handle attached to the rail, the photo CTA remains inside the viewport, touch swipe changes slides in both directions, and the not-interested plate has a working undo without accidental navigation. Like/profile writes remain local-only preview behavior, not Supabase persistence.
