...
> **Build ID:** `preview-20260627-event-pages-v13`
> **Public preview index:** <https://kenigevents.ru/preview-20260627-event-pages-v13/__preview/>
...
- Control event page: <https://kenigevents.ru/preview-20260627-event-pages-v13/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control event calendar file: <https://kenigevents.ru/preview-20260627-event-pages-v13/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Preview sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v13/sitemap.xml>
- Preview robots: <https://kenigevents.ru/preview-20260627-event-pages-v13/robots.txt>
- Yandex Object Storage website fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v13/__preview/>
...
- Preview canonical and `og:url` include `/preview-20260627-event-pages-v13/`; production canonical is not emitted by the preview build.
...
cd site
npm install
PREVIEW_BUILD_ID=preview-20260627-event-pages-v13 npm run build:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v13 npm run check:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v13 npm run deploy:preview
```
...
The first public preview (`preview-20260627-event-pages-v1`) was superseded after visual review. `preview-20260627-event-pages-v13` makes the event page more mobile/feed-oriented and feedback-aware:

- recommendation cards now have large image-led feed cards instead of text-only cards;
- hero/card/listing media follows the OCR-safe v13 rule: `image_text_mode=ocr_text|unknown` renders the actual image at its natural aspect ratio with no crop, no fixed 3:4 frame, no duplicate/backdrop underlay and no blur fill; only `image_text_mode=visual_only` may cover/crop inside a strict vertical 3:4 frame;
...
After direct product review, `preview-20260627-event-pages-v13` rolls back the UX regressions introduced by the split recommendation rails:
...
- hero, card and listing media use the same v13 OCR-safe policy: `visual_only` uses `cover` in a vertical 3:4 frame; `ocr_text` and `unknown` use the image natural aspect ratio, not `contain` inside a fixed frame. Duplicate same-poster underlays, blurred fills, repeated edges and OCR crop are forbidden.
- The share action is a permanent bottom-row button with a VK-like outlined repost/share arrow adapted from `@vkontakte/icons` `Icon24ShareOutline` (MIT), visible `Поделиться` label and share count when count is positive. Zero like/share counts are not rendered as `0`. After a successful like the share action is highlighted instead of showing a floating bubble. The same bottom row contains `Не интересно`, optional calendar, share and like; the old explicit `Открыть` card button is removed because media/title links plus full-card JS navigation preserve crawlable SEO/GEO links while reducing UI noise.
...
After consultant review, `preview-20260627-event-pages-v13` additionally hardens the first discovery layer:
