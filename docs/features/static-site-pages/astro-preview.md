...
Public focus preview `preview-20260711-event-transport-compact-v5` supersedes v4. Rail matching still accepts arrivals 20–90 minutes before the event, inflects `К началу …`, avoids guessed end times and exposes factual last/night/next-day service. The route is now printed once as `Калининград-Северный ⇄ …`; every compact train row is itself the `.ics` link with a calendar icon, keyboard focus and no repeated route or separate button.

The Romanovo block now uses the AVL39/OSM-verified common corridor: all `118/118А/119` departures share one `около 1 часа` journey estimate and one `Северный — ориентировочно 10–15 минут` note. Time chips contain only the departure, the SVG Repo bus is large and unboxed, return chips include earlier departures but are filtered by `11:00–16:00` hours/walk/wait instead of running into late night, and the map exposes one preferred walking CTA plus the venue point. The longer walk from `Романовский поворот` remains disclosed in text because it is genuinely different from the central-Romanovo path.

The 400-event focus fixture built 421 HTML pages and 169 transport `.ics` files and passed `npm run check:preview`; the focused Python duration tests also passed. Public pages and transport ICS returned `200` with `text/calendar`. Desktop/mobile Playwright QA found no transport console errors or horizontal overflow; all images loaded, exactly one walking CTA rendered, the desktop explicit-end rail block fell to `398px`, and keyboard `Enter` on a whole train row fetched its ICS with `200 text/calendar`. Evidence is under ignored `artifacts/codex/site-presentation-transport/visual-v5/`. The broader presentation release keeps its separate anonymous-personalization RPC and final-browser gates. Scheduled Kaggle refresh remains a P0 blocker before production promotion.

- Real rail event `6510`, explicit `17:00–18:10` duration and two trains each way: <https://kenigevents.ru/preview-20260711-event-transport-compact-v5/sobytiya/kontsert-posvyaschenie-muslimu-magomaevu-i-anne-german-svetlogorsk-6510/>
- Real late concert `6397`, factual `22:40` last train / no night service / `06:25` first next-day train: <https://kenigevents.ru/preview-20260711-event-transport-compact-v5/sobytiya/kontsert-kaver-gruppy-diskodyadi-svetlogorsk-6397/>
- Romanovo event `6710`, compact buses and drawn walking route (date still needs organizer reconfirmation): <https://kenigevents.ru/preview-20260711-event-transport-compact-v5/sobytiya/slet-babok-ezhek-romanovo-6710/>
- Preview index: <https://kenigevents.ru/preview-20260711-event-transport-compact-v5/__preview/>
