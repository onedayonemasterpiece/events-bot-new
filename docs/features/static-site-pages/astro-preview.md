...
> **Build ID:** `preview-20260627-event-pages-v34`
> **Public preview index:** <https://kenigevents.ru/preview-20260627-event-pages-v34/__preview/>
...
- Control event page: <https://kenigevents.ru/preview-20260627-event-pages-v34/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control event calendar file: <https://kenigevents.ru/preview-20260627-event-pages-v34/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Control discovery JSON: <https://kenigevents.ru/preview-20260627-event-pages-v34/data/discovery/5878.json>
- Preview sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v34/sitemap.xml>
- Preview robots: <https://kenigevents.ru/preview-20260627-event-pages-v34/robots.txt>
...
- Yandex Object Storage website fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v34/__preview/>
...
- v34 adds property-label polish: `Пушкинская карта` renders as a green check property, `Вход` uses specific labels (`Свободный`, `Бесплатно · регистрация`, `Бесплатно · по записи`, etc.), the mobile brand tag uses cropped `brand-mark.svg`, event nav hides the user-facing Sitemap link, and the fullscreen gallery pan is slower/farther with a pre-stop auto-advance.
...
- Preview canonical and `og:url` include `/preview-20260627-event-pages-v34/`; production canonical is not emitted by the preview build.
...
cd site
npm install
PREVIEW_BUILD_ID=preview-20260627-event-pages-v34 npm run build:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v34 npm run check:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v34 npm run deploy:preview
```
...
The first public preview (`preview-20260627-event-pages-v1`) was superseded after visual review. The current `preview-20260627-event-pages-v34` keeps the event page mobile/feed-oriented and feedback-aware, and replaces the v19 safe image block with the v20 hero composition lab from `event-hero-lab-2026-06-27.md`:
...
After direct product review, `preview-20260627-event-pages-v34` rolls back the UX regressions introduced by the split recommendation rails and adds the first static-seed/client-hydration discovery contract:
...
After consultant review, `preview-20260627-event-pages-v34` additionally hardens the first discovery layer:
...
### v34 product/UI corrections

`preview-20260627-event-pages-v34` adds the current product corrections:
...
- event `5370` is a documented fixture override: production currently marks the long-running exhibition «Точка и линия» free because a free curator round-table source was merged into it. The v34 fixture renders it as paid/ticketed for preview correctness, while production DB repair remains a separate source-of-truth task.

`npm run check:preview` passed for v34. Additional Playwright smoke evidence is local-only under `artifacts/codex/event-pages-v34/`: `listing-smoke-result.json`, `segodnya-mobile.png`, `zavtra-mobile.png`, `event-mobile-brand.png`, `gallery-v34.json`.

## v16/v17 personalization-contract correction + v18 UI A/B + v20 hero composition lab

`preview-20260627-event-pages-v34` keeps the discovery implementation aligned with the documented `event_detail_related` contract:
