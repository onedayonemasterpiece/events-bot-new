...
> **Build ID:** `preview-20260627-event-pages-v35`
> **Public preview index:** <https://kenigevents.ru/preview-20260627-event-pages-v35/__preview/>
...
- Control event page: <https://kenigevents.ru/preview-20260627-event-pages-v35/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control event calendar file: <https://kenigevents.ru/preview-20260627-event-pages-v35/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Control discovery JSON: <https://kenigevents.ru/preview-20260627-event-pages-v35/data/discovery/5878.json>
- Preview sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v35/sitemap.xml>
- Preview robots: <https://kenigevents.ru/preview-20260627-event-pages-v35/robots.txt>
...
- Yandex Object Storage website fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v35/__preview/>
...
- v35 keeps property-label polish and additionally removes the rejected mobile brand icon in favour of subtle title sway, adds hidden listing personal-feed slots with localStorage/RPC preparation, and prepares CDN-aware asset URLs via `PUBLIC_ASSET_BASE_URL`.
...
- Preview canonical and `og:url` include `/preview-20260627-event-pages-v35/`; production canonical is not emitted by the preview build.
...
cd site
npm install
PREVIEW_BUILD_ID=preview-20260627-event-pages-v35 npm run build:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v35 npm run check:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v35 npm run deploy:preview
```
...
The first public preview (`preview-20260627-event-pages-v1`) was superseded after visual review. The current `preview-20260627-event-pages-v35` keeps the event page mobile/feed-oriented and feedback-aware, and replaces the v19 safe image block with the v20 hero composition lab from `event-hero-lab-2026-06-27.md`:
...
After direct product review, `preview-20260627-event-pages-v35` rolls back the UX regressions introduced by the split recommendation rails and adds the first static-seed/client-hydration discovery contract:
...
After consultant review, `preview-20260627-event-pages-v35` additionally hardens the first discovery layer:
...
### v35 product/UI corrections

`preview-20260627-event-pages-v35` adds the current product corrections:
...
- event `5370` is a documented fixture override: production currently marks the long-running exhibition «Точка и линия» free because a free curator round-table source was merged into it. The v35 fixture renders it as paid/ticketed for preview correctness, while production DB repair remains a separate source-of-truth task.

`npm run check:preview` passed for v35. Live HTTP smoke (`curl`) returned 200 for the preview index, today listing and control event page; Playwright screenshot capture was not rerun in this pass because the local Playwright browser binary is not installed.

## v16/v17 personalization-contract correction + v18 UI A/B + v20 hero composition lab

`preview-20260627-event-pages-v35` keeps the discovery implementation aligned with the documented `event_detail_related` contract:
