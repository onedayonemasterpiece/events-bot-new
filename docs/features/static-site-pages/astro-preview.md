...
- Desktop event-page media lab: `preview-20260712t-desktop-scroll-compositions-v3/lab/event-desktop/` keeps the Editorial/Split/Gallery photo/OCR matrix and adds a clean sticky-media reading-column page. Editorial photo now has a sticky slow-parallax media/action layer with a normal-flow information slab rising over it; Editorial OCR is full-height and flush left; Split OCR keeps a readable width and natural page scroll. `Смотрите дальше` normalizes each row by media type and bounded OCR crop. The surface remains hidden below `1024px`; production mobile components/styles are untouched. Preserved v2: `preview-20260712t-desktop-clean-pages-v2/lab/event-desktop/`. Canonical notes: `event-desktop-media-families-2026-07-12.md`.
