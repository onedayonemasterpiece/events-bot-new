...
- Desktop event-page media lab: `preview-20260712t-desktop-clean-pages-v2/lab/event-desktop/` keeps the Editorial Slab, Split Canvas and Gallery Exhibition photo/OCR matrix and exposes six clean full-flow real-event pages. They preserve the accepted family geometry, use the production fullscreen gallery instead of a lower story gallery, keep no-OCR parallax bounded inside the hero, disable OCR/reduced-motion parallax, make the desktop header sticky and reuse production `split-actions` cards in the dark `Смотрите дальше` section. The event UI contains no research/service copy; the overview alone keeps the `326`-event audit. The surface is hidden below `1024px`, and production mobile components/styles are untouched. Canonical notes: `event-desktop-media-families-2026-07-12.md`. The rejected technical full-flow preview and the earlier media-only/six-option rounds remain historical evidence.
