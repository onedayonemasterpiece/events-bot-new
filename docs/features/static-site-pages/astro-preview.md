...
> **Build ID:** `preview-20260627-event-pages-v38`
> **Public preview index:** <https://kenigevents.ru/preview-20260627-event-pages-v38/__preview/>
...
- Control event page: <https://kenigevents.ru/preview-20260627-event-pages-v38/sobytiya/pesni-sssr-svetlogorsk-5878/>
- Control event calendar file: <https://kenigevents.ru/preview-20260627-event-pages-v38/sobytiya/pesni-sssr-svetlogorsk-5878/event.ics>
- Control discovery JSON: <https://kenigevents.ru/preview-20260627-event-pages-v38/data/discovery/5878.json>
- Preview sitemap: <https://kenigevents.ru/preview-20260627-event-pages-v38/sitemap.xml>
- Preview robots: <https://kenigevents.ru/preview-20260627-event-pages-v38/robots.txt>
...
- Yandex Object Storage website fallback: <http://kenigevents.ru.website.yandexcloud.net/preview-20260627-event-pages-v38/__preview/>
...
- v38 keeps property-label polish, restores the taller mobile drawer tag, limits the title sway to a two-state right-shift/back animation, restores the split-card lower rounding with `Не интересно` inside the card utility row and share/like under the card, adds hidden listing personal-feed slots with localStorage/RPC preparation, adds a local `Все / Для меня` static-list filter with `Скрыто N`, prepares CDN-aware asset URLs via `PUBLIC_ASSET_BASE_URL`, strengthens Open Graph image metadata and adds the temporary `Поделиться эксперимент` and `Поделиться эксперимент 2` Web Share payload test buttons with generated 1080×1350 PNG fallback.
...
- Preview canonical and `og:url` include `/preview-20260627-event-pages-v38/`; production canonical is not emitted by the preview build.
...
cd site
npm install
PREVIEW_BUILD_ID=preview-20260627-event-pages-v38 npm run build:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v38 npm run check:preview
PREVIEW_BUILD_ID=preview-20260627-event-pages-v38 npm run deploy:preview
```
...
The first public preview (`preview-20260627-event-pages-v1`) was superseded after visual review. The current `preview-20260627-event-pages-v38` keeps the event page mobile/feed-oriented and feedback-aware, and replaces the v19 safe image block with the v20 hero composition lab from `event-hero-lab-2026-06-27.md`:
...
After direct product review, `preview-20260627-event-pages-v38` rolls back the UX regressions introduced by the split recommendation rails and adds the first static-seed/client-hydration discovery contract:
...
After consultant review, `preview-20260627-event-pages-v38` additionally hardens the first discovery layer:
...
`preview-20260627-event-pages-v38` adds the current product corrections:
...
- event `5370` is a documented fixture override: production currently marks the long-running exhibition «Точка и линия» free because a free curator round-table source was merged into it. The v38 fixture renders it as paid/ticketed for preview correctness, while production DB repair remains a separate source-of-truth task.

### v38 listing switch/share corrections

`preview-20260627-event-pages-v38` adds the current acceptance fixes: calculated brand-tag sway distance, a bottom mobile segmented list switch that hides when `Скрыто 0`, defaults to `Для меня` when a local negative mark exists, and avoids footer overlap; plus a second share experiment where the event URL is embedded in the caption/text payload instead of passed as a separate `url` field. Browser notification/Web Push and production share-image generator requirements are documented as follow-up work, not implemented runtime prompts.

`npm run check:preview` passed for v38. Live HTTP smoke (`curl`) returned 200 for the preview index, today listing and control event page; Playwright screenshot capture was not rerun in this pass because the local Playwright browser binary is not installed.

## v16/v17 personalization-contract correction + v18 UI A/B + v20 hero composition lab

`preview-20260627-event-pages-v38` keeps the discovery implementation aligned with the documented `event_detail_related` contract:
