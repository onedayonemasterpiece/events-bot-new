...
- Desktop event-page media lab: `preview-20260712t-desktop-media-families/lab/event-desktop/` compares Editorial Slab, Split Canvas, Gallery Exhibition and gated Typographic Lead twice: real no-OCR imagery uses strong `cover`, while real OCR posters use ratio-aware placement and a measured `<=20%` safe-cover budget with automatic `contain` fallback. QA includes Full HD, QHD, ultrawide and deliberately short viewport heights; the production mobile hero-overlap is untouched. Canonical notes: `event-desktop-media-families-2026-07-12.md`. The previous six-option round remains published at `preview-20260711t-desktop-event-layouts/lab/event-desktop/` and documented in `event-desktop-layout-lab-2026-07-11.md`.
