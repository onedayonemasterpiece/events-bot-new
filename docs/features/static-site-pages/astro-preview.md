...
Public focus preview `preview-20260711-event-transport-compact-v6` supersedes v5. Rail behavior is unchanged from the accepted compact pass: arrivals 20–90 minutes before the event, factual unknown-end boundaries, one visible route label, and whole-row `.ics` links with keyboard focus.
...
The preferred north–south walking route now has two deterministic assets selected with `<picture>`: a 450×450 square map for desktop/tablet and a 360×450 portrait map for mobile. Both show the same OSM/Valhalla pedestrian geometry over Yandex Static Maps; the mobile route is no longer squeezed into a wide landscape crop.

The 400-event focus fixture built 421 HTML pages and 169 transport `.ics` files and passed `npm run check:preview`; focused Python duration tests passed. Public pages and both responsive map assets returned `200 image/png`. Desktop/mobile Playwright QA found no transport errors or horizontal overflow and proved that desktop loaded the 450×450 square source while mobile loaded the 360×450 portrait source. Evidence is under ignored `artifacts/codex/site-presentation-transport/visual-v6/`. Scheduled Kaggle refresh, final browser acceptance and the organizer-date conflict for event `6710` remain presentation gates.

- Real rail event `6510`, explicit `17:00–18:10` duration and two trains each way: <https://kenigevents.ru/preview-20260711-event-transport-compact-v6/sobytiya/kontsert-posvyaschenie-muslimu-magomaevu-i-anne-german-svetlogorsk-6510/>
- Real late concert `6397`, factual `22:40` last train / no night service / `06:25` first next-day train: <https://kenigevents.ru/preview-20260711-event-transport-compact-v6/sobytiya/kontsert-kaver-gruppy-diskodyadi-svetlogorsk-6397/>
- Romanovo event `6710`, filtered buses and responsive walking route (date still needs organizer reconfirmation): <https://kenigevents.ru/preview-20260711-event-transport-compact-v6/sobytiya/slet-babok-ezhek-romanovo-6710/>
- Preview index: <https://kenigevents.ru/preview-20260711-event-transport-compact-v6/__preview/>
