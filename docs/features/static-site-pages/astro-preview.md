...
- Desktop event-page media lab: `preview-20260712t-desktop-multimedia-full-flow/lab/event-desktop/` keeps the Editorial Slab, Split Canvas, Gallery Exhibition and gated Typographic Lead media matrix and adds four separate full-page real-event prototypes. They continue beyond hero into description, a clearly labelled planned comment-feedback aggregate, a manual/no-autoplay multi-image viewer, practical information, a truthful empty transport-media slot for out-of-city cases and the standard `Смотрите дальше` release boundary. The fresh production audit covers `326` public-eligible active events, `195` multi-image cases and the alternative-landscape/resolution gates; desktop-only parallax is bounded and disabled by reduced-motion. The production mobile hero-overlap is untouched. Canonical notes: `event-desktop-media-families-2026-07-12.md`. The prior media-only and six-option rounds remain published at `preview-20260712t-desktop-media-families/lab/event-desktop/` and `preview-20260711t-desktop-event-layouts/lab/event-desktop/`.
