...
> Статус: **M5 — обязательный release blocker; structured source support частичный, canonical Event/static projection/UI coverage отсутствуют**.
>
...
В репозитории уже встречается `age_restriction` в отдельных structured source parsers и тестах, включая Qtickets, Филармонию, «Пирамиду», «Дом искусств» и Universal Festival Parser. При этом текущая canonical `Event` model не содержит отдельного age-rating field, static exporter не проектирует его в `PreviewEvent`, а общие Astro event cards/detail facts его не рендерят. Возраст иногда остаётся только внутри `description_html`, что не даёт гарантированной маркировки и не считается выполнением требования.

Следовательно, это не косметическое добавление badge: нужен сквозной source → Smart Update → canonical DB → static manifest → every renderer contract.
...
## Data and rendering changes required

- canonical Event migration/field plus provenance contract;
- import/Smart Update preservation during create, update, merge, split and source refresh;
- production export, `PreviewEvent`, discovery/related/personal/tag/favorite projections and event fingerprint include the accepted value;
