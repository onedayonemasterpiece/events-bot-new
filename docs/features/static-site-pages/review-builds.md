...
Для отправки владельцу используйте `latest_review_url`: это прямая CDN-ссылка
на входную страницу последней доступной сборки. `review_entry_url` хранит её
в записи сборки; `public_url` остаётся адресом основного домена, `bucket_url` —
адресом фактического объекта. Поля `review_scope`, `source_snapshot` и
`verification` сохраняют границы ревью, происхождение данных и проверки при
повторной синхронизации. Отметка `desktop-only` не означает мобильную приёмку.
