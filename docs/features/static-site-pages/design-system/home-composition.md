...
Review polish: shared lower-navigation elevation is layered and visible (not a Home-only skin). Exhibition unread badge is repeated next to its actual destination in the mobile menu. Home feed heading is “Куда сходить”; the applied legacy personalization label states “По вашим действиям на сайте” instead of claiming inferred interests.

After a successful calendar-file download, an existing consented profile also favorites the event through the shared action/log path. The UI does not claim the OS calendar imported the file. Without a consented profile no hidden profile is created.
