...
- no more than three medallions are rendered for one event; when all three slots are used, the horizontal separator is removed and side medallions render as 3-column crops around the full 4-column middle medallion (`3+4+3` cells per row) to keep the row inside narrow Telegram mobile bubbles;
