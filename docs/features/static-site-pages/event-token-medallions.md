...
> **Status:** curated organizer/venue/festival/source/Pushkin assets are rendered on accepted static event pages and projected into Telegram RichMessages. SVG is the static-site primary where source-faithful; raster-only items are WebP-first with PNG fallback/QA. Organizer and venue-brand coverage includes the accepted v10 desktop runtime; the Telegram projection keeps deterministic same-stem PNG fallbacks so the production Pillow renderer needs no SVG engine.
