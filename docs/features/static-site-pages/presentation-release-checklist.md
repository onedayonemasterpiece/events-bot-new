...
- [x] **Done in static generator** — transport ICS files are generated only for rendered actions, use semantic Latin destination/date/service names, preserve VEVENT UIDs and enforce 4-file standard / 6-file dual-origin per-event ceilings without orphan files.
