...
>
> Restored 2026-07-17 from
> `origin/fix/static-site-v4-personalization-media-20260716@9e00f9a6`.
> Existing `Done in preview/locally` rows are historical candidate evidence and
> must be rerun on the frozen `origin/main`-reachable RC; they are not inherited
> automatically by a new build.
...
Stable scenario families are defined in [test-scenarios.md](test-scenarios.md):
`ADD-RC-01..06`, `ADD-BUILD-07..14`, `ADD-TR-06..08`,
`ADD-DS-01..07`, `ADD-SEO-04..07`, `ADD-SEC-03` and `ADD-PERF-01`.
Every checkbox closed for the final candidate must reference one or more evidence
rows from those families; broad wildcard coverage is insufficient.
