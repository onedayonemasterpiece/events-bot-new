...
> Status: **expanded release baseline restored; main-based design-system candidate
> `128a2d6a` exists in a side branch, while immutable RC preview and final
> product/design sign-off remain pending**.
...
The canonical component catalog is [the static-site design system](design-system/README.md),
rendered at `/lab/design-system/`. Because the candidate is not yet in
`origin/main`, its checks are evidence/WIP rather than release truth.
...
- approved product components have a registry row with runtime source and state coverage;
...
- `npm run check:design-system` and generated catalog assertions in `check:preview` pass on the exact RC SHA.
