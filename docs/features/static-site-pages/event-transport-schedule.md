# Static event transport schedule and presentation contract

> Status: the presentation snapshot/UI has historical public-preview evidence and
> the two-provider refresh control-plane foundation has a controlled synthetic
> mechanics canary. Official-source adapters, production status-ledger canary and
> production scheduling remain blocked/off. The official-source reference for the
> next multimodal localities is documented in
> [rail-multimodal-directory.md](rail-multimodal-directory.md).
...
> Status: **PARTIAL foundation / P0 presentation blocker**.

The earlier single `ParseTransportSchedule`/YDB proposal is superseded by two
independently runnable Kaggle CPU provider lanes and server-side validated fan-in:

- `scripts/run_kppk_transport_refresh_kaggle.py` /
  `kaggle/TransportKppkRefresh`;
- `scripts/run_bus_transport_refresh_kaggle.py` /
  `kaggle/TransportBusRefresh`.

Both emit `kenigevents.transport_provider.v1`. `TransportManifestStore` records
immutable provider attempts, explicit `fresh|failed|invalid|partial|stale` attempt
and serving state, preserves each provider's last-known-good snapshot, and advances
the combined current pointer only after complete validation. A changed combined
semantic hash persists one retry-safe `static_site_build:prod` rebuild intent;
unchanged content creates none. If the key is already running, one deferred
coalesced follow-up is retained.

### Static consumer and calendar boundary

The static builder may consume only the immutable manifest named by
`combined/current.json`; Astro/browser runtime never calls a provider. Build-time
selection in `transport_refresh/selection.py` rejects Kaliningrad/unbound events,
requires exact local service date and reviewed city/venue coordinates, keeps only
outbound arrivals before the event, and offers returns only after an explicit end.
It returns at most two deterministic options per direction.

`transport_refresh/ics.py` keeps occurrence-event ICS separate from transport-leg
ICS. Each visible selected leg has exact stops/times, a stable transport UID and a
30-minute `VALARM`; it never replaces the event calendar file.

### Controlled nightly schedule — documented, not active

After a successful real-source controlled canary, the proposed UTC schedule is:
...
This table is not scheduler configuration. There is no transport job/enable flag
in `scheduling.py` or `fly.toml`. Do not activate it until both reviewed official
KPPK HTML/PDF and bus HTML adapters have passed real dated canaries with the
production status DB, heartbeat callbacks, resource leases, immutable fan-in and a
changed/unchanged publish-DB pair proving one/zero builds.

### Current evidence and remaining gate

Local contracts cover provider isolation/schema, invalid/partial/stale recovery,
last-known-good, immutable fan-in, changed/unchanged/coalesced rebuild behavior,
event/transport ICS separation and Kaggle instrumentation. Controlled private runs
`controlled-20260717-kppk-v3` and `controlled-20260717-bus-v1` validated synthetic
dated JSON and produced combined semantic hash
`28b103cec10c768ed5c49a962956982cfb159e7248720b3a382bdfda0f4d191a`.

That is mechanics evidence only: no official-source adapter, status DB/callback,
production ledger, public manifest or static build was used. Until those gates are
closed, the presentation candidate must not claim automatic schedule freshness or
