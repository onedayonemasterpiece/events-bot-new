...
`/artefakty/kollektsii/znaki-yantarnogo-kraya/#amber-cosmonaut`. There is no
listener that sends this event to a
...
In a preview/secret candidate,
`/artefakty/kollektsii/znaki-yantarnogo-kraya/` is the noindex concrete
collection surface. It shows the review state `1/8`: the found amber object is a
keyboard-operable detail button, while seven unfilled slots reveal neither a
name nor an image. The detail is a labelled native dialog with an explicit
close action, focus return, factual source and email feedback route. The
separate `/artefakty/` route is the editorial registry/lab, not user progress.
Ordinary production renders a neutral unavailable state on both routes and no
collection UI or object reference.
