...
После `INC-2026-06-18-vk-title-shortlink-public-regression` deterministic title guard в `vk_intake` не имеет права заменять LLM-заголовок на шаблон `<event_type> — <venue>` (`Концерт — Бар Советов`). Exact-token grounding не должен считать нормальные морфологические/attendee-facing варианты вроде `Виниловый вечер с DJ Switchoff` подозрительными, когда source содержит `винил`/`вечер`/`DJ Switchoff`. Если после conservative grounding всё же остаётся gap, guard не переписывает title: poster OCR heading может быть только evidence в логах, а сам title остаётся LLM-owned и дальше проходит Smart Update LLM title-recovery/grounding. Ссылки регистрации из внешних shortener'ов вроде `clck.ru` раскрываются до конечного URL до сохранения `ticket_link`, чтобы публичный Telegram/VK/Telegraph не вёл пользователя через source shortlink.
