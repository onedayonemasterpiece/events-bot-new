...
## Контролируемый replay одного VK-поста

`vk_auto_queue.run_vk_auto_import(..., target_post_url="https://vk.com/wall-<owner>_<post>")` обрабатывает ровно указанную запись `vk_inbox`: функция берёт существующую строку, лочит её под текущий batch и не выбирает соседние pending-посты. Это основной безопасный путь для incident replay исходного поста после фикса Smart Update; не используйте для этого нашу итоговую managed VK-публикацию, если нужно проверить внешний источник.
...
- Посты об **отмене/переносе** обрабатываются LLM-first: `event_parse` должен извлечь конкретное событие и вернуть `lifecycle_status=cancelled|postponed`, а Smart Update затем матчится с существующей карточкой и применяет статус. Буквальные keyword/regex matcher'ы не имеют права по умолчанию деактивировать событие.
  - Legacy deterministic matcher допускается только как явный операторский emergency fallback через `VK_AUTO_IMPORT_LEGACY_CANCEL_MATCHER=1`; штатный путь должен идти через LLM parse + Smart Update.
  - Посты о переносе только **времени начала** (`8 мая время начала ... перенесено на 19.30`, при этом событие остаётся открытым/регистрация продолжается) должны пройти обычный LLM-first import + Smart Update и не деактивировать событие без решения LLM.
