...
- `tg_event_publish` различает новые объявления и edit/reconciliation jobs: если у события уже есть `tg_event_post_id`, такой job может обновить существующее сообщение, но не резервирует новый publish slot и не становится spacing-anchor для no-post событий. Когда в очереди одновременно есть no-post announcements и existing-post edits, сначала идут реальные новые посты; старые edit rows не должны делать канал визуально “активным” только за счёт правок старых message id.
...
- Canonical `tg_event_publish` sends/edits must await the Premium/custom-emoji editor immediately for event posts. Delayed background editor scheduling is allowed for daily/bulk non-event surfaces, but ordinary Afisha event posts should not have a public multi-minute gap where Premium emoji or medallion mosaics are missing.
