...
- Если Telegram Bot API отклоняет media URL при `sendPhoto`/`sendMediaGroup` (`WEBPAGE_*`, bad remote media, временные preview ошибки), publisher обязан деградировать в обычный text post с `disable_web_page_preview=True` и inline-кнопкой календаря, а не оставлять `tg_event_publish` в retry-loop. Это fallback публикации события; media качество чинится отдельно, но VK/TG fanout не должен зависать.
