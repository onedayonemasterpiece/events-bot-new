...
- Если у события есть `ics_post_url`, пост получает inline-кнопку вида `📅 <дата или диапазон> <время> · Добавить в календарь`, ведущую на Telegram-пост календаря (`https://t.me/kenigeventscalendar` / private `t.me/c/...` link), а не на сырой `.ics` файл.
