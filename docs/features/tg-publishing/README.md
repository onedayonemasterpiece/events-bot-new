...
- Canonical `tg_event_publish` does **not** enqueue or invoke the Premium/custom-emoji editor for `rich_message` posts. The delayed editor may still normalize unrelated labels on legacy posts, but receives `medallion_html_block=None`; using it to place event medallions again is forbidden.
...
- Resolver читает `site/src/data/organizerMedallions.json`, опциональный `festivalMedallions.json`, соответствующие local runtime assets, source asset `MEOW Афиша` и Pushkin-card composite. Venue/organizer aliases сопоставляются только с location/source identity fields, festival/program marks — со структурным `event.festival`; `event.festival=80 историй о главном` всегда даёт отдельные `KGD80` и `Знание`. Поэтому event `6811` в КОНБ обязан разрешаться в три медальона: `КОНБ + 80 историй + Знание`.
- Presentation cap по умолчанию — 5 source-grounded medallions (`TG_GRAPHIC_MEDALLION_MAX_ITEMS`), при переполнении все выбранные assets пропорционально уменьшаются в пределах safe area. Пушкинская карта добавляется как program medallion после organizer/festival/source identities.
