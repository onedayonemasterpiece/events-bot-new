...
- Обычный non-promo footer сохраняет текущий контракт канала. В promo-режиме `Подробнее` уходит в inline-кнопку `✨ Подробнее`, чтобы CTA не конкурировал с текстом промо; `Подписаться` и `Вконтакте` не добавляются в event captions.
...
- Source posts that look like third-party ticket giveaways (`розыгрыш` / `разыгрываем` / `выиграть` + `билет` / `пригласительный`) are fallback content for managed Telegram/VK event publication. They are skipped only when a non-giveaway event is already queued or published; if no alternative exists, the giveaway event may still publish so the feed is not empty.
