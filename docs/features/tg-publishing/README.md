...
- Spacing игнорирует `error` jobs и pending/running anchors дальше `TG_EVENT_PUBLISH_SPACING_HORIZON_HOURS` (по умолчанию `24`) от текущего времени: это защищает новые публикации от ручных cleanup-marker rows вроде `next_run_at=2036-...`. Если текущий локальный publish window уже закрыт и новый кандидат нормализован на завтрашнее `07:00`, stale anchors того же следующего дня, стоящие намного позже утреннего старта, тоже не должны сдвигать свежие импорты в вечерний кластер; нормальные утренние anchors продолжают задавать стандартный интервал.
