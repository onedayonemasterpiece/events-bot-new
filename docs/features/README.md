...
- `docs/features/static-site-pages/first-party-action-map.md` — canonical TO-BE
  временной component-aware action-map диагностики: read-only presentation
  receipt, zero-cost default-OFF, bounded YDB TTL и reviewed evidence без
  автоматической мутации персонального профиля
