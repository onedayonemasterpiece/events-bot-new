...
- `Посты: VK пост/⏳ · TG пост/⏳` (кликабельные ссылки на управляемый `klgdevents` VK-анонс и Telegram event post, если они уже опубликованы; для VK рядом может быть пометка `источник VK: @...`, когда publish-flow распознал известный VK-источник)
