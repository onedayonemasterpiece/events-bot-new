...
- Матчинг кандидата с существующими событиями:
  ...
  - **Merge-path final verdict (INC-2026-08-22).** Старый `merge_identity_gate` теперь также обязан завершиться typed verdict: accepted same/source update → `FINAL_MATCH`; explicit `related_but_distinct|festival_context_sibling` с confidence `>=0.80`, точными source/OCR quotes и concrete conflicts → `FINAL_DISTINCT`; `unsafe_to_merge`, blocking conflict, abstention и invalid/provider output → durable `FINAL_RETRY`. Промежуточный запрет merge больше не является разрешением CREATE. В том же существующем LLM-вызове schema несёт `source_grounded_evidence`; отдельного provider call нет. Для overlapping long-running cards одинаковые title + exact ticket identity являются узким fail-closed refresh rail: профиль репостера не может доказать отдельную площадку/дату, поэтому `3216/8284` retry, а не create. Аналогично `source_update`, превращённый anchor-conflict rail в `unsafe_to_merge` (`7907/8280`), retry, а не create.
...
- Semantic filters:
  ...
  - Полностью завершившийся до текущей локальной даты автоматический candidate получает сразу типизированный терминальный исход `REJECTED_PRODUCT_POLICY/past_event`: он не создаёт историческое событие и не попадает в retry. Этот исход принимается до occurrence/anchor/location LLM-review, поэтому шум исторического текста не может сначала превратить очевидно прошедшую запись в `FAILED_TECHNICAL`; после semantic narrowing проверка повторяется. Та же защита закрывает невозможный диапазон длиннее двух лет с прошлой датой начала (типичный результат ошибочной привязки года окончания, например `2022-11-10..2026-12-10`); ручной ввод `source_type=bot` ею не блокируется.
...
- После reference-нормализации подозрительные social-source локации проходят отдельный LLM-first `location_grounding_review`. Детерминированный слой только маршрутизирует high-risk случаи; LLM обязан выбрать закрытое финальное действие `keep|repair|reject_missing_location`. Для `repair` требуется дословная source/OCR quote и source-grounded имя либо адрес. Для `keep` семантический verdict принадлежит LLM, а механический guard требует, чтобы текущее имя **или адрес** действительно присутствовали в source/OCR; отдельная неточная citation-строка больше не теряет событие с уже доказанным адресом. Узкая структурная нормализация считает `проспект` и `пр-кт` одним адресным токеном, не меняя venue semantics. Для Telegram дополнительным авторитетным evidence может быть ровно текущая настроенная `default_location`, если producer извлёк ту же площадку и в посте нет явной конфликтующей attendee-facing локации. Эта source-profile улика передаётся самому LLM; явное место из поста всегда приоритетнее профиля. Numeric confidence сохраняется как диагностика, но больше не отменяет валидный grounded `keep`: именно этот fallthrough зацикливал `wall-32547811_11187`. Если источник подтверждает только город/посёлок, название фестиваля/праздника или программу, LLM возвращает `reject_missing_location`, и candidate сразу получает typed product terminal без semantic retry. Malformed/provider/invalid-grounding response заканчивается `FAILED_TECHNICAL`, также без фонового повтора. После `llm_repair` reference-normalization может привести только source-grounded эквивалент; если она снова подставляет неподтверждённое имя, сохраняется LLM-repaired venue. Это предотвращает подмены парк `Юность` → одноимённый дворец спорта, `остров Шайба` → `Остров Канта`, `Советский 12` → `ИЦАЭ` и programme/occasion label → venue.
...
The separate anchor-role reviewer also uses a closed result set:
`keep|repair|reject_missing_date`. When a source proves only an end date (for
example “daily until 30 August”) but no exact start date, it returns the typed
product terminal `missing_date` instead of a technical limbo. If the same
candidate/occurrence binding already owns an active canonical Event with the
exact same date/range/time, that accepted structural evidence bypasses only
the repeated anchor-role call; the edited packet still passes eventness,
location and normal merge processing. For online events with an empty city,
Smart Update may restore a region place from the selected source only when it
