...
- Мердж:
  ...
  - детерминированные non-event фильтры (без LLM) для авто-источников VK/TG:
    ...
    - generic digest wrapper (`Дайджест`, `Афиша`, `Подборка`, `Мы давно его ждали`) без event-local времени и площадки скипается как `skipped_non_event:generic_digest_shell`; это safety-net после LLM, а не попытка regex-ом извлечь событие из текста.
