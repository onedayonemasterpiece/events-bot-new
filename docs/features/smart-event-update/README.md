...
- `SMART_UPDATE_IDENTITY_GATE=off|shadow|enforce` — create-path identity gate after the widened-recall `_llm_dedup_adjudicator` and before a new event row is created. `off` is the default and does nothing. `shadow` builds/logs a structured verdict but never changes behavior. `enforce` may veto create on narrow deterministic evidence (same source identity, same ticket+slot, same poster identity) or future vector evidence; it never auto-merges. If the gate itself errors in `enforce`, Smart Update fails safe with `skipped_identity_gate` instead of creating or merging automatically.
