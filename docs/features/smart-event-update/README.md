...
- Детерминированные ранние фильтры “не-событий” (без LLM), чтобы не создавать псевдо-ивенты:
  ...
  - `skipped_non_event:work_schedule` (посты про режим/график работы учреждений, включая формулировки «праздничные дни», «выходные дни», «расширенный график работы», табличные часы/дни недели; обычные лекции/шоу/фестивальные слоты в музеях/библиотеках или на адресах вроде `Музейная аллея` не режутся этим guard)
