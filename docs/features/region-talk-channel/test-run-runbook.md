...
- `scripts/region_talk_embedding_model_decision_report.py` — local live-YDB decision exporter for CPU-only model-selection research; it reads only existing YDB row kinds, joins E5/vector-gate evidence with BGE/Gemma/Qwen enrichment rows, and writes `comparison.json`, `comparison.csv`, `model_only_candidates.xlsx` and a Markdown decision report under `artifacts/codex/`. Custom E5 probes may reuse the Qwen/Gemma no-Telegram worker by setting `REGION_TALK_QWEN3_MODEL_ID`, `REGION_TALK_QWEN3_MODEL_SHORT`, `REGION_TALK_QWEN3_ENCODER_CONTRACT` and a unique `REGION_TALK_QWEN3_KERNEL_TITLE`.
