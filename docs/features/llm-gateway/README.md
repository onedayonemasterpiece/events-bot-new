...
`gemini-3.8-flash` зарегистрирован с запрошенным владельцем локальным потолком
`20 RPD` на каждый `quota_scope/model`. `5 RPM / 250000 TPM` повторяют
существующие защитные потолки Flash. Эти числа не подтверждают текущие квоты
Google по отдельным проектам и должны быть сверены с AI Studio.
