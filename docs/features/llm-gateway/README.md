...
ключа.
Поэтому два ключа одного проекта расходуют общую квоту и суммируются перед
admission. Владелец подтвердил 2026-09-24, что шесть используемых ключей
принадлежат шести отдельным проектам; для них применяются одинаковые
показанные в AI Studio потолки. Исторический rollout blocker описан в
`INC-2026-07-31-google-ai-parallel-limiter-bypass`.
...
`gemini-3.8-flash` зарегистрирован с потолком `5 RPM / 250000 TPM / 20 RPD`
на каждый `quota_scope/model`. Эти значения были запрошены владельцем и затем
совпали с его readback AI Studio `2026-09-24` для всех шести проектов.

Readback AI Studio от владельца `2026-09-24` подтвердил одинаковые квоты для
шести отдельных проектов. Базовые строки `gemma-4-26b-a4b` и
`gemma-4-31b` исправлены с `15/15000/14000` на `30/16000/14400`
(`RPM/TPM/RPD`); именно к базовым ID клиент нормализует вызовы `-it`.
`gemini-embedding-2` исправлен с `10` на `100 RPM`, Antigravity — с
внутреннего запаса `54/96000/90` на показанные провайдером `60/100000/100`.
Для `gemini-3.5-transcribe` и двух `gemini-3.8-*-tts` добавлены положительные
конечные квоты из того же readback. Показанные в AI Studio модели с нулевой
ёмкостью остаются без строки и блокируются до отправки; `Unlimited` Live API и
Veo без числового TPM эта таблица не представляет.

Исторические консервативные seed-значения Gemma 4 по quota UI от `2026-07-31`
(заменены readback `2026-09-24` для двух строк Gemma 4):
