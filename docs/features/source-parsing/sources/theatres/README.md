...
Если location/date/time уже совпали и нормализованные названия равны точно,
официальный parser provenance добавляется без LLM: этот путь не переписывает
описание или смысл карточки, а только включает обычную синхронизацию
билетов/возраста/медиа. Любое неточное совпадение по title по-прежнему проходит
через Smart Update.
