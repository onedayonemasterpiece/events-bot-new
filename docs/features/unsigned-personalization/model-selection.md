...
| --- | --- | --- | --- |
| Online ranking | Deterministic formula `local_rerank_v1`; later CatBoost/LightGBM only after logs | Cheapest, explainable, no provider latency/quota risk; matches source analytics MVP recommendation | Implemented as reference JS contract; no provider call |
