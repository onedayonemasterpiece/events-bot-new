...
The frontend now renders three graphite card-shaped shimmer placeholders while the first search page is running. “Показать ещё” stays visible as a disabled `Загружаю ещё…` control during the next batch instead of disappearing silently. Search feedback is optimistic: the click immediately records a local queue item and shows a visible “saving” state; the Supabase RPC then confirms it as saved or leaves a clear local-only state if the server is slow. These are perceived-latency guards only; true vector-first progressive rendering remains a follow-up because the current reliable mobile path requests a single JSON response rather than the previously unreliable NDJSON stream.
