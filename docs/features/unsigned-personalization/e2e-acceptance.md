...
The release E2E route matrix must visit every generated HTML page family: root, listings/categories/tags, event detail, search, `/moi-sobytiya/` plus the `/izbrannoe/` compatibility entry, forwardable personal page, transport-enabled event and admin HTML surface. Each must expose the same shared identity controller and reach the same restored visible state. `.ics`, JSON, sitemap, robots and media are non-HTML exclusions.
...
| `FAV-MENU-001` | Calendar save, favorite, repeat save and undo from cards/detail while another same-origin tab is open | CTA and `Моё` / `Мои события` badge converge without prior-user/zero flash | one distinct durable union row; calendar first-save remains once-only | badge appears only at `N>0`; dual state counts once and likes/downloads/reminders/transport legs do not affect it |
| `FAV-PAGE-001` | Open `/moi-sobytiya/` with calendar-only, favorite-only, dual, rescheduled, cancelled/merged and past rows | static shell contains no private cached HTML; one batched load and local filters | RLS returns each resolved canonical saved event once | lifecycle groups and `В календаре` / `Избранное` states are visible; no per-card remote loop |
| `CAL-COUNT-001` | Request the same occurrence ICS five times, including retry/concurrent-tab delivery | every ICS remains usable; only confirmed server state uses `В моём календаре` | one `(profile,event)` first-save and aggregate delta `+1`, not `+5` | download attempts never inflate public/user counts |
| `CAL-MERGE-001` | Save the same occurrence in anonymous and existing authorized profiles, then link/merge | one current row remains visible after convergence | owner facts and aggregate reconcile without double count | another occurrence remains a separate saved object |
| `CAL-FALLBACK-001` | Save backend fails while direct/no-JS ICS is opened | ICS still opens and UI reports unconfirmed cross-device state | no trusted save/counter write is fabricated | static event navigation remains usable |
