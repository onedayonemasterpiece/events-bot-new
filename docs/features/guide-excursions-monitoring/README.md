...
- **Палитры и типографика** берутся из валидированного набора `guide_excursions/assets/vk_hook_card_palettes.json` (контраст ≥ 7:1 для основного текста); внутри одного поста палитры не повторяются и подбираются из разных семейств. Текст центрируется в safe-zone `x 150–930 / y 160–920` с большими полями, чтобы VK-обрезка краёв сетки не задевала смысловой текст; кегль главного крючка авто-подбирается 56–104 px, подпись 40–56 px. Шрифт — `assets/fonts/Cygre-ExtraBold.ttf` (кириллический гротеск).
- Код: `guide_excursions/hook_cards.py` (LLM-пайплайн + отбор) и `guide_excursions/hook_card_render.py` (Pillow-рендер); интеграция в `publish_latest_guide_digest_to_vk` через `_build_guide_vk_hook_card_attachments`, карточки дописываются в `attachments` после афиш и попадают как в обычную публикацию, так и в `repair_existing` edit. Покрыто `tests/test_guide_hook_cards.py`.
