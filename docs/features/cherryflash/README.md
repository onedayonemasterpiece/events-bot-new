...
The base CherryFlash profile may insert one guide-excursion promo scene per day when a future occurrence has at least one personal guide avatar and at least one free place. Selection is base-profile only: partner tracks and Eco never receive excursion promo scenes. Selection skips sold-out occurrences and avoids showing the same occurrence on consecutive days; insertion happens at a random position from slot 2 through slot 6, with a random approved palette.

**Approved production renderer:** `true3d-v4-approved-2026-07-11`, implemented in `scripts/render_cherryflash_guide_true3d_v4.py` and called from `scripts/render_cherryflash_full.py`. This file is copied from the agreed Telegram artifact run `run_video_20260711_130410`; production must call this tracked renderer instead of reimplementing the scene “по мотивам”. The legacy PIL `_render_guide_excursion_frame` remains only as a disabled fallback/dev reference (`CHERRYFLASH_GUIDE_TRUE3D_ENABLED=0`).

The approved 720×1280 contract preserves the agreed visual hierarchy: centered avatar/face-safe two-medallion group, top label plus SVG Repo icon, title only below the avatar, date pill with month text such as `10 июля 16:00`, and a large screenshotable CTA/contact block. The Blender pass supplies the accepted depth/material layer while crisp typography and icon masks stay as the high-resolution product overlay. For one guide avatar the same renderer uses one centered hero medallion; for multiple avatars production uses the first two personal avatars in the approved face-safe overlap.

The icon layer uses the approved SVG Repo-derived assets packaged in `video_announce/assets/cherryflash_icons/` (`walk`, `route`, `water`, `building`) via pre-rendered masks; hand-drawn substitute glyphs are fallback-only. The motion contract is non-static from encoded frame 1, keeps a tiny upward drift through P2, and keeps background bubbles moving during P3 with depth-scaled speeds. CTA text must remain screenshotable: phone numbers are shown as phone numbers, Telegram booking shows `@username`, and VK-only booking falls back to a compact `vk.com/<slug>`/`VK` label instead of a human name that cannot be used for booking.
