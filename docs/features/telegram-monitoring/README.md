...
- `Gemma 4` producer-level contract for `location_name`: поле должно быть реальным venue/place name, а не соседней прозой, биографией спикера, schedule commentary, film metadata, ticket instruction, описанием события или temporal/date fragment. Запрещены и decorated формы вроде `🤗Завтра` / bullet-prefixed `Сегодня`: producer schema и venue-review prompt обязаны отправлять такие значения в LLM review / empty+fallback path до server import safety-net. Для расписаний schedule-rescue передаёт в каждый day-block prompt общий контекст поста, чтобы хвостовые venue-линии вроде `📍Остров Канта` были видны LLM для всех строк расписания.
