...
- Внутри Kaggle status preflight `resource_acquire` для S22 lease ретраит транзиентные callback timeouts к `/internal/kaggle/run-event` перед fail-closed abort. Только явный `resource_action=blocked` означает реальный busy lease; отсутствие ответа после bounded retries остаётся ошибкой preflight и требует catch-up, но не маскируется в расследовании как подтверждённый активный holder. Настройки: `KAGGLE_STATUS_RESOURCE_ACQUIRE_ATTEMPTS` (default `4`), `KAGGLE_STATUS_RESOURCE_ACQUIRE_TIMEOUT_SEC` (default `20`), `KAGGLE_STATUS_RESOURCE_ACQUIRE_RETRY_DELAY_SEC` (default `3`).
