...
Server-side non-event guard `work_schedule` режет графики работы учреждений, но не должен подавлять
конкретные события в музейных/библиотечных площадках. Сигналы вида `регистрация на лекцию`,
`регистрация на встречу`, `регистрация на экскурсию` или явное `лекция пройдёт` считаются event-invite
и проходят дальше в Smart Update; positive control для настоящего графика работы остаётся покрыт тестом.
