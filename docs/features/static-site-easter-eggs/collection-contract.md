...
## Как называются коллекции

Публичное имя коллекции — не порядковый номер и не техническая категория. Это
короткое название одной региональной истории. Правила:

- 2–4 слова, пригодные для заголовка карточки;
- одна отчётливая тема и визуальный характер;
- связь со всей Калининградской областью, а не автоматическое сведение к городу;
- без номера сезона в public name: порядок хранится в `collection_id`;
- название не обещает приз и не раскрывает места находок.

Предлагаемая линейка:

| Порядок | Публичное название | Сюжет |
|---|---|---|
| 1 | **Знаки Янтарного края** | вводный sampler: море, природа, архитектура, вкус и космическая история |
| 2 | **Балтийский характер** | маяки, суда, порт, буй, якорь и океан |
| 3 | **Кирпичная память** | кирпич, форты, башни, виллы и историческая архитектура |
| 4 | **Куршский ветер** | коса, птицы, деревья, дюны и рыбацкие образы |
| 5 | **Курортный сезон** | Светлогорск, Зеленоградск, пляжные кабинки, одежда и курортный быт |
| 6 | **Космос над областью** | земляки-космонавты, спутник, связь, электрон и технологии |
| 7 | **Вкус Янтарного края** | марципан, клопс, шакотис, крендель, селёдка и локальные продукты |
| 8 | **По рельсам и мостам** | трамваи, поезда, разводные мосты, паром и дороги |
| 9 | **Звери и легенды** | животные, коты, хомлины, Неринга и локальные персонажи |
| 10 | **Люди и эпохи** | Кант, Гофман, королева Луиза, переселенцы и культурная память |

Это naming roadmap, а не утверждённый состав. Один артефакт не должен
одновременно закрывать threshold двух активных prize collections.

## Верхняя строка в «Для меня»

Сразу после заголовка страницы `Для меня` показывается компактный teaser текущей
коллекции:

- на телефоне в одну строку помещаются ровно `3` слота без горизонтального
  скролла;
- найденные слоты показывают токен и название, неизвестные — `?` и
  **«Не открыт»**, без спойлера;
- рядом явно написано `Открыто X из N` и `Ещё N−X скрыто`;
- ссылка **«Открыть коллекцию»** ведёт на полную collection surface;
- teaser не сообщает о заявке, пока reward release не открыт;
- при отсутствии server progress показывается честный empty state. Статический
  noindex-прототип может демонстрировать sample state, только если он явно
  помечен «пример состояния».
