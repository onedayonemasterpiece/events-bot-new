...
- MVP discovery and dry-run selection should prioritize completed events with multiple independent post-event sources (for example bloggers, attendees, organizations, partners, and media), because they provide stronger social-proof evidence and better material for learning recap formats.
- Multi-source recap candidates should classify each source report by its editorial contribution: emotional/personal report, most complete factual report, professional or official-quality report, unusual angle, strong photo/video evidence, official outcomes/results, and other categories learned from observed recaps.
...
- Multi-source processing must include source-report categorization so the system can identify which evidence is most emotional, most complete, most professional/official, most unusual/personal, strongest media, or otherwise useful for the recap.
...
- Source-report categories must be source-grounded and confidence-scored; the writer may use absolute labels such as “самый эмоциональный” only when the compared evidence supports that ranking, otherwise it should use descriptive labels such as “эмоциональный личный отчёт” or “подробный фотоотчёт”.
...
- Scoring must consider event match confidence, source trust, media quality, emotional tone strength, event significance, audience segment fit, repeatability, future-event link value, promo fit, source discovery value, independent-source count/diversity, and risk.
...
- Multiple independent source reports should increase priority for `multi-source recap plus page` when event matching, source trust, rights, and quality gates pass; it must not bypass match-confidence or risk gates.
...
- For multi-source recaps, channel posts and long pages should use source-report categories to choose and explain highlights, for example: emotional personal impression, detailed factual summary, professional/official report, unusual angle, best photo/video evidence, or outcome/result source.
...
2. **Phase 1 — candidate detection and matching**: route Telegram/VK completed-event reports into recap candidates, prioritize multi-source completed-event candidates for dry runs, produce deterministic shortlist, LLM adjudication, and `missed_event_signal` for unmatched reports; run dry reports before publication.
3. **Phase 2 — facts, emotions, scoring**: reuse Smart Update fact extraction, add source-report categorization, emotion/media/metrics/significance scoring, explain route decisions, and keep promo small recap separate from editorial recap.
...
- Treat source-report categories as an LLM-first, source-grounded, evolvable taxonomy seeded by user examples (emotional, complete, professional/official, unusual/personal, strong media, outcomes), not as a closed hard-coded keyword list.
...
- 2026-06-29: Integrated multi-source recap prioritization and source-report categorization from the latest voice intake; no conflict with previous requirements found.
...
## Archived intake 2026-06-29T10:43:01+00:00

Status: resolved / archived 2026-06-29

Resolution: Integrated the two voice-note transcripts into the canonical requirements: multi-source completed events become an MVP/dry-run priority, and multi-source evidence must be categorized by editorial contribution (emotional, complete, professional/official, unusual/personal, strong media, outcome source, etc.) using source-grounded, confidence-scored classification. No contradiction with previous requirements was found; the source audio remains linked below for auditability.
