...
- При пропущенном окне один поздний counter не копируется в несколько прошлых
  ...
  `skipped_late`.
- После первого terminal snapshot его `post_ts` повторно используется даже для
  старых публикаций без legacy age-day строки. Полностью обработанный target не
  отправляется в Kaggle каждые 30 минут только из-за неизвестного timestamp.
