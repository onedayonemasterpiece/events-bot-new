...
- один штамп/посещение = каноническая единица допуска/веса розыгрыша, без фиксированной цены в коде;
- соцактивность за один день для одного участника ограничена `0.5` посещения/штампа
  (`1/2` посещения);
- репост: `+0.3` посещения — самый сильный сигнал, потому что создаёт новый узел
  распространения;
- комментарий: `+0.1` посещения — осмысленная вовлечённость;
- лайк: `+0.02` посещения — лёгкая реакция;
- просмотр: `+0.0002` посещения, но только для агрегатного отчёта и не больше `0.2`
  посещения на пост — пользовательские просмотры VK сейчас не атрибутируются к
  ФИО.

Калибровка: день с минимум `1` репостом и `2` комментариями даёт около
`0.5` посещения/штампа (`0.3 + 2*0.1`), то есть два очень хороших social-дня примерно равны
одному посещению/штампу.

Этот соцэквивалент считается как **социальный бонус без winner-penalty**: если участник
