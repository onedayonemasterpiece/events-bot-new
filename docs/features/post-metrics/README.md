...
- один штамп/посещение = `10` баллов розыгрыша;
- соцактивность за один день для одного участника ограничена `5` баллами
  (`1/2` посещения);
- репост: `+3` балла — самый сильный сигнал, потому что создаёт новый узел
  распространения;
- комментарий: `+1` балл — осмысленная вовлечённость;
- лайк: `+0.2` балла — лёгкая реакция;
- просмотр: `+0.002` балла, но только для агрегатного отчёта и не больше `2`
  баллов на пост — пользовательские просмотры VK сейчас не атрибутируются к
  ФИО.

Калибровка: день с минимум `1` репостом и `2` комментариями даёт около
`5` баллов (`3 + 2*1`), то есть два очень хороших social-дня примерно равны
