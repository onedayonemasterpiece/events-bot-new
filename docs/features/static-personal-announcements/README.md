...
| ID | Capability | Canonical home | Stage |
|---|---|---|---|
| H1 | Hero Talk / «Городской обзор»: static-first editorial scenario and promo router above the homepage discovery surface | [Typed briefing research](../static-site-pages/typed-briefing-hero-research.md), [cross-product model](../static-site-pages/product-system-analysis-2026-07-18.md) | product capability accepted by owner on 2026-07-18; no lab variant is accepted as production UI, current-main clean slice and frozen RC evidence remain missing |

H1's minimal static router is now part of the homepage product scope. This does not accept the existing side prototypes or make optional motion/personal/backend/writer layers mandatory. Before F5 freezes the homepage, the owner must accept one tested exact branch/SHA and explicitly record each optional layer as `ship|defer`; the grounded route/supply, static fallback, accessibility, performance and downstream-value gates apply to the selected implementation.
...
| M2-B | Release-time brand tag on text-free Telegram/VK images | [Social publish brand tag](../event-media/social-publish-brand-tag.md) | Telegram/VK event publishing already exists; only the branded derivative/activation remains planned for `2026-07-30 18:00 Europe/Kaliningrad`, with renderer/config/VK postponed smoke still missing |
...
- H1 Hero Talk / «Городской обзор» is now an accepted homepage capability, not a pending `ship|defer` product decision. Before F5 UI freeze, integrate a minimal grounded static router that always resolves to real eligible inventory and measure downstream interest/value; the existing labs remain research evidence rather than accepted production UI. Semantic-fragment motion, personal catalog delta, backend reads and any LLM writer are separate layers and may be deferred without removing the basic Hero Talk capability. Any selected exact implementation must pass zero-CLS, no-JS, reduced-motion, mobile/desktop and owner immutable-preview gates before M4.
...
- From the separately confirmed release timestamp, managed Telegram/VK event posts
  use one versioned high-quality brand-tag derivative for every confidently
  `visual_only` image; OCR/unknown and unsafe placement fail closed to the original.
  Activation is fixed at `2026-07-30 18:00 Europe/Kaliningrad`
  (`2026-07-30T16:00:00Z`). Existing Telegram/VK publication paths are reused;
  only the derivative selection is added. Before enabling, create/inspect/delete a
  VK postponed smoke and retain full-size visual/API evidence.
  See [the canonical social-image contract](../event-media/social-publish-brand-tag.md).
...
- [Static festival section and dedicated festival channels](../festivals/static-site-release.md): queue/root-cause cleanup, permanent website monitoring, a distinct festival card, stable event↔edition relations and festival index/detail pages through the standard static promotion pipeline; festival-only Telegram/VK/MAX destinations carry the edition lifecycle from first announcement through schedule clarifications, and festival detail pages expose their subscription CTA. It is explicitly outside the first presentation GO scope and has its own UI freeze/RC/SEO-GEO evidence.
