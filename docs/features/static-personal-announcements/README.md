...
- [Interest clubs](../../backlog/features/interest-clubs/README.md): a separate research-first release that audits past and future events before any implementation, distinguishes stable communities from linked occurrences/festivals/duplicates, and only then may introduce an automatically maintained `/kluby-po-interesam/` section. No club count or BGE production choice is accepted before the evidence report.
