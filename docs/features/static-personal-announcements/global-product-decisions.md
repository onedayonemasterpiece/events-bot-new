...
34. **Hero Talk / «Городской обзор» принят владельцем как постоянная capability главной, но ни один lab-вариант ещё не принят как production UI.** Его продуктовая роль — static-first редакционный диспетчер: выбирать один актуальный пользовательский или promo-сценарий и вести в реальный inventory через `Сегодня`, `Завтра`, `Выходные`, `Популярное`, public search или конкретное событие. Campaign priority не может обходить lifecycle, relevance, provenance, fatigue, dismissal и supply gates; Hero Talk измеряется downstream interest/value, а не CTR/длиной разговора. Минимальный grounded static router входит в homepage scope, а motion, personal delta, backend и LLM-writer остаются отдельными экспериментальными слоями с accessibility/performance/no-JS/reduced-motion и owner exact-preview gates.
