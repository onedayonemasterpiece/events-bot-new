...
- Fixed Region Talk exact-text restoration idempotency: a restored post with current E5+BGE now reuses both vectors but still rebuilds the policy/fusion and candidate-memory projection; canonical post URL reuses the existing candidate-memory identity across source-history/exact fetch routes, and source onboarding prefers the current restored body over a stale compacted duplicate.
