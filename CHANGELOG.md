...
- **LLM / Lollipop Legacy Lab**: added and tightened the experimental `lollipop_legacy.v1` benchmark variant for Smart Update copy: it now runs the current Gemma 3 baseline first, counts that baseline stage in legacy timing, uses the full baseline fact set as the mandatory floor, requires legacy output to be at least baseline length, and runs one bounded Gemma 4 31b writer pass as the additive lollipop layer. The benchmark harness supports `--variants baseline,lollipop_legacy`, ships five real-post fixtures, records the stricter `1.0 < speed_ratio <= 3.0` gate, validates style/promo/report leakage plus duplicate word/tail artifacts, and falls back to baseline with explicit warnings on writer validation failures or `LOLLIPOP_GEMMA_WRITER_TIMEOUT_SEC` timeouts.
