...
- Fixed Region Talk Telegram source-media resolution: generic `hero:1` presentation ordinals are no longer misread as Telegram message id 1, `#media` sentinels no longer replace exact source-video refs, and explicit zero-LLM media repair reclassifies the exact anchor as video when Telethon proves it.
- Added a real-production, deliberately recall-oriented review seed for the new static collection heads (science, strong impressions, medieval, kids and family) with compact evidence and hard negatives, plus an external-analyst dossier. Corrected the misleading guest `1/0` interpretation and documented the 71-occurrence/61-family performance audit; all provisional semantic labels remain publication-blocked pending owner review and metric recalibration.
...
- Completed production evidence for static-selection data prep: v2 audience/people backfills and the event 6595 role correction are persisted, the real 409-event BGE-M3 cold batch validates 15 heads with zero provider/Supabase core calls, and a live Fly deploy-overlap recovered the exact remote output without a duplicate notebook; one forced warm successor verifies final cache reuse.
