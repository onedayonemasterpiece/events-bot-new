...
- **CherryFlash / Telegram Feed Preview**: final CherryFlash render now exports `telegram_preview.jpg` from frame `0001`, and final channel/test publish sends video with `supports_streaming=True` plus that explicit first-frame JPEG thumbnail, so Telegram list/feed preview no longer depends on Fly-side media tooling or client-side HEVC thumbnail inference.
