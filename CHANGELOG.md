...
- Fixed Region Talk exact-link state resurrection: the final compact snapshot no longer rewrites stale start-of-run `post_link_queue` rows over newer online `terminal_source_rejected` decisions when row-level replay is disabled. Orchestrator metrics now treat `operator_rejected` and authoritative local/spam source links as terminal cleanup rather than exact/BGE product backlog, and processed-post projections retain `first_seen_run_id` so newly acquired work is not misreported as a rescan.
