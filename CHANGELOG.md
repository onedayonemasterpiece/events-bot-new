...
- **Operations / Daily Job Incident Closure**: for daily/scheduled production tasks, incident closure now explicitly requires a compensating rerun/catch-up after deploy when today’s slot was already missed or failed, so “fix shipped” no longer counts as recovery if the current day’s data/publication remains lost.
- **CrumpleVideo / Scheduled Recovery One-Shot Retry**: `video_tomorrow` startup catch-up and live watchdog no longer treat an earlier same-day `ops_run=success` as final delivery when the only matching session for that target date failed early with `missing video output` or `kaggle push failed`; the runtime now allows one automatic rerun through the same scheduled path and then stops auto-retrying after a second failed attempt.
- **CrumpleVideo / Telegram Poster Rescue Contract**: poster HTTP download remains the primary path, but notebook-side Telegram cache now explicitly reuses the same encrypted story auth runtime when available, so `catbox`/dead direct poster URLs can still be rescued by filename lookup without depending solely on Kaggle Secrets API.
