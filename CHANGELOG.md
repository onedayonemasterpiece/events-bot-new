...
- **LLM / Lollipop Legacy Lab**: added the experimental `lollipop_legacy.v1` benchmark variant for Smart Update copy: it keeps the current Gemma 3 baseline fact extractor as the mandatory event-facing fact floor, filters logistics/ticket/date facts out of narrative coverage, then uses Gemma 4 31b for a compact lollipop-style enhancement pass and final writer. The benchmark harness now supports `--variants baseline,lollipop_legacy`, ships five real-post fixtures, records speed ratio against baseline, and validates baseline fact-floor coverage, style/promo/report leakage, length, hooks, and the failed optional `--legacy-g4-extract` latency path.
