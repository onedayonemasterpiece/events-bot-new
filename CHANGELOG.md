...
- **Telegram Monitoring / Gemma 4 Kaggle Bootstrap Fix**: the Kaggle `telegram_monitor` runner now bootstraps the staged `google_ai` bundle before its first import by checking the kernel root, `/kaggle/working`, and `/kaggle/input`, fixing the live `ModuleNotFoundError: No module named 'google_ai'` failure on Gemma 4 kernels. Added a regression contract in `tests/test_tg_monitor_gemma4_contract.py`.
