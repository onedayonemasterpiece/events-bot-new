...
- **CherryFlash / Telegram Feed Preview + Publish-Compatible Copy**: final CherryFlash render now exports `telegram_preview.jpg` from frame `0001` and an additional `telegram_publish.mp4` in H.264/AVC for Telegram channel delivery, while the main release artifact can stay on the compact HEVC path. Final channel/test publish uses that Telegram-compatible copy with `supports_streaming=True`, so Telegram feed preview no longer depends on HEVC thumbnail behaviour.
