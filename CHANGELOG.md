...
- **CherryFlash / Compact HEVC Final Profile**: the final one-file HEVC artifact now uses a more aggressive delivery budget (`preset=slow`, `crf=28`, `audio=128k`) on the fixed `hvc1/+faststart/closed-GOP` path, so the single release file moves back toward the `<=15 MB` target instead of staying in the high-20 MB range.
- **CherryFlash / Story-Safe Shared H.264 Upload**: the shared Kaggle `story_publish.py` helper no longer forwards the raw CherryFlash HEVC file into `SendStoryRequest`. Story delivery now always normalizes video uploads into a Telegram-safe `H.264/AAC` mp4 canvas first, even when the source artifact is already `1080x1920`, which prevents `MEDIA_FILE_INVALID` while keeping channel/test publication on the single HEVC release artifact.
