...
- **CherryFlash / Compact HEVC Final Profile**: the final one-file HEVC artifact now uses `preset=slow` and `crf=24` with the fixed `hvc1/+faststart/closed-GOP` path, reducing CherryFlash back toward story-safe payload size after the previous preview-oriented HEVC settings inflated the file into the high-20 MB range.
