...
- Connected the stable `region_talk_external_blogger_evidence` YDB table to the Region Talk canonical source queue: confirmed non-local Telegram channels and VK communities/personal profiles are deduplicated, durably prioritized ahead of generic backlog, exposed in funnel metrics, and searched first with their evidence-specific locations in adaptive mode. While this cohort is pending, generic cached fast-check rows no longer consume its time budget. Added the missing Talpaki/Taplacken lexicon anchor while retaining Telegram governor, cached-entity and FloodWait safety limits.
