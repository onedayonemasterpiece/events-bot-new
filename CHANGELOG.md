...
- **Incident / Kenigsberg winter dataset not mounted**: opened `INC-2026-05-12-kenigsberg-winter-dataset-not-mounted` after the second production `/kenigsberg` smoke reached renderer startup but failed because the bot selected `period=winter` and Kaggle did not mount `koenigsberg-winter` for session `#254`. The MVP selector now defaults to the verified `1919-1940` period only, with `winter` behind explicit `KENIGSBERG_STORIES_PERIODS=winter,...` opt-in after a dedicated mount smoke. Renderer missing-dataset errors now include the mounted `/kaggle/input` directory names, and tests pin the safe period default plus winter opt-in parsing.
...
- **Kenigsberg Stories / manual Kaggle MVP render**: wired `/kenigsberg` to launch a real manual Kaggle render when `KENIGSBERG_STORIES_KAGGLE_ENABLED=1` (enabled in `fly.toml` for manual-only testing). The command now creates a `kenigsberg_story` `VideoAnnounceSession`, selects a thought from `docs/features/kenigsberg-stories/thoughts.md`, reserves an issue number, chooses a period dataset (`zigomaro/koenigsberg19191940` or `zigomaro/koenigsberg-winter`), creates a per-run `kenigsberg-session-*` Kaggle dataset, deploys the new local kernel `kaggle/KoenigsbergStories`, and lets the existing video poller post the generated MP4 to `@keniggpt`. Added `scripts/render_kenigsberg_story.py`, a first `heuristic_v1` renderer using source video datasets, `zigomaro/koenigsberg-music`, bottom crop for VEO, stripe typography, watermark, two-screen outro, H.265 story-safe encoding, and `kenigsberg_issue_manifest.json`; the poller imports that manifest back into `setting.kenigsberg_stories_state` so `/a`/`/kenigsberg ban` can map generated seconds to source-video bans after a successful run.
