...
- Fixed Region Talk's pinned CLIP loader so it no longer sets process-wide `HF_HUB_OFFLINE=1` and accidentally disables the later pyiqa/NIMA weight lookup; CLIP remains network-isolated through `local_files_only=True` while independent image components retain their own model-loading policy.
