...
- **Added: KGD80 social activity report and raffle scoring contract**: added a
  daily evening automatic private report for `80 историй о главном` VK social
  activity, using verified VK post metrics (`views/likes/comments/reposts`) and
  a documented bonus model for future raffle weighting. The report is sent only
  to the resolved superadmin chat, marks itself as automatic, and keeps social
  bonus separate from cross-draw winner damping.
