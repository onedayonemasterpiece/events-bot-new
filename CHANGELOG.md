...
- Fixed three Region Talk idempotency dead-ends found by final-SHA/live review: CandidateReport now collapses same-run exact/history observations before vector planning, historical `llm_decision=accept|reject` rows cannot reopen text restoration, and the isolated BGE worker shares CandidateReport's semantic bank and overwrites an existing model/post/text PK when its stored bank contract is stale instead of returning a misleading empty batch.
