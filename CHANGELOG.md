...
- **Incident / Telegram Monitoring Work Schedule False Skips**: opened `INC-2026-04-30-tg-monitoring-work-schedule-false-skips` after `@kenigatom/496` and `@kraftmarket39/199` were extracted by Telegram Monitoring but skipped as `skipped_non_event:work_schedule`; Smart Update now limits this deterministic guard to explicit work-hours/closure wording and no longer infers schedule notices from museum/library venue words plus dates, leaving normal event meaning to the LLM/extractor path.
