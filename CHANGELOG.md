...
- **Incident / Kenigsberg command silent during Gemma retry**: opened `INC-2026-05-12-kenigsberg-command-silent-during-gemma-retry` after production `/kenigsberg` appeared silent for 142 seconds while the pre-launch Gemma 4 rewrite retried a provider `503 UNAVAILABLE`. The command now sends an immediate operator acknowledgement after reserving the issue/thought, caps the rewrite wait at 45 seconds, and falls back to deterministic story text if the LLM is overloaded, so Kaggle launch remains visible and auditable.
