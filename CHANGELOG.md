...
- Added a read-only Smart Update identity-gate rollout audit (`scripts/inspect/audit_identity_gate_rollout.py`) and embedded its decision/veto/fail-safe/vector-error counters into the scheduled exhibition duplicate audit; low-risk vector recall failures are now persisted as `suppressed_vector_error` instead of existing only in runtime logs, and identity-gate exception handling records a single fail-safe decision row.
