...
- Subscriber Acquisition semantic retrieval now applies freshness gates: comments older than 365 days are excluded from semantic analysis by default, surfaces with no observed activity within 92 days are marked `reject_stale_inactive` while remaining eligible for future revival, and XLSX reports include `summary_counts` totals by platform/type/status.
- Subscriber Acquisition freshness gates now keep old comments as `historical_calibration` rows for canonical-question extraction and semantic QA while restricting monitoring/LLM candidates to fresh rows; VK discovery can also scan explicitly discovered/seeded personal profile walls (`vk:id...`) when their wall/comments are public and active.
...
- Subscriber Acquisition seed rotation now remembers resolved/rejected Telegram catalog channels when building the next Kaggle payload, so static Telega.in seeds do not reintroduce the same no-comments/resolved channels as fresh work.; terminal TG handles are also passed into the runtime so rediscovered links in scanned messages are not queued again.
...
- Added Tretyakov daily announcement premium venue markers: `🖼🖼` before visible Tretyakov location lines and `🖼🖼` instead of `🚩` for structured Tretyakov rows in `ДОБАВИЛИ В АНОНС`.
- Added `lovekenigofficial` `🤘`/date-`🎟` Telegram premium emoji updates: rock-concert title/category icons become `🤘`, date/calendar `📅` can be premiumized to custom `🎟`, ticket rows use distinct `🎫`, paid ticket prices gain `💰` before the number with textual `руб.` removed, and future rock event announcements render `🤘` before the Telethon editor premiumizes it.
...
- Fixed rock-concert premium emoji mapping: `🤘` now uses the actual rock/guitarist custom emoji from `lovekenigofficial` (`5404517529362128309`) instead of the neighboring cathedral/heart symbol, and the editor corrects older wrong custom-emoji entities in-place.
- Hardened Tretyakov premium venue markers: the editor now drops stale title-level `🖼️` custom emoji entities and re-corrects live `🖼🖼` pairs whose visible text is right but document ids regressed.
...
- Fixed Telegraph page HTML reads to use python-telegraph `content` when `return_html=True`, preventing maintenance backfills from treating existing pages as empty.
- Fixed Telegraph event-page fallback so Telegraph flood-control/transient edit failures no longer create replacement pages and orphan already-published links; fallback remains limited to `PAGE_ACCESS_DENIED`.
- **Incident / Telegraph footer backfill content loss (INC-2026-06-27)**: formalized the production Telegraph content-loss incident from the Max backfill and made it an active regression contract for all future Telegraph bulk maintenance.
...
- **Promo campaigns / popular Telegram reposts**: added a weighted-popularity
  ...
  `klgdevents`/`kenigeventsofficial` with configurable ×4 weight, forwards the
  corresponding `@kldevents` post to `@kenigevents`, accepts stored
  `t.me/c/...` event-post links as forwardable Bot API message ids, and skips
  ranked events that do not yet have a forwardable event-flow post.
...
- **Incident / VK community Channel wrong surface (INC-2026-06-25)**: disabled
  ...
  community Channel tab. `vk_channel_publish` now creates no public exposures
  and returns `vk_community_channel_post_api_unsupported` until a documented or
  verified non-Messenger VK community Channel posting API exists; the compact
  title/info/description/CTA copy builder remains available for that future
  transport. Production activity was disabled, the mistaken `VK_CHANNEL_SENT`
  exposure was invalidated as `FAILED_WRONG_SURFACE`, and the messenger peer
  secret was removed.
- **Incident / VK actuals published past events (INC-2026-06-24-vk-past-actuals)**: investigated and mitigated a fresh `klgdevents` VK post for the already-finished `Калининград: Город-сад...` event. Deleted/reconciled stale VK postponed/live posts and added prevention so Promo VK selection excludes same-day timed events after start, Afisha Engagement debug-shadow copies cannot be scheduled at/after event start, and managed VK/TG fanout jobs skip one-day timed events that already started.
- **Incident / future event date + source-default venue regressions (INC-2026-06-24)**: audited the active future production inventory, repaired high-confidence date/venue/non-event/description defects, and edited affected public `@kldevents` captions. The Telegram Monitoring prompt/schema now treats Russian numeric dates as `DD.MM`, keeps month-word/hashtag dates authoritative, forbids gate/floor/address/price/coordinate numbers as date/time anchors, skips retrospective reports without a future invite, and states that event-local venue/address evidence beats a source default. Added a narrow single-event source-date correction guard and server-side candidate grounding so an explicit offsite address can replace a seeded `source.default_location` when the extractor omitted venue. Production mitigation repaired events `2872`, `4795`, `5493`, `5766`, `5783`, `6213`, `6223`, `6252`, `6276`, `6286`, `6333`, and `6334`; follow-up repairs resolved the `5538/6141` time conflict as one `15:00` concert, cancelled duplicate rows `6141`, `6287`, and `6300`, deleted duplicate TG/VK managed posts, and corrected canonical TG/Telegraph/VK surfaces. A second future-inventory duplicate audit repaired/merged `5266/4452`, `6376/6352`, `5525/5511`, `5528/5512`, `5379/4961`, `5415/5581`, and `6369/5415`; Smart Update match/create prompting now treats same event source-time conflicts as merge candidates while preserving explicit multi-session schedules, and public description sanitization strips LLM editor meta before Telegraph render. Telegraph rendering now ignores non-active linked occurrences when listing “Другие даты”.
- **Fixed: Poll to Repost topic underfill after first LLM plan**: production topic creation now keeps semantic grouping LLM-first by adding a second grounded LLM repair pass before the conservative deterministic fallback. This prevents a rich eligible inventory from skipping the public poll solely because the first LLM plan validated to too few options. Tracks `INC-2026-06-23-poll-repost-topic-underfill-no-poll`.
...
- **Added: VK location marker v1**: managed VK event posts now best-effort add
  ...
  `vk_location_marker_cache`, and retry `wall.post` without marker params if VK
  rejects them. Marker-bearing community posts now prefer the user VK actor
  because production verification showed group-token posts accepted `lat`/`long`
  but published without a visible `geo` marker.
- **Fixed: Smart Update speaker roster preservation**: G4 split-create now treats multi-speaker lecture/public-talk rosters as mandatory named facts, keeps them in the description instead of collapsing to generic categories, and runs an LLM logistics-cleanup rescue before dropping a split-writer draft. Production event 6244 was repaired and the stale VK duplicate was removed. Tracks `INC-2026-06-20-tg-speaker-roster-dropped`.
...
- **Fixed: LLM-first decorated temporal location review**: Telegram Monitoring Gemma 4 extraction schema and the producer venue-review prompt now explicitly forbid emoji/bullet-prefixed temporal fragments such as `🤗Завтра` in `location_name`, and the LLM review trigger normalizes that decorated shape before server import / Smart Update safety nets see it. This keeps the fix at the producer contract first, with deterministic guards only as fail-closed backup. Follows up `INC-2026-06-18-tg-location-prose-still-extracted`.
...
- **Changed: Kenigsberg story cadence and catch-up retry cap**:
  ...
  temporarily reduced from daily to Fridays at `19:30 Europe/Kaliningrad`;
  startup catch-up also runs only for that weekly slot and stops after repeated
  same-day failed Kenigsberg story sessions to avoid relaunching on every deploy
  restart. Tracks
  `INC-2026-06-12-kenigsberg-story-media-invalid-catchup-loop` and is covered by
  `tests/test_scheduling.py` and `tests/test_kenigsberg_stories.py`.
...
- **Changed: Poll to Repost LLM-first repost comments**: final public comments
  before forwarding are now generated by a separate LLM composer with a strict
  `{{EVENT_LINK}}` placeholder, so the thank-you/theme/recommendation/feedback
  flow stays intact while wording varies naturally and avoids broken agreement
  such as "предложил Лекция". Deterministic code only validates, escapes, links
  the event title, and falls back to neutral wording if the composer fails.
...
- **Changed: release governance uses manual Fly deploy only**:
  ...
  through the shared devserver token file
  `/home/dev/.config/fly/release.env` and Codex/Claude session-history recovery
...
- **Added: VK guide digest engagement/hook cards (2026-06-05)**: the guide `new_occurrences` VK digest now appends 1–3 rendered 1080×1080 "hook" cards (a single marketing curiosity question in big editorial type) *after* the real excursion afishas, so the VK grid leads with a catchy phrase straight from the image. Slot math is additive — `available_slots = max(0, 9 − afisha_attachments)`, cards never displace afishas, and the whole step is best-effort (any generate/render/upload failure is logged and swallowed; the digest still publishes with its afishas). A single GPT-4o call (`ask_4o`, `response_format=GuideHookCards`, same curiosity-hook voice as the announce rewrite) generates a grounded hook per promising occurrence and returns only the strongest, most *diverse* `cap`; each hook is sanitized fail-closed (no emoji/URLs/usernames/phones/prices/dates/times/CTAs, 3–14 words, ≤90 chars, dominant прогулка/экскурсия/джип-тур term preserved). Cards lead the VK grid (`attachments = cards + afishas`). All cards in one post share a single palette (`select_post_palette(seed=issue_id)`) that rotates across publication days; centered text inside the `x150–930/y160–920` safe-zone, bundled Cyrillic grotesque `assets/fonts/Cygre-ExtraBold.ttf`. Card footer is `date · guide` (deterministic from the occurrence). New `guide_excursions/hook_cards.py` + `guide_excursions/hook_card_render.py`, wired into `publish_latest_guide_digest_to_vk` (both normal publish and `repair_existing` edit) via `_build_guide_vk_hook_card_attachments`. No new on/off flag (cards are part of the digest publish, fewer failure points). Covered by `tests/test_guide_hook_cards.py`. Canonical doc: `docs/features/guide-excursions-monitoring/README.md` → "Hook-карточки (engagement cards)". Requirement clarifications vs the original backlog spec (`docs/backlog/features/guide-excursions-monitoring/engagementcards.md`): (a) `9 − existing_images` assumed afishas already in the VK post — confirmed true on `origin/main` (commit `038c3bda`), so cards are strictly additional attachments; (b) the card hook is a new marketing question, not the existing grounded `main_hook`. Requirement overrides recorded 2026-06-05 (user review): card order changed *afishas-first → cards-first*; palette changed *distinct-per-card → one palette per publication, varying by day*; footer changed *marketing slogan ("Экскурсия недели") → date · guide*; hooks must be **questions** (sanitizer rejects any hook without exactly one trailing "?"); main type size is **uniform across all cards in a post** (smallest fitting size applied to all) so adjacent cards don't read as different weights/widths.
- **Fixed: VK wall-linked stories (2026-06-05)**: CherryFlash/КОНБ VK story
  fanout is now wall-first (`vk_wall` then `vk_wall_story`): the wall post is
  created first, then the mp4 is uploaded to VK stories with `link_url` pointing
  at that wall post. Promo VK stories remain image-only and omit `link_url`,
  preventing VK from drawing a white wall-post caption/card block under the
  image.
- **Fixed: VK past-event prune backlog drain + dry-run reporting (2026-06-05)**: a successful real `wall.delete` now clears the event's `source_vk_post_url` (and matching `vk_repost_url`), so a capped run (`VK_POST_PRUNE_LIMIT`) drains a large backlog instead of re-processing the same already-deleted first-N candidates until the 7-day `cleanup_old_events` removes their rows. Failed deletes keep the URL for retry; `dry_run` clears nothing. The scheduled summary now distinguishes modes (`VK prune (dry-run): would delete=N of M …` vs `VK prune: deleted=N of M …`) and the `vk_post_prune_ok` log line carries `dry_run=`. First prod dry-run reported 824 eligible past-event posts.
- **Added: auto-delete past-event VK posts (2026-06-05)**: new `vk_post_prune_scheduler` runs twice a day (default `02:30,14:30` Europe/Kaliningrad, `ENABLE_VK_POST_PRUNE` enabled in production) and deletes managed `klgdevents` event posts whose event is already in the past and that gained no reposts/story shares (`reposts.count == 0`), so VK stops recommending stale events. Safety contract: only posts whose `Event.source_vk_post_url` resolves to `-VK_EVENTS_GROUP_ID` are eligible; external VK-import source walls, pinned posts, future/ongoing events (via `end_date`), and daily/poll/promo posts are never deleted (past-ness is read from the DB, not from arbitrary wall scanning). The job joins `_HEAVY_JOB_IDS` so it skips (and notifies `ADMIN_CHAT_ID`) when another heavy job holds the gate instead of competing for the VK token. Bounded by `VK_POST_PRUNE_LIMIT` (default `50`) with a `VK_POST_PRUNE_DRY_RUN` observe-only mode. Canonical doc `docs/features/vk-publishing/autodeletevkposts.md`; covered by `tests/test_vk_post_prune.py`.
- **Incident / VK published event quality batch (2026-06-05)**: promo `vk_publication` now shares the Telegram-origin no-media fail-closed guard with managed `vk_sync`, ignores failed/invalidated exposures in rolling-window counts, tries Telegraph media recovery for empty `event.photo_urls`, records `FAILED_NO_MEDIA` audit rows with recovery instructions when a Telegram promo candidate still has no media, logs `campaign_id` / `activity_id` / `event_id` / `photo_urls_count` / `attachments_count`, and raises `vk_sync_missing_media_for_telegram_event` before `wall.post` instead of publishing text-only promo VK posts. Added `Культурное место, Остров Канта, Калининград` plus aliases to the location reference so source-grounded island venue mentions do not normalize to unrelated `Место Силы`. Tightened LLM-first extraction prompts to reject historical/background exhibition dates such as `9 октября 1947 года` as future event dates unless the source also provides a concrete upcoming attendee slot.
- **Promo VK stories for campaign activity**: added `vk_story` promo activity
  ...
  normalized `promo_exposure` evidence. Promo VK exposure recording now retries
  transient SQLite locks after external VK side effects, preventing the next
  scheduler tick from repeating an already-created post only because audit
  persistence briefly failed. The built-in `80 историй о главном` campaign now
  idempotently seeds two daily stories from recent `klgdevents` festival event
  posts into both `vk.com/klgdevents` and `vk.com/kenigeventsofficial`; `/promo`
  and `/promo report` include story activity/report rows. Covered by
  `tests/test_promo.py`.
...
- **Promo `/promo` card — VK activities + per-activity statistics**: the
  interactive `/promo` campaign card (`handlers/partner_promo_cmd.py`, the menu
  партнёр/админ use) now renders `vk_publication` / `vk_repost` activities with
  human-readable labels (target community, `source → target`, `минимум N/окно`).
  The «📊 Статистика» screen was reworked from a flat surface tally into a
  per-activity breakdown that, for each activity, shows total actions, a
  rolling-window counter (`промо-действий за Nч: X / цель`), and the latest
  posts/reposts as clickable VK links (with the repost source link). Statistics
  now count `VK_SCHEDULED` exposures (promo VK posts land in the community
  postponed queue) — previously they were under-counted. Unlinked/legacy
  exposures fold into a «Прочее» section. (Earlier the VK rendering/stats were
  mistakenly added only to the secondary `/promo <args>` text interface in
  `handlers/promo_cmd.py`, not the card the operator actually opens.)
...
- **Incident / raw provider SDK response leaked into posts + near-duplicate posters + location-as-title placeholders (INC-2026-05-29)**: a public `klgdevents` VK post (`wall-231920894_1613`) and 16 active future events had their `description` set to the **stringified `repr` of a google-genai `GenerateContentResponse`** (model chain-of-thought, token counts, http-response wrapper) instead of a real description. Root cause: `google_ai/client.py` `_extract_text` had a `return str(resp).strip()` last-resort branch that fired when the model (`gemma-4-31b-it`) returned **only a thought-channel part and no answer** (it spent the whole `max_output_tokens` budget thinking — `thoughts_token_count=1897`, truncated mid-sentence). The non-empty repr both defeated the adjacent `empty_response` `ProviderError` guard and short-circuited the existing Gemma→GPT-4o fallback, so a recoverable case published garbage silently (no `ProviderError` in 24h of logs). Fix: `_extract_text` now returns `""` (never stringifies the response) so the empty-response guard fires and the 4o fallback runs, and logs `google_ai.empty_response` with `finish_reasons`/`thought_only`/`thoughts_token_count`. Defense-in-depth: `markup.looks_like_genai_response_dump` detector drops such a dump in `_sanitize_description_output` (Telegraph/daily/VK), `sanitize_for_vk` returns `""` on a dump, and `job_sync_vk_source_post` falls back to `source_text` when `event.description` looks like a dump. **Image near-duplicates** (same report): live poster dedup was exact-match only (`sha256` bytes + exact URL); the perceptual hash was a storage object-key only and `None` on the VK path. Now a real `phash` (dh16, shared with the storage key) is threaded into poster candidates incl. the VK intake path, and `_apply_posters` collapses near-duplicate posters by Hamming distance (`SMART_UPDATE_POSTER_NEAR_DUP_HAMMING`, default 10) keeping the highest-quality survivor. **Location-as-title placeholders** (~44 active future events, ~9%): when the VK-parse title tripped the brittle `_missing_title_tokens` ground check it was replaced by a generic `"{event_type} — {venue}"` placeholder (`Концерт — Кафедральный собор`) with no recovery — even though the source almost always contained a real title. Smart Update create now runs a dedicated native-schema grounded title-recovery stage (`_llm_recover_event_title`) when `final_title` trips `_is_generic_title_event_type_venue`; the recovered title is accepted only if grounded in the candidate's facts/source/OCR and not itself a generic placeholder, otherwise the placeholder is kept. The 16 leaked descriptions and the placeholder titles are repaired via `scripts/incident/`. Canonical record: `docs/reports/incidents/INC-2026-05-29-genai-response-repr-leak.md`. Tests: `tests/test_google_ai_client.py::test_thought_only_response_raises_instead_of_leaking_sdk_repr`, `tests/test_genai_dump_and_poster_dedup.py`.
- **VK media dedup root fix (INC-2026-05-29 follow-up)**: `_apply_posters` now normalizes the persisted `Event.photo_urls` set by perceptual hash after adding Smart Update posters. This fixes the actual source of duplicate VK attachments where an older site/CDN thumbnail (for example qTickets) stayed in `photo_urls` beside the newer managed-storage poster for the same image; the VK publish boundary also has a final near-duplicate guard before upload.
- **Smart Update title recovery (INC-2026-05-29 follow-up)**: generic `"<event_type> — <venue>"` placeholders now get a second LLM-first recovery pass when no formal organizer title exists: the fallback can build a short public heading from grounded source/OCR/fact material such as a festival+performer, central artwork/programme, or holiday/theme. Acceptance was tightened so every meaningful title token must be grounded in the same source/OCR/fact corpus; title-recovery labels now route through the stable facts-stage model instead of burning Gemma retries on thought-only truncation.
- **VK title-only resync (INC-2026-05-29 follow-up)**: managed VK event-post sync hashes now include `event.title` as well as the body text, so a grounded title repair re-edits an existing `klgdevents` post instead of being skipped as "unchanged".
- **VK publishing follow-up (INC-2026-05-29)**: VK text sanitizing now breaks inline Markdown `###` headings into plain VK heading blocks before publish/edit, `post_to_vk` / `vk_sync` resolve `postponed_id` URLs to real wall ids through the user actor with a short retry for VK visibility lag, raises the default 256-bit `dh16` near-duplicate image threshold to 20, quotes image URLs with spaces before hashing, and canonicalizes the known place alias `Янтарное => Янтарный`.
- **Incident / Guide VK digest missing media (INC-2026-05-29)**: Guide excursion VK fanout to `vk.com/uhtykaliningrad` now uploads materialized guide media assets and sends them as attachments in the same `wall.post` as the plain-text digest; existing target posts can be repaired through the same path with `wall.edit` while VK still allows edits. Shared postponed `post_to_vk` now resolves VK's returned `postponed_id` to the real wall `id` before storing the URL, so later `wall.getById`/`wall.edit` operate on the actual post, and repair metadata is normalized to that real id too. The VK lead line now shows exact digest dates (`Новые экскурсии: 3 выхода, 30 мая, 2 июня и 7 июня`) instead of only month names. Added regression tests for attachments and refreshed the date-sensitive VK postponed test fixture.
- **LLM / gpt-4o daily spend guard**: direct `gpt-4o` chat-completions now check today's persisted `token_usage` before sending. If current `gpt-4o`/`gpt-4o-2024-08-06` usage plus the next estimated request would exceed `FOUR_O_GPT4O_DAILY_TOKEN_LIMIT` (default `950000` tokens per UTC day), the call falls back to `FOUR_O_GPT4O_FALLBACK_MODEL` (`gpt-4o-mini` by default) and logs `four_o.budget_fallback`.
- **Incident / event quality duplicates and source-grounded identity (INC-2026-05-27)**: Smart Update now treats prose-like `location_name` leaks as unsupported venue anchors: such candidates can only merge by `date + explicit time + related title + near-identical source_text`, and otherwise fail closed as `invalid:prose_location` instead of creating public cards with prose venues. Telegram Monitor and master title prompts now prefer caption/source event names over poster slogans/CTA (`Живой сундук` vs `Читайте бумажные книги!`), while Smart Update rich-facts/create-bundle prompts preserve organizer/community/inspiration identity facts literally from source evidence (`ОКЦ на Горького 116`, `Плоский мир Терри Пратчетта`). Added regression tests and replay fixtures for the `Дачники` duplicate/prose-venue and `Живой сундук` writer/title incidents.
- **Incident / Kaggle GPU queue auto-demote fallback (INC-2026-05-26 round 3)**: production hit a Kaggle P100 queue of "position 369" — every push sat in QUEUED indefinitely. Empirically verified (and the user confirmed via Kaggle UI side-by-side comparison): Kaggle's public Python SDK can cancel only `create_kernel_session` interactive sessions, not the batch runs we trigger via `save_kernel`/`kernels_push` — the batch session_id is never exposed. So we can't unstick a queued run; we can only redirect *future* pushes to a different accelerator. Added `video_announce/accel_pref.py`: per-resolved-Kaggle-slug preference recorded in the `setting` table with a **randomized TTL of 18-30h** (jittered so the cron slot that paid the 5-min queue cost on demote day doesn't pay it again the same time tomorrow — expiry drifts across days). Poller now treats `>5 min in QUEUED` as a demote trigger: writes pref `t4x2` (next ladder rung after `p100`), marks the in-flight session FAILED (lock releases immediately via the round-2 kernel-slug guard), and sends a Telegram warning with the new expiry. Ladder for video-announce kernels is `p100 → t4x2 → exhausted (FAIL)` — CPU intentionally excluded (Blender 3D on CPU is not validated). `kaggle_client.deploy_kernel_update` accepts `machine_shape=...` and injects it into kernel-metadata so Kaggle uses the chosen accelerator. The scenario layer reads the pref before each push (`scenario._push_kernel`) and threads `machine_shape` through. Env knobs: `KAGGLE_QUEUE_DEMOTE_THRESHOLD_SEC` (default 300), `KAGGLE_ACCEL_PREF_TTL_HOURS_MIN`/`MAX` (default 18/30). Zombie Kaggle sessions in QUEUED can't be cancelled programmatically; they sit until Kaggle eventually schedules them — accepted as cost (~5 min P100 wasted per demote cycle ≈ <2% of the 30h/week unified GPU quota confirmed via `kaggle.com/settings`). Tests: `tests/test_accel_pref.py` (12 cases) + full regression on `test_video_announce_poller`, `test_video_announce_v_pipeline`, `test_partner_track_scheduling`, `test_partner_track_isolation`, `test_video_announce_popular_review`, `test_kaggle_client` — 69 cases green.
- **Incident / video-announce round-2 — Kaggle kernel-slug clash (INC-2026-05-26 round 2)**: the round-1 fix made `start_render`'s lock profile-scoped, but partner tracks `popular_review_konb` and `popular_review_eco` both push to the same `zigomaro/cherryflash` Kaggle kernel slug (different profile_keys, same physical kernel). Sessions #535 (KONB) and #538 (eco) ended up both `RENDERING` in DB and both `kernels_push`ed to the same kernel → Kaggle queued v126 and v127 of `zigomaro/cherryflash` as **independent** sessions (per [kaggle-api#388](https://github.com/Kaggle/kaggle-api/issues/388) — `kernels_push` does NOT cancel previous versions), so they contended for the account's concurrent-GPU-session quota and both stuck in QUEUED. Fix: replaced the profile-scoped lock with a **resolved-Kaggle-slug**-scoped lock. New helper `kaggle_client.resolve_kaggle_slug(kernel_ref)` normalizes `local:CherryFlash` → the underlying Kaggle id from `kernel-metadata.json` (`zigomaro/cherryflash`), so two RENDERING sessions whose refs resolve to the same Kaggle slug now serialize correctly even when their `profile_key` differs. `has_rendering` accepts an optional `kaggle_slug=` filter (keeps the legacy `profile_key=` for callers that genuinely run on independent kernels). `start_render`, `start_import_render`, and `run_partner_track_pipeline` now compute the prospective resolved slug and lock on it. Manual prod mitigation: marked #535 + #538 FAILED to free the lock; user must Stop (not just Dismiss) leftover Kaggle sessions via the UI since the Kaggle Python API has no `kernels_cancel`.
- **Incident / video-announce stuck rendering + cascade SELECTED sessions (INC-2026-05-26)**: production `/v` froze with `Рендеринг уже запущен` for ~5 hours and 24 cascading partner-track SELECTED sessions piled up. Two independent bugs collided. (a) **Poller never treated Kaggle's `CANCEL_ACKNOWLEDGED` as terminal.** When a Kaggle CherryFlash kernel was cancelled (out-of-band on Kaggle UI or via API), `_poll_kernel_status` in [video_announce/poller.py](video_announce/poller.py) only treated `complete`/`error`/`failed` as terminal — `cancel_acknowledged` / `cancel_requested` / `cancelled` kept the poller in `sleep+continue` until the full `VIDEO_KAGGLE_TIMEOUT_MINUTES` deadline, leaving the session pinned in `RENDERING` and the global render lock held. Fix: expanded the terminal-state set to include `cancel_acknowledged`, `cancel_requested`, `cancelled`, `canceled`; the existing FAILED-transition + log download + dataset cleanup path now also runs for cancels. (b) **`start_render` global lock contradicted partner-track profile-scoped lock.** Partner-track pipelines correctly checked `has_rendering(profile_key=...)` in [video_announce/scenario.py:1765](video_announce/scenario.py#L1765) (intent per 2026-05-18 incident comment: eco render must not block KОНB), but `start_render` and `start_import_render` still did global `has_rendering()` at session-start time, so any stuck RENDERING session in **any** profile (here `popular_review` #508) blocked **all** partner tracks. Each partner watchdog tick then created a new SELECTED session, never reached Kaggle handoff (`_video_session_has_remote_handoff` requires `kaggle_dataset` + non-`local:` kernel ref), `_partner_track_session_exists_today` saw no remote handoff and re-dispatched 10 min later — feeding the cascade. Fix: `start_render` and `start_import_render` now load the session first and call `has_rendering(profile_key=sess.profile_key)`, consistent with the DB-level unique RENDERING-per-profile index. Manual prod mitigation: `UPDATE videoannounce_session SET status='FAILED'` on #508 and the 24 cascaded SELECTED sessions to free the lock. Tests: `tests/test_video_announce_poller.py`, `tests/test_video_announce_v_pipeline.py`, `tests/test_partner_track_scheduling.py`, `tests/test_partner_track_isolation.py`, `tests/test_video_announce_popular_review.py` all green (60 cases).
