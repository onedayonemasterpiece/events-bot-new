...
- Fixed Region Talk final-copy style regression: staged writer v9 and its critic now forbid the `не …, а …` adversative AI-cliché family, deterministic validation triggers at most one LLM-first rewrite and then fails closed, and notifier/planner/caption readiness blocks stale or manually injected copies containing the pattern. Added exact-URL forced regeneration, unpublished-only backfill selection, versioned cache/review invalidation, and production-candidate backfill support.
