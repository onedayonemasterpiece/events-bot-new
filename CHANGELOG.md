...
- **Promo campaigns / VK channel activity**: added `vk_channel_publish` for
  compact event promos in the VK Channel of `klgdevents` ("Полюбить Калининград
  Афиша"). It uses the documented `messages.send` contract only when explicit
  `VK_AFISHA_CHANNEL_PEER_ID(S)` is configured, sends a Telegram-channel-like
  one-link event copy without footer links/hashtags, records
  `VK_CHANNEL_SENT` promo exposure evidence, and appears in the promo campaign
  card/statistics with enable/disable controls for the `80 историй о главном`
  campaign. For VK-channel-style peers that reject `group_id` with `Cannot
  message as group`, the user-token send retries the same explicit peer id
  without `group_id`.
