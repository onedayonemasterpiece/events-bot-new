...
- Fixed confirmed-blogger scheduling so the bounded source pool always exposes the single safe uncached Telegram lane and exhausts never-scanned Telegram/VK evidence before any cached delta rescan. Added first-scan-versus-rescan telemetry, preserved all human-like/FloodWait limits, fixed VK screen names being counted as Telegram source aliases, and retained source-history counts in the terminal YDB heartbeat. VK `club<ID>`/`public<ID>` URLs now retain their community prefix instead of silently scanning a positive user id; API-100 domain failures get one official screen-name resolution retry, while successfully unresolved stale evidence URLs become terminal audit rows instead of wasting every subsequent history slot.
