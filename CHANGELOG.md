...
- **Incident / VK Smart Update False Skips**: opened `INC-2026-04-28-vk-smart-update-false-skips` after two normal VK events were rejected as `skipped_festival_post` and `skipped_non_event:online_event`; Smart Update no longer treats `онлайн-регистрация` as online-only, festival routing rescues a single concrete event inside a cycle/program from `festival_post`, and the event-parse prompt now distinguishes whole-festival posts from `event_with_festival` single events.
