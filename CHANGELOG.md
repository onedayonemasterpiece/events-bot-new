...
- **Fixed: Kenigsberg story Telegram media profile and catch-up retry cap**:
  `/kenigsberg` production stories now use the shared H.264/`avc1` story-safe
  upload path instead of the CherryFlash HEVC-native bypass that triggered
  Telegram `MEDIA_FILE_INVALID` in Kaggle session `#661`; startup catch-up now
  stops after repeated same-day failed Kenigsberg story sessions to avoid
  relaunching on every deploy restart. Tracks
  `INC-2026-06-12-kenigsberg-story-media-invalid-catchup-loop` and is covered by
  `tests/test_scheduling.py`, `tests/test_video_announce_story_publish.py`, and
