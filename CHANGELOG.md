...
- **LLM / Lollipop G4 Fast Lab**: added the experimental `lollipop_g4_fast.v2` Smart Update lab variant: parallel source-local Gemma 4 31b extraction, coverage-first deterministic pack/layout over LLM-owned fact labels, final `writer.final_4o`, benchmark support for `--variants lollipop_g4_fast`, and regression coverage for literal-title fidelity, grouped casts, source-local passthrough, and no semantic repair/reroute layer. The latest quality pass tightens musical/stage extraction to keep plot/character premise, gives people-heavy stage cases an explicit narrative body slot before cast/credits, caps extractor output at `1600` tokens with short evidence spans, and hardens final-writer validation against audience-template roots, weak dangling participles, `наполн...`, `уникаль...`, `в программе представлены`, and missing names from people/role facts. Paired live evidence now passes validation and the temporary `<=2.5x` hard cap, while the primary `<=1.5x` latency target remains open.
