...
### Fixed
- **Scheduler / SQLite Lock Storm Tolerance**: `ops_run` bootstrap/finish/startup-cleanup and `vk_review` recovery helpers (`release_stale_locks`, `release_due_deferred`, `release_all_locks`) now retry transient `database is locked` write/commit failures, so short SQLite contention no longer cascades into scheduler recovery degradation.
- **VK Auto Queue / Terminal Rate-Limit Cutoff**: provider-side `429` defers now increment persisted `vk_inbox.attempts` and stop at `VK_AUTO_IMPORT_RATE_LIMIT_MAX_DEFERS` (default `3`), marking the row `failed` instead of resurfacing the same problematic VK post forever in future batches.
- **VK API / Blocked App Token Fallback**: `_vk_api` now treats VK `code=8 / Application is blocked` as a no-retry signal for the current actor/token and falls through to the next available token path, preventing `/daily` shortlink generation from stalling on a bad user-token path when a valid group token exists.
