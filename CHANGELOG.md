...
- **Incident / Telegram Monitoring Work Schedule False Skips**: opened `INC-2026-04-30-tg-monitoring-work-schedule-false-skips` after `@kenigatom/496` and `@kraftmarket39/199` were extracted by Telegram Monitoring but skipped as `skipped_non_event:work_schedule`; Smart Update now requires a real work-hours headline/timetable shape before applying the museum/library schedule fallback, so normal lectures, shows, and festival slots at library/museum venues or addresses like `Музейная аллея` stay importable.
