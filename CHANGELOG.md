...
- **Incident / Kenigsberg deterministic text fallback quality**: opened `INC-2026-05-12-kenigsberg-deterministic-text-fallback-quality.md` after issue `#3` launched with `text=fallback` and produced visibly broken semantic cuts. The Kenigsberg text rewrite step now uses `gemini-3.1-flash-lite` directly instead of Gemma 4, requires a valid LLM JSON payload with at least two coherent `scene_lines`, disables deterministic text splitting in the Kaggle renderer, and fails closed before Kaggle launch when LLM text preparation is unavailable.
