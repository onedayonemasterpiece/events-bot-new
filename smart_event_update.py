...
_RELATIVE_DATE_EVENTNESS_RE = re.compile(r"(?iu)\b(?:сегодня|завтра|послезавтра)\b")
...
def _candidate_needs_llm_eventness_review(candidate: EventCandidate, text: str | None) -> bool:
    ...
    if _candidate_date_is_inferred(candidate, is_canonical_site=False) and (
        not str(candidate.time or "").strip()
        or bool(_RELATIVE_DATE_EVENTNESS_RE.search(raw))
    ):
        return True
