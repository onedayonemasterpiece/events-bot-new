...
OUT="$ROOT/artifacts/codex/email-postbox-events"
STAGE=$(mktemp -d)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$OUT"
cp "$ROOT/serverless/email_postbox_events/index.py" "$STAGE/index.py"
cp "$ROOT/serverless/email_postbox_events/requirements.txt" "$STAGE/requirements.txt"
